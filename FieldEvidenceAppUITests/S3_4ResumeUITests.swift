...
final class S3_4ResumeUITests: XCTestCase {
    ...
    func testRelaunchAfterWideResumesExactCloseAndFinishesOneReport() throws {
        ...
        app.launch()
        let relaunchedCloseHeading = element(in: app, identifier: "s3.capture.heading")
        XCTAssertTrue(relaunchedCloseHeading.waitForExistence(timeout: 20))
        XCTAssertEqual(relaunchedCloseHeading.label, "2 of 2 · Close view")
        ...
        XCTAssertFalse(
            element(in: app, identifier: "s3.receipt.view-report").isEnabled
        )
        XCTAssertFalse(element(in: app, identifier: "s3.receipt.share").isEnabled)

        let screenshot = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        screenshot.name = "S3.4 relaunched check value receipt"
    ...
    private func launch() throws -> XCUIApplication {
        let bundle = Bundle(for: S3_4ResumeUITests.self)
