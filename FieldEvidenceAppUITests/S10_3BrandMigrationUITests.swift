...
class S10BrandMigrationRouteUITestCase: XCTestCase {
   ...
   private func acceptImportedPhoto(
    ...
    private func diagnoseDefaultLightDiagnosticsPositioning(
        ...
        let startScreenshot = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        startScreenshot.name = "S10.4 default-light diagnostics positioning start app"
        startScreenshot.lifetime = .keepAlways
        add(startScreenshot)
        let startTree = XCTAttachment(string: app.debugDescription)
        startTree.name = "S10.4 default-light diagnostics positioning start accessibility tree"
        ...
        var terminalReason = "six-attempt diagnostic bound exhausted"
        var finalMinimumShift: CGFloat = 0
        var finalMaximumShift: CGFloat = 0
        ...
        for attempt in 0..<6 {
            ...
            finalMinimumShift = minimumShift
            finalMaximumShift = maximumShift
            var sample: [String: Any] = [
                "attempt": attempt,
                "minimumShift": Double(minimumShift),
                "maximumShift": Double(maximumShift),
                "stagingCountBefore": stagingCount,
                "upwardUndertravelBefore": Double(upwardUndertravel),
                "downwardUndertravelBefore": Double(downwardUndertravel),
            ...
            guard minimumShift <= maximumShift else {
                sample["failureReason"] = "interval impossible"
                attempts.append(sample)
                terminalReason = "interval impossible"
                break
            }
            if minimumShift <= 0, maximumShift >= 0 {
            ...
            if maximumShift < 0 {
                ...
                sample["failureReason"] = "interval has no signed direction"
                attempts.append(sample)
                terminalReason = "interval has no signed direction"
                break
            }
            sample["requiredFinalDirection"] = Double(requiredFinalDirection)
            if let stagedFinalDirection,
               stagedFinalDirection != requiredFinalDirection {
                sample["failureReason"] = "staged direction changed"
                attempts.append(sample)
                terminalReason = "staged direction changed"
                break
            }
            ...
            let downwardCapacity = diagnosticsScrollView.frame.maxY
                - dragInset
                - startPoint.y
            sample["receiverFrame"] = auditFrameObject(diagnosticsScrollView.frame)
            sample["dragStart"] = auditFrameObject(
                CGRect(origin: startPoint, size: .zero)
            )
            sample["upwardCapacity"] = Double(upwardCapacity)
            sample["downwardCapacity"] = Double(downwardCapacity)
            guard upwardCapacity >= minimumGestureDistance,
                  downwardCapacity >= minimumGestureDistance else {
                sample["failureReason"] = "receiver capacity below 44"
                attempts.append(sample)
                terminalReason = "receiver capacity below 44"
                break
            }

            var dragDistance: CGFloat = 0
            var isStaging = false
            var recognizedMinimum: CGFloat?
            var recognizedMaximum: CGFloat?
            var failureReason: String?
            if maximumShift < 0 {
                let lower = max(minimumShift, -upwardCapacity)
                let upper = min(maximumShift, -minimumGestureDistance)
                recognizedMinimum = lower
                recognizedMaximum = upper
                if lower <= upper {
                    dragDistance = max(lower, upper - upwardUndertravel)
                } else if minimumShift > -minimumGestureDistance,
                          maximumShift < 0,
                          stagingCount < 2 {
                    let stagingDistance = min(
                        downwardCapacity,
                    ...
                    guard stagingDistance >= minimumGestureDistance else {
                        failureReason = "downward staging is not recognizable"
                        dragDistance = 0
                        isStaging = true
                        sample["stagingDistance"] = Double(stagingDistance)
                        sample["isStaging"] = true
                        sample["recognizedMinimum"] = Double(lower)
                        sample["recognizedMaximum"] = Double(upper)
                        sample["failureReason"] = failureReason!
                        attempts.append(sample)
                    ...
                    failureReason = "no bounded upward residual strategy"
                }
            } else if minimumShift > 0 {
                let lower = max(minimumShift, minimumGestureDistance)
                let upper = min(maximumShift, downwardCapacity)
                recognizedMinimum = lower
                recognizedMaximum = upper
                if lower <= upper {
                    dragDistance = min(upper, lower + downwardUndertravel)
                } else if maximumShift < minimumGestureDistance,
                          minimumShift > 0,
                          stagingCount < 2 {
                    let stagingDistance = min(
                        upwardCapacity,
                    ...
                    guard stagingDistance >= minimumGestureDistance else {
                        failureReason = "upward staging is not recognizable"
                        dragDistance = 0
                        isStaging = true
                        sample["stagingDistance"] = Double(stagingDistance)
                        sample["isStaging"] = true
                    ...
                    failureReason = "no bounded downward residual strategy"
                }
            } else {
                failureReason = "interval was neither negative nor positive"
            }
            if let recognizedMinimum {
                sample["recognizedMinimum"] = Double(recognizedMinimum)
            } else {
                sample["recognizedMinimum"] = NSNull()
            }
            if let recognizedMaximum {
                sample["recognizedMaximum"] = Double(recognizedMaximum)
            } else {
                sample["recognizedMaximum"] = NSNull()
            }
            sample["isStaging"] = isStaging
            sample["stagingCountBeforeAction"] = stagingCount
            ...
            guard (dragDistance < 0 ? upwardCapacity : downwardCapacity)
                >= abs(dragDistance) else {
                sample["failureReason"] = "requested distance exceeds capacity"
                attempts.append(sample)
                terminalReason = "requested distance exceeds capacity"
                break
            }
            ...
            let authorityBefore = authorityBeforeFrame.minY
            sample["requestedDistance"] = Double(dragDistance)
            sample["dragEnd"] = auditFrameObject(
                CGRect(origin: dragEnd.screenPoint, size: .zero)
            )
            sample["authorityFrameBefore"] = auditFrameObject(authorityBeforeFrame)
            dragStart.press(
                forDuration: 0.2,
                thenDragTo: dragEnd,
                withVelocity: .slow,
            ...
            let authorityAfter = authorityAfterFrame.minY
            let actualDistance = authorityAfter - authorityBefore
            sample["authorityFrameAfter"] = auditFrameObject(authorityAfterFrame)
            sample["actualDistance"] = Double(actualDistance)
            guard actualDistance * dragDistance > 0 else {
                sample["failureReason"] = "signed movement was not recognized"
                attempts.append(sample)
                terminalReason = "signed movement was not recognized"
                break
            }
            let observedUndertravel = max(
                0,
            ...
            sample["observedUndertravel"] = Double(observedUndertravel)
            sample["upwardUndertravelAfter"] = Double(upwardUndertravel)
            sample["downwardUndertravelAfter"] = Double(downwardUndertravel)
            sample["stagingCountAfter"] = stagingCount
            ...
            sample["postMinimumShift"] = Double(postMinimumShift)
            sample["postMaximumShift"] = Double(postMaximumShift)
            sample["postHeadingFrame"] = auditFrameObject(diagnosticsHeading.frame)
            sample["postAuthorityFrame"] = auditFrameObject(diagnosticsAuthority.frame)
            sample["postExportFrame"] = auditFrameObject(diagnosticsExport.frame)
            attempts.append(sample)
            finalMinimumShift = postMinimumShift
            finalMaximumShift = postMaximumShift
            if postMinimumShift <= 0, postMaximumShift >= 0 {
                terminalReason = "zero-containing interval reached"
                break
            }
        }
        ...
        terminalScreenshot.name = "S10.4 default-light diagnostics positioning terminal app"
        terminalScreenshot.lifetime = .keepAlways
        add(terminalScreenshot)
        let terminalTree = XCTAttachment(string: app.debugDescription)
        terminalTree.name = "S10.4 default-light diagnostics positioning terminal accessibility tree"
        terminalTree.lifetime = .keepAlways
        add(terminalTree)
        let terminalQueries = routeObject()

        printJSONLine(prefix: "S10_4_DIAGNOSTICS_POSITIONING_DIAGNOSTIC", object: [
            ...
            "upwardUndertravel": Double(upwardUndertravel),
            "downwardUndertravel": Double(downwardUndertravel),
            "stagingCount": stagingCount,
            "elapsedMilliseconds": Int((CFAbsoluteTimeGetCurrent() - startedAt) * 1000),
