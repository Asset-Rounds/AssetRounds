...
class S10BrandMigrationRouteUITestCase: XCTestCase {
    ...
    private func positionWorkValidationShortDescriptionForAXText(
        ...
        guard workScreens.count == 1,
              ...
              navigationBars.count == 1,
              tabBars.count == 1,
              keyboards.count == 1 else {
            XCTFail("AX-text work-validation positioning bindings are ambiguous.")
            return false
        }
        ...
        func diagnoseSegment2InvalidRestorationInterval(
            ...
            guard intervalRelations.count == 15,
                  ...
                  noteFrame.maxY > safeBottom,
                  noteFrame.maxY <= liveBottom else {
                XCTFail(
                    "S10.4 AX-text work-validation restoration-interval audit gate drifted"
                )
                return false
            }
            var observedIssueCount = 0
            ...
            do {
                try app.performAccessibilityAudit(for: .contrast) { issue in
                    ...
                    if let auditedElement {
                        ...
                        issueAttachment.name =
                            "S10.4 AX-text work-validation restoration interval diagnostic audited element "
                                + String(observedIssueCount)
                        issueAttachment.lifetime = .keepAlways
                        self.add(issueAttachment)
                    }
                ...
                XCTFail(
                    "S10.4 AX-text work-validation restoration-interval diagnostic audit failed: \(error)"
                )
                return false
            }
            self.printJSONLine(
        ...
        func diagnoseSegment2FinalSemantics(
            _ finalSemanticRelations: [(String, Bool)]
        ) -> Bool {
            guard automationSegment == .segment2,
                  let shard = automationShard,
                  shard.shardID == "s10.4.current.ax-text",
                  ...
                  app.state == .runningForeground else {
                XCTFail(
                    "S10.4 AX-text work-validation final-semantics diagnostic context drifted"
                )
                return false
            }
            let publicNodeObject: (XCUIElement) -> [String: Any] = { element in
            ...
            let failedRelations = finalSemanticRelations.compactMap { relation in
                relation.1 ? nil : relation.0
            }
            let context: [String: Any] = [
                "schemaVersion": 1,
                ...
                "orderedFinalSemanticRelations": orderedRelations,
                "failedFinalSemanticRelations": failedRelations,
                "queries": [
                    "workScreens": publicQueryObject(workScreens),
                    "descriptionFields": publicQueryObject(descriptionFields),
                    "focusedDescriptionFields": publicQueryObject(
                ...
                "currentFrames": [
                    ...
                    "workScreen": self.auditFrameObject(workScreen.frame),
                    "descriptionScrollView": self.auditFrameObject(
                        descriptionScrollView.frame
                    ),
                    "navigationBar": self.auditFrameObject(navigationBar.frame),
                    "tabBar": self.auditFrameObject(tabBar.frame),
                    "keyboard": self.auditFrameObject(keyboard.frame),
                    "fieldLabel": self.auditFrameObject(
                        shortDescriptionFieldLabel.frame
                    ),
                    "description": self.auditFrameObject(descriptionField.frame),
                    "focusedDescription": self.auditFrameObject(
                        focusedDescriptionField.frame
                    ),
                    "validation": self.auditFrameObject(validationLabel.frame),
                    "note": self.auditFrameObject(noteStaticText.frame),
                ],
            ]
            guard JSONSerialization.isValidJSONObject(context),
                  let contextData = try? JSONSerialization.data(
                  ...
                  !contextText.contains("\n") else {
                XCTFail(
                    "S10.4 AX-text work-validation final-semantics diagnostic JSON is invalid"
                )
                return false
            }
            self.printJSONLine(
                prefix: "S10_4_AX_TEXT_WORK_VALIDATION_FINAL_SEMANTICS_DIAGNOSTIC",
                object: context
            )
            let appAttachment = XCTAttachment(screenshot: app.screenshot())
            appAttachment.name =
                "S10.4 AX-text work-validation final semantics diagnostic app"
            appAttachment.lifetime = .keepAlways
            add(appAttachment)
            let treeAttachment = XCTAttachment(string: app.debugDescription)
            treeAttachment.name =
                "S10.4 AX-text work-validation final semantics diagnostic tree"
            treeAttachment.lifetime = .keepAlways
            add(treeAttachment)
            let contextAttachment = XCTAttachment(string: contextText)
            contextAttachment.name =
                "S10.4 AX-text work-validation final semantics diagnostic context"
            contextAttachment.lifetime = .keepAlways
            add(contextAttachment)
            XCTFail(
        ...
        for _ in 0..<4 {
            ...
            guard liveFramesAreValid,
                ...
                XCTFail("AX-text work-validation positioning geometry is invalid.")
                return false
            }
            let liveTop = max(liveScrollFrame.minY, navigationFrame.maxY)
            let liveBottom = min(
    ...
    private func diagnoseDifferentiateWithoutColorDiagnosticsPositioning(
        ...
        let applicationStateRawValue = String(describing: app.state.rawValue)
        let foreground = app.state == .runningForeground
        let applicationFrame = auditFrameObject(app.frame)
        ...
        var actionCount = 0
        var selectedBranch = "two-attempt-complete"
        ...
        for attemptOrdinal in attemptOrdinals {
            ...
            var attempt: [String: Any] = [
                ...
                "targetDistance": NSNull(),
                "direction": NSNull(),
                "correctionDirectionBefore": nullableNumber(correctionDirection),
                "previousResidualMagnitude": nullableNumber(previousResidualMagnitude),
                "residualMagnitude": NSNull(),
                "measuredUndertravelBefore": Double(measuredUndertravel),
                "requestedDistance": NSNull(),
                "dragStart": NSNull(),
                "dragEnd": NSNull(),
                "availableDistance": NSNull(),
                "authorityFrameBefore": NSNull(),
                "authorityFrameAfter": NSNull(),
                "actualDistance": NSNull(),
                "signedProgress": NSNull(),
                "measuredUndertravelAfter": Double(measuredUndertravel),
                "postNavigationFrame": NSNull(),
                "postHeadingFrame": NSNull(),
                "postAuthorityFrame": NSNull(),
                "postExportFrame": NSNull(),
                "postSignsTabFrame": NSNull(),
                "postScrollViewFrame": NSNull(),
            ...
            guard minimumShift <= maximumShift else {
                selectedBranch = "interval-impossible"
                failureReason = "interval impossible"
                attempt["branch"] = selectedBranch
                attempt["failureReason"] = nullableString(failureReason)
                attempts.append(attempt)
                break
            }
            if minimumShift <= 0, maximumShift >= 0 {
                selectedBranch = "zero-containing"
                attempt["branch"] = selectedBranch
                attempts.append(attempt)
                break
            }
            ...
            if maximumShift < 0 {
                targetDistance = maximumShift
                attempt["branch"] = "negative"
            } else if minimumShift > 0 {
                targetDistance = minimumShift
                attempt["branch"] = "positive"
            } else {
                selectedBranch = "directionless"
                failureReason = "interval has no signed correction"
                attempt["branch"] = selectedBranch
                attempt["failureReason"] = nullableString(failureReason)
                attempts.append(attempt)
                break
            }
            attempt["targetDistance"] = Double(targetDistance)
            let direction: CGFloat = targetDistance > 0 ? 1 : -1
            attempt["direction"] = Double(direction)
            if let correctionDirection {
                guard correctionDirection == direction else {
                    selectedBranch = "direction-changed"
                    failureReason = "correction direction changed"
                    attempt["branch"] = selectedBranch
                    attempt["failureReason"] = nullableString(failureReason)
                    attempts.append(attempt)
                    break
            ...
            let residualMagnitude = abs(targetDistance)
            attempt["residualMagnitude"] = Double(residualMagnitude)
            if let previousResidualMagnitude {
                guard residualMagnitude < previousResidualMagnitude else {
                    selectedBranch = "residual-not-decreasing"
                    failureReason = "residual did not decrease"
                    attempt["branch"] = selectedBranch
                    attempt["failureReason"] = nullableString(failureReason)
                    attempts.append(attempt)
                    break
            ...
            let requestedDistance = targetDistance
                + direction * measuredUndertravel
            attempt["requestedDistance"] = Double(requestedDistance)
            guard abs(requestedDistance) >= minimumGestureDistance else {
                selectedBranch = "gesture-unrecognizable"
                failureReason = "requested distance below 44"
                attempt["branch"] = selectedBranch
                attempt["failureReason"] = nullableString(failureReason)
                attempts.append(attempt)
                break
            ...
            let availableDistance = direction < 0
                ? startPoint.y - (diagnosticsScrollView.frame.minY + dragInset)
                : diagnosticsScrollView.frame.maxY - dragInset - startPoint.y
            attempt["dragStart"] = auditFrameObject(
                CGRect(origin: startPoint, size: .zero)
            )
            attempt["availableDistance"] = Double(availableDistance)
            guard availableDistance >= abs(requestedDistance) else {
                selectedBranch = "receiver-capacity"
                failureReason = "request exceeds receiver capacity"
                attempt["branch"] = selectedBranch
                attempt["failureReason"] = nullableString(failureReason)
                attempts.append(attempt)
                break
            ...
            attempt["dragEnd"] = auditFrameObject(
                CGRect(origin: dragEnd.screenPoint, size: .zero)
            )
            let authorityBeforeDrag = diagnosticsAuthority.frame
            attempt["authorityFrameBefore"] = auditFrameObject(authorityBeforeDrag)
            dragStart.press(
                forDuration: 0.2,
                thenDragTo: dragEnd,
                withVelocity: .slow,
            ...
            actionCount += 1
            let authorityAfterDrag = diagnosticsAuthority.frame
            attempt["authorityFrameAfter"] = auditFrameObject(authorityAfterDrag)
            let actualDistance = authorityAfterDrag.minY - authorityBeforeDrag.minY
            attempt["actualDistance"] = Double(actualDistance)
            let signedProgress = actualDistance * direction > 0
            attempt["signedProgress"] = signedProgress
            guard signedProgress else {
                selectedBranch = "movement-unrecognized"
            ...
            attempt["measuredUndertravelAfter"] = Double(measuredUndertravel)
            ...
            attempt["postNavigationFrame"] = auditFrameObject(navigationBar.frame)
            attempt["postHeadingFrame"] = auditFrameObject(diagnosticsHeading.frame)
            attempt["postAuthorityFrame"] = auditFrameObject(diagnosticsAuthority.frame)
            attempt["postExportFrame"] = auditFrameObject(diagnosticsExport.frame)
            attempt["postSignsTabFrame"] = auditFrameObject(signsTab.frame)
            attempt["postScrollViewFrame"] = auditFrameObject(diagnosticsScrollView.frame)
            attempt["postMinimumShift"] = Double(postMinimumShift)
            attempt["postMaximumShift"] = Double(postMaximumShift)
            attempts.append(attempt)
        }
        ...
        } else if finalMinimumShift <= 0, finalMaximumShift >= 0 {
            selectedBranch = "zero-containing"
            terminalReason = "zero-containing interval reached"
        } else {
            selectedBranch = "two-attempt-exhausted"
            terminalReason = "two attempts completed without a zero-containing interval"
        }
        let telemetry: [String: Any] = [
            "schemaVersion": 1,
            "diagnosticID": "S10.4-DIFFERENTIATE-DIAGNOSTICS-POSITIONING-V2",
            "shardID": automationShard?.shardID ?? "UNSET",
            "requirementID": automationShard?.requirementID ?? "UNSET",
            "deviceProfileID": automationShard?.deviceProfileID ?? "UNSET",
            "segmentID": automationSegment.rawValue,
            "applicationStateRawValue": applicationStateRawValue,
            ...
            "finalMinimumShift": Double(finalMinimumShift),
            "finalMaximumShift": Double(finalMaximumShift),
            "finalHeadingFrame": auditFrameObject(diagnosticsHeading.frame),
            "finalAuthorityFrame": auditFrameObject(diagnosticsAuthority.frame),
            "finalExportFrame": auditFrameObject(diagnosticsExport.frame),
            "finalNavigationFrame": auditFrameObject(navigationBar.frame),
            "finalSignsTabFrame": auditFrameObject(signsTab.frame),
            "finalScrollViewFrame": auditFrameObject(diagnosticsScrollView.frame),
            "terminalApplicationStateRawValue": String(describing: app.state.rawValue),
            "terminalForeground": app.state == .runningForeground,
            "terminalApplicationFrame": auditFrameObject(app.frame),
            "terminalQueries": routeObject(),
