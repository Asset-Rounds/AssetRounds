...
class S10BrandMigrationRouteUITestCase: XCTestCase {
   ...
   private func acceptImportedPhoto(
    ...
    private func diagnoseCurrentWorkSavingNativeContrast(
        ...
        let expectedShardIDs: Set<String> = [
            "s10.4.current.default-light",
            "s10.4.current.default-dark",
            "s10.4.current.differentiate-without-color",
        ]
        guard stateID == "state.work.saving",
              expectedShardIDs.contains(shard.shardID),
              automationSegment == .none,
              migratedStateIDs.last == stateID,
              automationAXTreeDigests[stateID] == nil,
              automationContrastExceptions[stateID] == nil,
              app.state == .runningForeground else {
            throw AutomationConfigurationError.invalid(
                "S10.4 current Record-work saving native contrast diagnostic gate is invalid"
            )
        }
        ...
        let diagnosticQueryBindings: [(
            ...
            (
                ...
                app.staticTexts.matching(
                    NSPredicate(format: "label == %@", "Short description")
                )
            ),
            (
                "savingStatuses",
            ...
            (
                "workPhotos",
                app.images.matching(identifier: "s5.1.work.photo")
            ),
        ]
        let diagnosticElementObject: (XCUIElement) -> [String: Any] = {
            element in
        ...
        throw AutomationConfigurationError.invalid(
            "S10.4 current Record-work saving native contrast diagnostic completed nonaccepting"
        )
    }
