        ...
        for _ in 0..<4 {
            ...
            if workEditingAXTextEnabled,
               provenSavingGestureCount == 1 {
                ...
                let diagnosticFrameObject: (CGRect) -> Any = { [self] frame in
                    workEditingFrameIsValid(frame)
                        ? auditFrameObject(frame) as Any
                        : NSNull()
                }
                let diagnosticScalarObject: (CGFloat) -> Any = { value in
                ...
                let diagnosticWorkScreen = workScreens.firstMatch
                let diagnosticSavingIndicator = savingIndicators.firstMatch
                let diagnosticNote = workNoteHeadings.firstMatch
                let diagnosticHelper = workHelperTexts.firstMatch
                let diagnosticPreview = workPreviewImages.firstMatch
                let diagnosticScroll = workScrollViews.firstMatch
                let diagnosticNavigation = workNavigationBars.firstMatch
                let diagnosticTab = workTabBars.firstMatch
                let beforeSample = diagnosticQuerySample()
                let beforeQueries = beforeSample.objects
                let beforeCounts = beforeSample.counts
                let startApp = XCTAttachment(
                    screenshot: XCUIScreen.main.screenshot()
                )
                startApp.name =
                    "S10.4 AX-text work-saving diagnostic start app"
                startApp.lifetime = .keepAlways
                add(startApp)
                let startTree = XCTAttachment(string: app.debugDescription)
                startTree.name =
                    "S10.4 AX-text work-saving diagnostic start tree"
                startTree.lifetime = .keepAlways
                add(startTree)
                let beforeApplicationStateRawValue = app.state.rawValue
                let beforeApplicationFrame = app.frame
                let beforeWorkScreenCountIsOne =
                    beforeCounts["workScreens"] == 1
                let beforeSavingCountIsOne =
                    beforeCounts["savingIndicators"] == 1
                let beforeNoteCountIsOne = beforeCounts["noteHeadings"] == 1
                let beforeHelperCountIsOne = beforeCounts["helperTexts"] == 1
                let beforePreviewCountIsOne =
                    beforeCounts["previewImages"] == 1
                let beforeScrollCountIsOne = beforeCounts["scrollViews"] == 1
                let beforeNavigationCountIsOne =
                    beforeCounts["navigationBars"] == 1
                let beforeTabCountIsOne = beforeCounts["tabBars"] == 1
                let beforeWorkScreenFrame = beforeWorkScreenCountIsOne
                    ? diagnosticWorkScreen.frame
                    : CGRect.null
                let beforeSavingFrame = beforeSavingCountIsOne
                    ? diagnosticSavingIndicator.frame
                    : CGRect.null
                let beforeNoteFrame = beforeNoteCountIsOne
                    ? diagnosticNote.frame
                    : CGRect.null
                let beforeHelperFrame = beforeHelperCountIsOne
                    ? diagnosticHelper.frame
                    : CGRect.null
                let beforePreviewFrame = beforePreviewCountIsOne
                    ? diagnosticPreview.frame
                    : CGRect.null
                let beforeScrollFrame = beforeScrollCountIsOne
                    ? diagnosticScroll.frame
                    : CGRect.null
                let beforeNavigationFrame = beforeNavigationCountIsOne
                    ? diagnosticNavigation.frame
                    : CGRect.null
                let beforeTabFrame = beforeTabCountIsOne
                    ? diagnosticTab.frame
                    : CGRect.null
                let beforeCardinalityIsExact =
                    beforeWorkScreenCountIsOne
                        ...
                        && beforeTabCountIsOne
                let beforeWorkScreenExists =
                    beforeWorkScreenCountIsOne && diagnosticWorkScreen.exists
                let beforeSavingExists =
                    beforeSavingCountIsOne && diagnosticSavingIndicator.exists
                let beforeNoteExists =
                    beforeNoteCountIsOne && diagnosticNote.exists
                let beforeHelperExists =
                    beforeHelperCountIsOne && diagnosticHelper.exists
                let beforePreviewExists =
                    beforePreviewCountIsOne && diagnosticPreview.exists
                let beforeScrollExists =
                    beforeScrollCountIsOne && diagnosticScroll.exists
                let beforeNavigationExists =
                    beforeNavigationCountIsOne && diagnosticNavigation.exists
                let beforeTabExists =
                    beforeTabCountIsOne && diagnosticTab.exists
                let beforeExistenceIsExact =
                    beforeWorkScreenExists
                        ...
                        && beforeTabExists
                let beforeWorkScreenHittable =
                    beforeWorkScreenExists && diagnosticWorkScreen.isHittable
                let beforeSavingHittable =
                    beforeSavingExists && diagnosticSavingIndicator.isHittable
                let beforeNoteHittable =
                    beforeNoteExists && diagnosticNote.isHittable
                let beforeHelperHittable =
                    beforeHelperExists && diagnosticHelper.isHittable
                let beforePreviewHittable =
                    beforePreviewExists && diagnosticPreview.isHittable
                let beforeScrollHittable =
                    beforeScrollExists && diagnosticScroll.isHittable
                let beforeNavigationHittable =
                    beforeNavigationExists && diagnosticNavigation.isHittable
                let beforeTabHittable =
                    beforeTabExists && diagnosticTab.isHittable
                let beforeAppForeground = app.state == .runningForeground
                let beforeRouteIsExact =
                    beforeAppForeground
                        && beforeCardinalityIsExact
                        && beforeExistenceIsExact
                let beforeApplicationFrameValid =
                    workEditingFrameIsValid(beforeApplicationFrame)
                let beforeWorkScreenFrameValid =
                    workEditingFrameIsValid(beforeWorkScreenFrame)
                let beforeSavingFrameValid =
                    workEditingFrameIsValid(beforeSavingFrame)
                let beforeNoteFrameValid =
                    workEditingFrameIsValid(beforeNoteFrame)
                let beforeHelperFrameValid =
                    workEditingFrameIsValid(beforeHelperFrame)
                let beforePreviewFrameValid =
                    workEditingFrameIsValid(beforePreviewFrame)
                let beforeScrollFrameValid =
                    workEditingFrameIsValid(beforeScrollFrame)
                let beforeNavigationFrameValid =
                    workEditingFrameIsValid(beforeNavigationFrame)
                let beforeTabFrameValid =
                    workEditingFrameIsValid(beforeTabFrame)
                let beforeAllFramesValid =
                    beforeApplicationFrameValid
                        && beforeWorkScreenFrameValid
                        && beforeSavingFrameValid
                        && beforeNoteFrameValid
                        && beforeHelperFrameValid
                        && beforePreviewFrameValid
                        && beforeScrollFrameValid
                        && beforeNavigationFrameValid
                        && beforeTabFrameValid
                let safeBandIsValid = safeBottom > safeTop
                let targetFitsSafeBand =
                    targetBottom - targetTop <= safeBottom - safeTop
                let shiftIntervalIsFeasible = minimumShift <= maximumShift
                let receiverCapacityIsRecognizable =
                    receiverCapacity >= minimumGestureDistance
                let dragDistanceIsRecognizable =
                    (dragDistance <= -minimumGestureDistance
                        || dragDistance >= minimumGestureDistance)
                        && dragDistance >= -receiverCapacity
                        && dragDistance <= receiverCapacity
                let upwardShiftRequested = maximumShift < 0
                let downwardShiftRequested = minimumShift > 0
                let directionlessShift =
                    !upwardShiftRequested && !downwardShiftRequested
                let upwardRecognizedMinimum = max(
                    minimumShift,
                    -receiverCapacity
                )
                let upwardRecognizedMaximum = min(
                    maximumShift,
                    -minimumGestureDistance
                )
                let downwardRecognizedMinimum = max(
                    minimumShift,
                    minimumGestureDistance
                )
                let downwardRecognizedMaximum = min(
                    maximumShift,
                    receiverCapacity
                )
                let upwardDirectRecognizable =
                    upwardShiftRequested
                        && upwardRecognizedMinimum
                            <= upwardRecognizedMaximum
                let downwardDirectRecognizable =
                    downwardShiftRequested
                        && downwardRecognizedMinimum
                            <= downwardRecognizedMaximum
                let downwardStagedRecognizable =
                    downwardShiftRequested
                        && !downwardDirectRecognizable
                        && minimumShift > receiverCapacity
                let selectionBranch = upwardShiftRequested
                    ? "direct-upward"
                    : (downwardDirectRecognizable
                        ? "direct-downward"
                ...
                let dragEndPoint = CGPoint(
                    x: scrollFrame.midX,
                    y: dragStartY + dragDistance
                )
                let receiverFrame = CGRect(
                    x: liveScrollFrame.minX,
                    y: receiverTop,
                    width: liveScrollFrame.width,
                    height: receiverCapacity
                )
                let receiverContainsStart = receiverFrame.contains(
                    dragStartPoint
                )
                let receiverContainsEnd = receiverFrame.contains(dragEndPoint)
                let startIntersectsNote = noteFrame.contains(dragStartPoint)
                let startIntersectsHelper = helperFrame.contains(dragStartPoint)
                let startIntersectsPreview = beforePreviewFrame.contains(
                    dragStartPoint
                )
                let endIntersectsNote = noteFrame.contains(dragEndPoint)
                let endIntersectsHelper = helperFrame.contains(dragEndPoint)
                let endIntersectsPreview = beforePreviewFrame.contains(
                    dragEndPoint
                )
                dragStart.press(
                    forDuration: 0.2,
                ...
                let gestureWasPerformed = true
                let immediateAfterNoteFrame = workNoteHeading.frame
                let immediateAfterHelperFrame = workHelper.frame
                let immediateAfterPreviewFrame = workPreviewImage.frame
                let observedNoteShift =
                    immediateAfterNoteFrame.minY - noteMinYBeforeDrag
                let observedHelperShift =
                    immediateAfterHelperFrame.minY - helperMinYBeforeDrag
                let observedPreviewShift = previewMinYBeforeDrag.map {
                    immediateAfterPreviewFrame.minY - $0
                }
                let immediatePreviewFrameValid =
                    workEditingFrameIsValid(immediateAfterPreviewFrame)
                let noteSignedProgress = observedNoteShift * dragDistance > 0
                let helperSignedProgress =
                    observedHelperShift * dragDistance > 0
                let previewSignedProgress = observedPreviewShift.map {
                    $0 * dragDistance > 0
                } ?? false
                let previewShiftEqualsHelperShift = observedPreviewShift.map {
                    $0 == observedHelperShift
                } ?? false
                let savingAXTextCoMovementWouldPass =
                    previewMinYBeforeDrag != nil
                        && immediatePreviewFrameValid
                        && previewSignedProgress
                        && previewShiftEqualsHelperShift
                let guardNoteCountIsOne = workNoteHeadings.count == 1
                let guardTabCountIsOne = workTabBars.count == 1
                let guardHelperCountIsOne = workHelperTexts.count == 1
                let guardScrollCountIsOne = workScrollViews.count == 1
                let guardNavigationCountIsOne = workNavigationBars.count == 1
                let guardNoteExists = workNoteHeading.exists
                let guardTabExists = workTabBar.exists
                let guardHelperExists = workHelper.exists
                let guardSavingExists = progress.exists
                let originalAggregateWouldPass =
                    guardNoteCountIsOne
                        && guardTabCountIsOne
                ...
                let afterSample = diagnosticQuerySample()
                let afterQueries = afterSample.objects
                let afterCounts = afterSample.counts
                let afterApplicationFrame = app.frame
                let afterWorkScreenCountIsOne =
                    afterCounts["workScreens"] == 1
                let afterSavingCountIsOne =
                    afterCounts["savingIndicators"] == 1
                let afterNoteCountIsOne = afterCounts["noteHeadings"] == 1
                let afterHelperCountIsOne = afterCounts["helperTexts"] == 1
                let afterPreviewCountIsOne =
                    afterCounts["previewImages"] == 1
                let afterScrollCountIsOne = afterCounts["scrollViews"] == 1
                let afterNavigationCountIsOne =
                    afterCounts["navigationBars"] == 1
                let afterTabCountIsOne = afterCounts["tabBars"] == 1
                let afterWorkScreenFrame = afterWorkScreenCountIsOne
                    ? diagnosticWorkScreen.frame
                    : CGRect.null
                let afterSavingFrame = afterSavingCountIsOne
                    ? diagnosticSavingIndicator.frame
                    : CGRect.null
                let afterNoteFrame = afterNoteCountIsOne
                    ? diagnosticNote.frame
                    : CGRect.null
                let afterHelperFrame = afterHelperCountIsOne
                    ? diagnosticHelper.frame
                    : CGRect.null
                let afterPreviewFrame = afterPreviewCountIsOne
                    ? diagnosticPreview.frame
                    : CGRect.null
                let afterScrollFrame = afterScrollCountIsOne
                    ? diagnosticScroll.frame
                    : CGRect.null
                let afterNavigationFrame = afterNavigationCountIsOne
                    ? diagnosticNavigation.frame
                    : CGRect.null
                let afterTabFrame = afterTabCountIsOne
                    ? diagnosticTab.frame
                    : CGRect.null
                let afterCardinalityIsExact =
                    afterWorkScreenCountIsOne
                        ...
                        && afterTabCountIsOne
                let afterWorkScreenExists =
                    afterWorkScreenCountIsOne && diagnosticWorkScreen.exists
                let afterSavingExists =
                    afterSavingCountIsOne && diagnosticSavingIndicator.exists
                let afterNoteExists =
                    afterNoteCountIsOne && diagnosticNote.exists
                let afterHelperExists =
                    afterHelperCountIsOne && diagnosticHelper.exists
                let afterPreviewExists =
                    afterPreviewCountIsOne && diagnosticPreview.exists
                let afterScrollExists =
                    afterScrollCountIsOne && diagnosticScroll.exists
                let afterNavigationExists =
                    afterNavigationCountIsOne && diagnosticNavigation.exists
                let afterTabExists =
                    afterTabCountIsOne && diagnosticTab.exists
                let afterExistenceIsExact =
                    afterWorkScreenExists
                        ...
                        && afterTabExists
                let afterWorkScreenHittable =
                    afterWorkScreenExists && diagnosticWorkScreen.isHittable
                let afterSavingHittable =
                    afterSavingExists && diagnosticSavingIndicator.isHittable
                let afterNoteHittable =
                    afterNoteExists && diagnosticNote.isHittable
                let afterHelperHittable =
                    afterHelperExists && diagnosticHelper.isHittable
                let afterPreviewHittable =
                    afterPreviewExists && diagnosticPreview.isHittable
                let afterScrollHittable =
                    afterScrollExists && diagnosticScroll.isHittable
                let afterNavigationHittable =
                    afterNavigationExists && diagnosticNavigation.isHittable
                let afterTabHittable =
                    afterTabExists && diagnosticTab.isHittable
                let afterAppForeground = app.state == .runningForeground
                let afterRouteIsExact =
                    afterAppForeground
                        && afterCardinalityIsExact
                        && afterExistenceIsExact
                let afterApplicationFrameValid =
                    workEditingFrameIsValid(afterApplicationFrame)
                let afterWorkScreenFrameValid =
                    workEditingFrameIsValid(afterWorkScreenFrame)
                let afterSavingFrameValid =
                    workEditingFrameIsValid(afterSavingFrame)
                let afterNoteFrameValid =
                    workEditingFrameIsValid(afterNoteFrame)
                let afterHelperFrameValid =
                    workEditingFrameIsValid(afterHelperFrame)
                let afterPreviewFrameValid =
                    workEditingFrameIsValid(afterPreviewFrame)
                let afterScrollFrameValid =
                    workEditingFrameIsValid(afterScrollFrame)
                let afterNavigationFrameValid =
                    workEditingFrameIsValid(afterNavigationFrame)
                let afterTabFrameValid =
                    workEditingFrameIsValid(afterTabFrame)
                let afterAllFramesValid =
                    afterApplicationFrameValid
                        && afterWorkScreenFrameValid
                        && afterSavingFrameValid
                        && afterNoteFrameValid
                        && afterHelperFrameValid
                        && afterPreviewFrameValid
                        && afterScrollFrameValid
                        && afterNavigationFrameValid
                        && afterTabFrameValid
                let terminalApp = XCTAttachment(
                    screenshot: XCUIScreen.main.screenshot()
                )
                terminalApp.name =
                    "S10.4 AX-text work-saving diagnostic terminal app"
                terminalApp.lifetime = .keepAlways
                add(terminalApp)
                let terminalTree = XCTAttachment(string: app.debugDescription)
                terminalTree.name =
                    "S10.4 AX-text work-saving diagnostic terminal tree"
                terminalTree.lifetime = .keepAlways
                add(terminalTree)
                printJSONLine(
                    prefix: "S10_4_AX_TEXT_WORK_SAVING_DIAGNOSTIC",
                    object: [
                        "shardID": automationShard?.shardID ?? "",
                        "deviceProfileID": automationShard?.deviceProfileID ?? "",
                        "stateID": "state.work.saving",
                        "ordinal": provenSavingGestureCount + 1,
                        "elapsedMilliseconds": Int(
                            (ProcessInfo.processInfo.systemUptime
                                - diagnosticStartedAt) * 1_000
                        ),
                        "applicationStateRawValueBefore":
                            beforeApplicationStateRawValue,
                        "applicationStateRawValueAfter": app.state.rawValue,
                        "beforeQueries": beforeQueries,
                        "afterQueries": afterQueries,
                        "beforeFrames": [
                            "application": diagnosticFrameObject(
                                beforeApplicationFrame
                            ),
                            "workScreen": diagnosticFrameObject(
                                beforeWorkScreenFrame
                            ),
                            "saving": diagnosticFrameObject(beforeSavingFrame),
                            "note": diagnosticFrameObject(beforeNoteFrame),
                            "helper": diagnosticFrameObject(beforeHelperFrame),
                            "preview": diagnosticFrameObject(beforePreviewFrame),
                            "scroll": diagnosticFrameObject(beforeScrollFrame),
                            "navigation": diagnosticFrameObject(
                                beforeNavigationFrame
                            ),
                            "tabBar": diagnosticFrameObject(beforeTabFrame),
                        ],
                        "afterFrames": [
                            "application": diagnosticFrameObject(
                                afterApplicationFrame
                            ),
                            "workScreen": diagnosticFrameObject(
                                afterWorkScreenFrame
                            ),
                            "saving": diagnosticFrameObject(afterSavingFrame),
                            "note": diagnosticFrameObject(afterNoteFrame),
                            "helper": diagnosticFrameObject(afterHelperFrame),
                            "preview": diagnosticFrameObject(afterPreviewFrame),
                            "scroll": diagnosticFrameObject(afterScrollFrame),
                            "navigation": diagnosticFrameObject(
                                afterNavigationFrame
                            ),
                            "tabBar": diagnosticFrameObject(afterTabFrame),
                        ],
                        "geometry": [
                            "verticalInset": diagnosticScalarObject(
                                verticalInset
                            ),
                            "receiverInset": diagnosticScalarObject(
                                receiverInset
                            ),
                            "minimumGestureDistance": diagnosticScalarObject(
                                minimumGestureDistance
                            ),
                            "application": diagnosticFrameObject(
                                applicationFrame
                            ),
                            "liveScroll": diagnosticFrameObject(
                                liveScrollFrame
                            ),
                            "navigation": diagnosticFrameObject(
                                navigationFrame
                            ),
                            "tabBar": diagnosticFrameObject(tabBarFrame),
                            "receiver": diagnosticFrameObject(receiverFrame),
                            "safeTop": diagnosticScalarObject(safeTop),
                            "safeBottom": diagnosticScalarObject(safeBottom),
                            "liveBottom": diagnosticScalarObject(liveBottom),
                            "receiverTop": diagnosticScalarObject(receiverTop),
                            "receiverBottom": diagnosticScalarObject(
                                receiverBottom
                            ),
                            "receiverCapacity": diagnosticScalarObject(
                                receiverCapacity
                            ),
                            "targetTop": diagnosticScalarObject(targetTop),
                            "targetBottom": diagnosticScalarObject(targetBottom),
                            "minimumShift": diagnosticScalarObject(minimumShift),
                            "maximumShift": diagnosticScalarObject(maximumShift),
                            "upwardRecognizedMinimum": diagnosticScalarObject(
                                upwardRecognizedMinimum
                            ),
                            "upwardRecognizedMaximum": diagnosticScalarObject(
                                upwardRecognizedMaximum
                            ),
                            "downwardRecognizedMinimum": diagnosticScalarObject(
                                downwardRecognizedMinimum
                            ),
                            "downwardRecognizedMaximum": diagnosticScalarObject(
                                downwardRecognizedMaximum
                            ),
                            "selectionBranch": selectionBranch,
                            "dragDistance": diagnosticScalarObject(dragDistance),
                            "startX": diagnosticScalarObject(scrollFrame.midX),
                            "startY": diagnosticScalarObject(dragStartY),
                            "endX": diagnosticScalarObject(scrollFrame.midX),
                            "endY": diagnosticScalarObject(
                                dragStartY + dragDistance
                            ),
                        ],
                        "beforePredicates": [
                            "appForeground": beforeAppForeground,
                            "workScreenCountIsOne":
                                beforeWorkScreenCountIsOne,
                            "savingCountIsOne": beforeSavingCountIsOne,
                            "noteCountIsOne": beforeNoteCountIsOne,
                            "helperCountIsOne": beforeHelperCountIsOne,
                            "previewCountIsOne": beforePreviewCountIsOne,
                            "scrollCountIsOne": beforeScrollCountIsOne,
                            "navigationCountIsOne":
                                beforeNavigationCountIsOne,
                            "tabCountIsOne": beforeTabCountIsOne,
                            "cardinalityIsExact": beforeCardinalityIsExact,
                            "workScreenExists": beforeWorkScreenExists,
                            "savingExists": beforeSavingExists,
                            "noteExists": beforeNoteExists,
                            "helperExists": beforeHelperExists,
                            "previewExists": beforePreviewExists,
                            "scrollExists": beforeScrollExists,
                            "navigationExists": beforeNavigationExists,
                            "tabExists": beforeTabExists,
                            "existenceIsExact": beforeExistenceIsExact,
                            "workScreenHittable": beforeWorkScreenHittable,
                            "savingHittable": beforeSavingHittable,
                            "noteHittable": beforeNoteHittable,
                            "helperHittable": beforeHelperHittable,
                            "previewHittable": beforePreviewHittable,
                            "scrollHittable": beforeScrollHittable,
                            "navigationHittable": beforeNavigationHittable,
                            "tabHittable": beforeTabHittable,
                            "routeIsExact": beforeRouteIsExact,
                        ],
                        "afterPredicates": [
                            "appForeground": afterAppForeground,
                            "workScreenCountIsOne": afterWorkScreenCountIsOne,
                            "savingCountIsOne": afterSavingCountIsOne,
                            "noteCountIsOne": afterNoteCountIsOne,
                            "helperCountIsOne": afterHelperCountIsOne,
                            "previewCountIsOne": afterPreviewCountIsOne,
                            "scrollCountIsOne": afterScrollCountIsOne,
                            "navigationCountIsOne":
                                afterNavigationCountIsOne,
                            "tabCountIsOne": afterTabCountIsOne,
                            "cardinalityIsExact": afterCardinalityIsExact,
                            "workScreenExists": afterWorkScreenExists,
                            "savingExists": afterSavingExists,
                            "noteExists": afterNoteExists,
                            "helperExists": afterHelperExists,
                            "previewExists": afterPreviewExists,
                            "scrollExists": afterScrollExists,
                            "navigationExists": afterNavigationExists,
                            "tabExists": afterTabExists,
                            "existenceIsExact": afterExistenceIsExact,
                            "workScreenHittable": afterWorkScreenHittable,
                            "savingHittable": afterSavingHittable,
                            "noteHittable": afterNoteHittable,
                            "helperHittable": afterHelperHittable,
                            "previewHittable": afterPreviewHittable,
                            "scrollHittable": afterScrollHittable,
                            "navigationHittable": afterNavigationHittable,
                            "tabHittable": afterTabHittable,
                            "routeIsExact": afterRouteIsExact,
                            "guardNoteCountIsOne": guardNoteCountIsOne,
                            "guardTabCountIsOne": guardTabCountIsOne,
                            "guardHelperCountIsOne": guardHelperCountIsOne,
                            "guardScrollCountIsOne": guardScrollCountIsOne,
                            "guardNavigationCountIsOne":
                                guardNavigationCountIsOne,
                            "guardNoteExists": guardNoteExists,
                            "guardTabExists": guardTabExists,
                            "guardHelperExists": guardHelperExists,
                            "guardSavingExists": guardSavingExists,
                            "immediatePreviewFrameValid":
                                immediatePreviewFrameValid,
                            "noteSignedProgress": noteSignedProgress,
                            "helperSignedProgress": helperSignedProgress,
                            "previewSignedProgress": previewSignedProgress,
                            "previewShiftEqualsHelperShift":
                                previewShiftEqualsHelperShift,
                            "savingAXTextCoMovementWouldPass":
                                savingAXTextCoMovementWouldPass,
                        ...
                        "frameValidity": [
                            "beforeApplication": beforeApplicationFrameValid,
                            "beforeWorkScreen": beforeWorkScreenFrameValid,
                            "beforeSaving": beforeSavingFrameValid,
                            "beforeNote": beforeNoteFrameValid,
                            "beforeHelper": beforeHelperFrameValid,
                            "beforePreview": beforePreviewFrameValid,
                            "beforeScroll": beforeScrollFrameValid,
                            "beforeNavigation": beforeNavigationFrameValid,
                            "beforeTabBar": beforeTabFrameValid,
                            "beforeAllFramesValid": beforeAllFramesValid,
                            "afterApplication": afterApplicationFrameValid,
                            "afterWorkScreen": afterWorkScreenFrameValid,
                            "afterSaving": afterSavingFrameValid,
                            "afterNote": afterNoteFrameValid,
                            "afterHelper": afterHelperFrameValid,
                            "afterPreview": afterPreviewFrameValid,
                            "afterScroll": afterScrollFrameValid,
                            "afterNavigation": afterNavigationFrameValid,
                            "afterTabBar": afterTabFrameValid,
                            "afterAllFramesValid": afterAllFramesValid,
                        ],
                        "plannerPredicates": [
                            "safeBandIsValid": safeBandIsValid,
                            "targetFitsSafeBand": targetFitsSafeBand,
                            "shiftIntervalIsFeasible": shiftIntervalIsFeasible,
                            "receiverCapacityIsRecognizable":
                                receiverCapacityIsRecognizable,
                            "dragDistanceIsRecognizable":
                                dragDistanceIsRecognizable,
                            "upwardShiftRequested": upwardShiftRequested,
                            "downwardShiftRequested": downwardShiftRequested,
                            "directionlessShift": directionlessShift,
                            ...
                            "downwardStagedRecognizable":
                                downwardStagedRecognizable,
                            "receiverContainsStart": receiverContainsStart,
                            "receiverContainsEnd": receiverContainsEnd,
                            "startIntersectsNote": startIntersectsNote,
                            "startIntersectsHelper": startIntersectsHelper,
                            "startIntersectsPreview": startIntersectsPreview,
                            "endIntersectsNote": endIntersectsNote,
                            "endIntersectsHelper": endIntersectsHelper,
                            "endIntersectsPreview": endIntersectsPreview,
                            "gestureWasPerformed": gestureWasPerformed,
                        ],
                        "observedShifts": [
                            "note": diagnosticScalarObject(observedNoteShift),
                            "helper": diagnosticScalarObject(
                                observedHelperShift
                            ),
                            "preview": observedPreviewShift.map(
                                diagnosticScalarObject
                            ) ?? NSNull(),
                        ],
                    ]
                )
