   ...
   private func acceptImportedPhoto(
    ...
    private func diagnoseMinimumTallPreflightAuditTimeout(
        ...
        guard let shard = automationShard,
              ...
              app.state == .runningForeground else {
            throw AutomationConfigurationError.invalid(
                "S10.4 minimum-tall preflight audit-timeout diagnostic gate is invalid"
            )
        }
        ...
        preAuditAppAttachment.name =
            "S10.4 minimum-tall preflight audit-timeout diagnostic pre-audit app"
        preAuditAppAttachment.lifetime = .keepAlways
        add(preAuditAppAttachment)
        let preAuditTreeAttachment = XCTAttachment(string: app.debugDescription)
        preAuditTreeAttachment.name =
            "S10.4 minimum-tall preflight audit-timeout diagnostic pre-audit tree"
        preAuditTreeAttachment.lifetime = .keepAlways
        add(preAuditTreeAttachment)
        ...
        do {
            try app.performAccessibilityAudit(for: .contrast) { issue in
                ...
                var diagnosticIssue: [String: Any] = [
                    ...
                    "elementTypeDescription": NSNull(),
                    "elementFrame": NSNull(),
                    "applicationFrame": preAuditApplicationFrame,
                ]
                if let auditedElement {
                    auditedElementCount += 1
                    let auditedElementObject = diagnosticElementObject(auditedElement)
        ...
        let diagnosticContext: [String: Any] = [
            ...
            "axTreeDigestStateIDs": automationAXTreeDigests.keys.sorted(),
            "contrastExceptionStateIDs": automationContrastExceptions.keys.sorted(),
            "applicationState": preAuditApplicationState,
            "applicationStateRawValue": preAuditApplicationStateRawValue,
            "applicationForeground": preAuditApplicationForeground,
            "applicationFrame": preAuditApplicationFrame,
            "application": preAuditApplicationObject,
            "queries": diagnosticQueryObjects,
        ...
        printJSONLine(
            prefix: "S10_4_MINIMUM_TALL_PREFLIGHT_AUDIT_TIMEOUT_DIAGNOSTIC",
            object: diagnosticContext
        )
        let contextData = try JSONSerialization.data(
            withJSONObject: diagnosticContext,
        ...
        contextAttachment.name =
            "S10.4 minimum-tall preflight audit-timeout diagnostic context"
        contextAttachment.lifetime = .keepAlways
        add(contextAttachment)
