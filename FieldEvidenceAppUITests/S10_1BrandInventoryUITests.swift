...
final class S10_1BrandInventoryUITests: XCTestCase {
    ...
    @MainActor
    func testCapturesFrozenS9BrandBaselinesAcrossReleasedGoldenFlow() throws {
    ...
    private func captureBaseline(
        ...
        XCTAssertEqual(
            stateID,
            stateID.lowercased(),
            "Baseline state IDs must remain canonical lowercase values",
            file: file,
            line: line
        )
        ...
        let hash = SHA256.hash(data: data)
            ...
            .joined()
        print("S10_BASELINE state=\(stateID) sha256=\(hash) bytes=\(data.count)")
        let attachment = XCTAttachment(
            data: data,
            uniformTypeIdentifier: "public.data"
        )
        attachment.name = "S10.1 baseline bytes \(stateID)"
