...
enum GuidedSurveyStreamingArchiveDispositionV1 {
    static func validate(records: V4BackupRecordsV1) throws {
        guard records.recordsSchemaVersion < 24 ||
                ((24...LightingNightWorkflowBackupEnrollmentV1.recordsSchemaVersion).contains(records.recordsSchemaVersion) &&
        ...
        if records.recordsSchemaVersion >= C05RoundSessionStreamingArchiveBoundaryV1.recordsSchemaVersion {
            do { try C05RoundSessionStreamingArchiveBoundaryV1.validate(records: records) }
            catch { throw StreamingArchiveFailureV1.invalidArchive }
        }
        if records.recordsSchemaVersion >= C08ImportBulkBackupEnrollmentV1.recordsSchemaVersion {
            do { try C08ImportBulkBackupEnrollmentV1.validate(records) }
            catch { throw StreamingArchiveFailureV1.invalidArchive }
        }
    }
}
