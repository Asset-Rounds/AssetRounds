...
private extension BackupExportService {
    ...
    private func surveyDefinitionRecords(_ rows: Rows, history: MutationHistorySnapshotV1) throws -> [V24BackupSurveyDefinitionRecordV1] {
        let identities = try rows.surveyDefinitionIdentities.map { try $0.value() }
        let releases = try rows.surveyDefinitionReleases.map { try $0.value() }
        try SurveyDefinitionBackupGraphClosureV1.validate(identities: identities,
            releases: releases, history: history, expectedWorkspaceID: nil)
        let result = try identities.map {
