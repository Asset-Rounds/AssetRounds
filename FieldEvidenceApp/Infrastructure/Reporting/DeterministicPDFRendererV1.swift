...
extension DeterministicPDFRendererV1{
 static func renderC18LightingNight(_ p:LightingReportProjectionV1)throws->ReportProjectionOutputV1{try C18LightingReportProjectionSupportV1.validate(p);let d=try C18LightingReportProjectionSupportV1.digest(p);var lines=["Night lighting follow-up","Workflow revision: \(p.workflowRevision)","Workflow digest: \(p.workflowSHA256)","Recorded deltas: \(p.deltaCount)","Open issues: \(p.openIssueIDs.count)","Resolved for recorded scope: \(p.resolvedForRecordedScopeIssueIDs.count)","Reopened issues: \(p.reopenedIssueIDs.count)","Measurement dispositions: \(p.measurementDispositions.map(\.rawValue).joined(separator:", "))","Grouped issues retain child history.",C18LightingNightLocalizationKeyV1.claimBoundary.englishDefaultValue];lines.append(contentsOf:p.claims.map{"Claim \($0.claimID.uuidString.lowercased()) tier \($0.tier.rawValue); key \($0.textKey); source \($0.sourceSHA256); limitation \($0.limitationKey)"});if let patrol=p.patrol{lines.append("Patrol round: \(patrol.round.sessionID.uuidString.lowercased()) revision \(patrol.round.revision); item \(patrol.itemID.uuidString.lowercased()); completion \(patrol.completion.completionID.uuidString.lowercased()) revision \(patrol.completion.revision)")};let data=try C18LightingNightPDFPayloadV1.data(projectionSHA256:d,lines:lines);guard data.count<=SnapshotProjectionLimitsV1.maximumProjectionBytes else{throw SnapshotProjectionFailureV1.limitExceeded};return .init(format:.pdf,data:data,sha256:KernelCanonicalHashV1.sha256(data),semanticSHA256:d,orderedSemanticIDs:[p.workflowID.uuidString.lowercased()],taggedPDFAccessibilityEvidence:false)}
}
...
// MARK: - C34 route snapshot PDF exclusion

enum C34RouteSnapshotPDFBoundaryV1 {
...
enum C52ServiceRequestBoundary_FieldEvidenceApp_Infrastructure_Reporting_DeterministicPDFRendererV1_swift {
