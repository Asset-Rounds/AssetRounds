...
private extension StoreGenerationFactory {
    ...
    private func performAdjacentCloneMigration(
        ...
        case (.v2, .v3):
            return try autoreleasepool { () throws -> String in
                ...
                return (try adjacentSemanticDigest(in: context, release: .v3, aggregate: aggregate))
            }
        case (.v3, .v4):
            return try autoreleasepool { () throws -> String in
        ...
        case (.v4, .v5):
            return try autoreleasepool { () throws -> String in
                ...
                guard (try adjacentSemanticDigest(in: context, release: .v4, aggregate: aggregate)) == expectedSemanticDigest else {
                    throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)
                }
                ...
                return (try adjacentSemanticDigest(in: context, release: .v5, aggregate: aggregate))
            }
        case (.v5, .v6):
            return try autoreleasepool { () throws -> String in
                ...
                guard try isAuthorizedTargetRetry || (try adjacentSemanticDigest(in: context, release: .v5, aggregate: aggregate)) == expectedSemanticDigest else {
                    throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)
                }
                ...
                return (try adjacentSemanticDigest(in: context, release: .v6, aggregate: aggregate))
            }
        case (.v6, .v7):
            return try autoreleasepool { () throws -> String in
                let container = try makeV7Container(at: modelStoreURL, migrate: true)
                let context = container.mainContext
                guard (try adjacentSemanticDigest(in: context, release: .v6, aggregate: aggregate)) == expectedSemanticDigest else {
                    throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)
                }
                try backfillV7Marker(in: context, migrationID: migrationID)
                return (try adjacentSemanticDigest(in: context, release: .v7, aggregate: aggregate))
            }
        case (.v7, .v8):
            return try autoreleasepool { () throws -> String in
                ...
                if markers.count == 1, markers[0].schemaVersion == 8 {
                    ...
                    return (try adjacentSemanticDigest(in: context, release: .v8, aggregate: aggregate))
                }
                guard (try adjacentSemanticDigest(in: context, release: .v7, aggregate: aggregate)) == expectedSemanticDigest else {
                    throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)
                }
        ...
        case (.v8, .v9):
            return try autoreleasepool { () throws -> String in
                ...
                if markers.count == 1, markers[0].schemaVersion == 9 {
                    ...
                    return (try adjacentSemanticDigest(in: context, release: .v9, aggregate: aggregate))
                }
                guard markers.count == 1, markers[0].schemaVersion == 8 else {
                    throw StoreMigrationFailure.maintenanceRequired(.targetMismatch)
                }
                guard (try adjacentSemanticDigest(in: context, release: .v8, aggregate: aggregate)) == expectedSemanticDigest else {
                    throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)
                }
        ...
        case (.v9, .v10):
            return try autoreleasepool { () throws -> String in
                ...
                if markers.count == 1, markers[0].schemaVersion == 10 {
                    ...
                    return (try adjacentSemanticDigest(in: context, release: .v10, aggregate: aggregate))
                }
                guard markers.count == 1, markers[0].schemaVersion == 9,
                      (try adjacentSemanticDigest(in: context, release: .v9, aggregate: aggregate)) == expectedSemanticDigest else {
                    throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)
                }
        ...
        case (.v10, .v11):
            return try autoreleasepool { () throws -> String in
                ...
                if markers.count == 1, markers[0].schemaVersion == 11 {
                    ...
                    return (try adjacentSemanticDigest(in: context, release: .v11, aggregate: aggregate))
                }
                guard markers.count == 1, markers[0].schemaVersion == 10,
                      (try adjacentSemanticDigest(in: context, release: .v10, aggregate: aggregate)) == expectedSemanticDigest,
        ...
        case (.v11, .v12):
            return try autoreleasepool { () throws -> String in
                ...
                if markers.count == 1, markers[0].schemaVersion == 12 {
                    ...
                    return (try adjacentSemanticDigest(in: context, release: .v12, aggregate: aggregate))
                }
                guard markers.count == 1, markers[0].schemaVersion == 11,
                      (try adjacentSemanticDigest(in: context, release: .v11, aggregate: aggregate)) == expectedSemanticDigest,
        ...
        case (.v12,.v13):return try autoreleasepool{let container=try makeV13Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==13{_ = try requireV13Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v13, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==12,(try adjacentSemanticDigest(in: context, release: .v12, aggregate: aggregate))==expectedSemanticDigest,try context.fetch(FetchDescriptor<EvidenceVisibilityRow>()).isEmpty,try context.fetch(FetchDescriptor<ClaimEvidenceLinkRow>()).isEmpty,try context.fetch(FetchDescriptor<AssuranceManifestRow>()).isEmpty,try context.fetch(FetchDescriptor<AttestationRow>()).isEmpty else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV13Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v13, aggregate: aggregate))}
        case (.v13,.v14):return try autoreleasepool{let container=try makeV14Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==14{_ = try requireV14Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v14, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==13,(try adjacentSemanticDigest(in: context, release: .v13, aggregate: aggregate))==expectedSemanticDigest,try reviewRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV14Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v14, aggregate: aggregate))}
        case (.v14,.v15):return try autoreleasepool{let c=try makeV15Container(at:modelStoreURL,migrate:true);let x=c.mainContext;let m=try x.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if m.count==1,m[0].schemaVersion==15{_ = try requireV15Marker(in:x,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: x, release: .v15, aggregate: aggregate))};guard m.count==1,m[0].schemaVersion==14,(try adjacentSemanticDigest(in: x, release: .v14, aggregate: aggregate))==expectedSemanticDigest,try workPacketRowsAreEmpty(in:x)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV15Marker(in:x,migrationID:migrationID);return (try adjacentSemanticDigest(in: x, release: .v15, aggregate: aggregate))}
        case (.v15,.v16):return try autoreleasepool{let container=try makeV16Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==16{_ = try requireV16Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v16, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==15,(try adjacentSemanticDigest(in: context, release: .v15, aggregate: aggregate))==expectedSemanticDigest,try fieldDraftRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV16Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v16, aggregate: aggregate))}
        case (.v16,.v17):return try autoreleasepool{let container=try makeV17Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==17{_ = try requireV17Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v17, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==16,(try adjacentSemanticDigest(in: context, release: .v16, aggregate: aggregate))==expectedSemanticDigest,try packageEvolutionRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV17Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v17, aggregate: aggregate))}
        case (.v17,.v18):return try autoreleasepool{let container=try makeV18Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==18{_ = try requireV18Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v18, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==17,(try adjacentSemanticDigest(in: context, release: .v17, aggregate: aggregate))==expectedSemanticDigest,try measurementIntegrityRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV18Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v18, aggregate: aggregate))}
        case (.v18,.v19):return try autoreleasepool{let container=try makeV19Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==19{_ = try requireV19Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v19, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==18,(try adjacentSemanticDigest(in: context, release: .v18, aggregate: aggregate))==expectedSemanticDigest,try privacyTransformRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV19Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v19, aggregate: aggregate))}
        case (.v19,.v20):return try autoreleasepool{let container=try makeV20Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==20{_ = try requireV20Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v20, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==19,(try adjacentSemanticDigest(in: context, release: .v19, aggregate: aggregate))==expectedSemanticDigest,try clientCapabilityRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV20Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v20, aggregate: aggregate))}
        case (.v20,.v21):return try autoreleasepool{let container=try makeV21Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==21{_ = try requireV21Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v21, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==20,(try adjacentSemanticDigest(in: context, release: .v20, aggregate: aggregate))==expectedSemanticDigest,try recoverabilityVerificationRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV21Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v21, aggregate: aggregate))}
        case (.v21,.v22):return try autoreleasepool{let container=try makeV22Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==22{_ = try requireV22Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v22, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==21,(try adjacentSemanticDigest(in: context, release: .v21, aggregate: aggregate))==expectedSemanticDigest,try fieldReferenceRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV22Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v22, aggregate: aggregate))}
        case (.v22,.v23):return try autoreleasepool{let container=try makeV23Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==23{_ = try requireV23Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v23, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==22,(try adjacentSemanticDigest(in: context, release: .v22, aggregate: aggregate))==expectedSemanticDigest,try accessibleDocumentRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV23Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v23, aggregate: aggregate))}
        case (.v23,.v24):return try autoreleasepool{let container=try makeV24Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==24{_ = try requireV24Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v24, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==23,(try adjacentSemanticDigest(in: context, release: .v23, aggregate: aggregate))==expectedSemanticDigest,try surveyDefinitionRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV24Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v24, aggregate: aggregate))}
        case (.v24,.v25):return try autoreleasepool{let container=try makeV25Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==25{_ = try requireV25Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v25, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==24,(try adjacentSemanticDigest(in: context, release: .v24, aggregate: aggregate))==expectedSemanticDigest,try surveySessionRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV25Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v25, aggregate: aggregate))}
        case (.v25,.v26):return try autoreleasepool{let container=try makeV26Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==26{_ = try requireV26Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v26, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==25,(try adjacentSemanticDigest(in: context, release: .v25, aggregate: aggregate))==expectedSemanticDigest,try assetLocatorRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV26Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v26, aggregate: aggregate))}
        case (.v26,.v27):return try autoreleasepool{let container=try makeV27Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==27{_ = try requireV27Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v27, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==26,(try adjacentSemanticDigest(in: context, release: .v26, aggregate: aggregate))==expectedSemanticDigest,try scheduleRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV27Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v27, aggregate: aggregate))}
        case (.v27,.v28):return try autoreleasepool{let container=try makeV28Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==28{_ = try requireV28Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v28, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==27,(try adjacentSemanticDigest(in: context, release: .v27, aggregate: aggregate))==expectedSemanticDigest,try planRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV28Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v28, aggregate: aggregate))}
        case (.v28,.v29):return try autoreleasepool{let container=try makeV29Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==29{_ = try requireV29Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v29, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==28,(try adjacentSemanticDigest(in: context, release: .v28, aggregate: aggregate))==expectedSemanticDigest,try placementPoseRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV29Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v29, aggregate: aggregate))}
        case (.v29,.v30):return try autoreleasepool{let container=try makeV30Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==30{_ = try requireV30Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v30, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==29,(try adjacentSemanticDigest(in: context, release: .v29, aggregate: aggregate))==expectedSemanticDigest,try evidenceContextRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV30Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v30, aggregate: aggregate))}
        case (.v30,.v31):return try autoreleasepool{let container=try makeV31Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==31{_ = try requireV31Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v31, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==30,(try adjacentSemanticDigest(in: context, release: .v30, aggregate: aggregate))==expectedSemanticDigest,try lightingRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV31Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v31, aggregate: aggregate))}
        case (.v31,.v32):return try autoreleasepool{let container=try makeV32Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==32{_ = try requireV32Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v32, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==31,(try adjacentSemanticDigest(in: context, release: .v31, aggregate: aggregate))==expectedSemanticDigest,try assistanceAcceptanceReceiptRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV32Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v32, aggregate: aggregate))}
        case (.v32,.v33):return try autoreleasepool{let container=try makeV33Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==33{_ = try requireV33Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v33, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==32,(try adjacentSemanticDigest(in: context, release: .v32, aggregate: aggregate))==expectedSemanticDigest,try temporalEvidenceRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV33Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v33, aggregate: aggregate))}
        case (.v33,.v34):return try autoreleasepool{let container=try makeV34Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==34{_ = try requireV34Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v34, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==33,(try adjacentSemanticDigest(in: context, release: .v33, aggregate: aggregate))==expectedSemanticDigest,try assetLabelRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV34Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v34, aggregate: aggregate))}
        case (.v34,.v35):return try autoreleasepool{let container=try makeV35Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==35{_ = try requireV35Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v35, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==34,(try adjacentSemanticDigest(in: context, release: .v34, aggregate: aggregate))==expectedSemanticDigest,try operationalContactRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV35Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v35, aggregate: aggregate))}
        case (.v35,.v36):return try autoreleasepool{let container=try makeV36Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==36{_ = try requireV36Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v36, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==35,(try adjacentSemanticDigest(in: context, release: .v35, aggregate: aggregate))==expectedSemanticDigest,try activityContractRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV36Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v36, aggregate: aggregate))}
        case (.v36,.v37):return try autoreleasepool{let container=try makeV37Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==37{_ = try requireV37Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v37, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==36,(try adjacentSemanticDigest(in: context, release: .v36, aggregate: aggregate))==expectedSemanticDigest,try workResourceRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV37Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v37, aggregate: aggregate))}
        case (.v37,.v38):return try autoreleasepool{let container=try makeV38Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==38{_ = try requireV38Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v38, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==37,(try adjacentSemanticDigest(in: context, release: .v37, aggregate: aggregate))==expectedSemanticDigest,try scheduleExceptionRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV38Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v38, aggregate: aggregate))}
        case (.v38,.v39):return try autoreleasepool{let container=try makeV39Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==39{_ = try requireV39Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v39, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==38,(try adjacentSemanticDigest(in: context, release: .v38, aggregate: aggregate))==expectedSemanticDigest,try serviceRequestRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV39Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v39, aggregate: aggregate))}
        case (.v39,.v40):return try autoreleasepool{let container=try makeV40Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==40{_ = try requireV40Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v40, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==39,(try adjacentSemanticDigest(in: context, release: .v39, aggregate: aggregate))==expectedSemanticDigest,try serviceReliabilityRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV40Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v40, aggregate: aggregate))}
        case (.v40,.v41):return try autoreleasepool{let container=try makeV41Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==41{_ = try requireV41Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v41, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==40,(try adjacentSemanticDigest(in: context, release: .v40, aggregate: aggregate))==expectedSemanticDigest,try partsStockRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV41Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v41, aggregate: aggregate))}
        case (.v41,.v42):return try autoreleasepool{let container=try makeV42Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==42{_ = try requireV42Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v42, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==41,(try adjacentSemanticDigest(in: context, release: .v41, aggregate: aggregate))==expectedSemanticDigest,try myDayRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV42Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v42, aggregate: aggregate))}
        case (.v42,.v43):return try autoreleasepool{let container=try makeV43Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==43{_ = try requireV43Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v43, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==42,(try adjacentSemanticDigest(in: context, release: .v42, aggregate: aggregate))==expectedSemanticDigest,try evidenceCurationRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV43Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v43, aggregate: aggregate))}
        case (.v43,.v44):return try autoreleasepool{let container=try makeV44Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==44{_ = try requireV44Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v44, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==43,(try adjacentSemanticDigest(in: context, release: .v43, aggregate: aggregate))==expectedSemanticDigest,try shopReportProfileRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV44Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v44, aggregate: aggregate))}
        case (.v44,.v45):return try autoreleasepool{let container=try makeV45Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==45{_ = try requireV45Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v45, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==44,(try adjacentSemanticDigest(in: context, release: .v44, aggregate: aggregate))==expectedSemanticDigest,try roundSessionRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV45Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v45, aggregate: aggregate))}
        case (.v45,.v46):return try autoreleasepool{let container=try makeV46Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==46{_ = try requireV46Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v46, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==45,(try adjacentSemanticDigest(in: context, release: .v45, aggregate: aggregate))==expectedSemanticDigest,try importBulkRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV46Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v46, aggregate: aggregate))}
        case (.v46,.v47):return try autoreleasepool{let container=try makeV47Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==47{_ = try requireV47Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v47, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==46,(try adjacentSemanticDigest(in: context, release: .v46, aggregate: aggregate))==expectedSemanticDigest,try evidenceQualityRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV47Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v47, aggregate: aggregate))}
        case (.v47,.v48):return try autoreleasepool{let container=try makeV48Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==48{_ = try requireV48Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v48, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==47,(try adjacentSemanticDigest(in: context, release: .v47, aggregate: aggregate))==expectedSemanticDigest,try fastSurveyInboxRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV48Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v48, aggregate: aggregate))}
        case (.v48,.v49):return try autoreleasepool{let container=try makeV49Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==49{_ = try requireV49Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v49, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==48,(try adjacentSemanticDigest(in: context, release: .v48, aggregate: aggregate))==expectedSemanticDigest,try reinspectionExceptionRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV49Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v49, aggregate: aggregate))}
        case (.v49,.v50):return try autoreleasepool{let container=try makeV50Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==50{_ = try requireV50Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v50, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==49,(try adjacentSemanticDigest(in: context, release: .v49, aggregate: aggregate))==expectedSemanticDigest,try entityIdentityResolutionRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV50Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v50, aggregate: aggregate))}
        case (.v50,.v51):return try autoreleasepool{let container=try makeV51Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==51{_ = try requireV51Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v51, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==50,(try adjacentSemanticDigest(in: context, release: .v50, aggregate: aggregate))==expectedSemanticDigest,try workspaceExperienceRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV51Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v51, aggregate: aggregate))}
        case (.v51,.v52):return try autoreleasepool{let container=try makeV52Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==52{_ = try requireV52Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v52, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==51,(try adjacentSemanticDigest(in: context, release: .v51, aggregate: aggregate))==expectedSemanticDigest,try lightingDayInventoryRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV52Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v52, aggregate: aggregate))}
        case (.v52,.v53):return try autoreleasepool{let container=try makeV53Container(at:modelStoreURL,migrate:true);let context=container.mainContext;let markers=try context.fetch(FetchDescriptor<PersistentSchemaReleaseMarker>());if markers.count==1,markers[0].schemaVersion==53{_ = try requireV53Marker(in:context,expectedMigrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v53, aggregate: aggregate))};guard markers.count==1,markers[0].schemaVersion==52,(try adjacentSemanticDigest(in: context, release: .v52, aggregate: aggregate))==expectedSemanticDigest,try lightingNightWorkflowRowsAreEmpty(in:context)else{throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)};try backfillV53Marker(in:context,migrationID:migrationID);return (try adjacentSemanticDigest(in: context, release: .v53, aggregate: aggregate))}
        default:
            throw StoreMigrationFailure.invalidContract
        }
    }
    ...
    private func semanticProjection(in context: ModelContext, release: PersistentSchemaReleaseV1,
                                    purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        if release == .v21{return try semanticExportV21(in:context, purpose: purpose)}
        if release == .v22{return try semanticExportV22(in:context, purpose: purpose)}
        if release == .v23{return try semanticExportV23(in:context, purpose: purpose)}
        if release == .v24{return try semanticExportV24(in:context, purpose: purpose)}
        if release == .v25{return try semanticExportV25(in:context, purpose: purpose)}
        if release == .v26{return try semanticExportV26(in:context, purpose: purpose)}
        if release == .v27{return try semanticExportV27(in:context, purpose: purpose)}
        if release == .v28{return try semanticExportV28(in:context, purpose: purpose)}
        if release == .v29{return try semanticExportV29(in:context, purpose: purpose)}
        if release == .v30{return try semanticExportV30(in:context, purpose: purpose)}
        if release == .v31{return try semanticExportV31(in:context, purpose: purpose)}
        if release == .v32{return try semanticExportV32(in:context, purpose: purpose)}
        if release == .v33{return try semanticExportV33(in:context, purpose: purpose)}
        if release == .v34{return try semanticExportV34(in:context, purpose: purpose)}
        if release == .v35{return try semanticExportV35(in:context, purpose: purpose)}
        if release == .v36{return try semanticExportV36(in:context, purpose: purpose)}
        if release == .v37{return try semanticExportV37(in:context, purpose: purpose)}
        if release == .v38{return try semanticExportV38(in:context, purpose: purpose)}
        if release == .v39{return try semanticExportV39(in:context, purpose: purpose)}
        if release == .v40{return try semanticExportV40(in:context, purpose: purpose)}
        if release == .v41{return try semanticExportV41(in:context, purpose: purpose)}
        if release == .v42{return try semanticExportV42(in:context, purpose: purpose)}
        if release == .v43{return try semanticExportV43(in:context, purpose: purpose)}
        if release == .v44{return try semanticExportV44(in:context, purpose: purpose)}
        if release == .v45{return try semanticExportV45(in:context, purpose: purpose)}
        if release == .v46{return try semanticExportV46(in:context, purpose: purpose)}
        if release == .v47{return try semanticExportV47(in:context, purpose: purpose)}
        if release == .v48{return try semanticExportV48(in:context, purpose: purpose)}
        if release == .v49{return try semanticExportV49(in:context, purpose: purpose)}
        if release == .v50{return try semanticExportV50(in:context, purpose: purpose)}
        if release == .v51{return try semanticExportV51(in:context, purpose: purpose)}
        if release == .v52{return try semanticExportV52(in:context, purpose: purpose)}
        if release == .v53{return try semanticExportV53(in:context, purpose: purpose)}
        if release == .v20{return try semanticExportV20(in:context, purpose: purpose)}
        if release == .v19{return try semanticExportV19(in:context, purpose: purpose)}
        if release == .v18{return try semanticExportV18(in:context, purpose: purpose)}
        if release == .v17{return try semanticExportV17(in:context, purpose: purpose)}
        if release == .v16{return try semanticExportV16(in:context, purpose: purpose)}
        if release == .v15{return try semanticExportV15(in:context, purpose: purpose)}
        if release == .v14{return try semanticExportV14(in:context, purpose: purpose)}
        if release == .v13{return try semanticExportV13(in:context, purpose: purpose)}
        if release == .v12 { return try semanticExportV12(in: context, purpose: purpose) }
        if release == .v11 { return try semanticExportV11(in: context, purpose: purpose) }
        if release == .v10 { return try semanticExportV10(in: context, purpose: purpose) }
        if release == .v9 { return try semanticExportV9(in: context, purpose: purpose) }
        if release == .v8 { return try semanticExportV8(in: context, purpose: purpose) }
        if release == .v7 { return try semanticExportV7(in: context, purpose: purpose) }
        if release == .v6 { return try semanticExportV6(in: context, purpose: purpose) }
        if release == .v5 { return try semanticExportV5(in: context, purpose: purpose) }
        if release == .v4 { return try semanticExportV4(in: context, purpose: purpose) }
    ...
    @MainActor
    private func semanticExportV3(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        let records = try semanticExport(in: context)
        let ledger = try DeletionLedgerStore(context: context).snapshot().canonicalData()
        return try purpose.encode(
            StoreSemanticEnvelopeV3(records: records, deletionLedger: ledger)
        )
    }

    @MainActor
    private func semanticExportV4(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        let base = try semanticExportV3(in: context, purpose: purpose)
    ...
    @MainActor
    private func semanticExportV5(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        let base = try semanticExportV4(in: context, purpose: purpose)
        ...
        let observationAndTime = try records.map { record in
            ...
            return try ObservationAndTimeSemanticV1(
                recordID: record.id,
    ...
    @MainActor
    private func semanticExportV6(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        let base = try semanticExportV5(in: context, purpose: purpose)
        ...
        return try purpose.encode(StoreSemanticEnvelopeV6(
            base: base,
    ...
    @MainActor
    private func semanticExportV7(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        let base = try semanticExportV6(in: context, purpose: purpose)
    ...
    @MainActor
    private func semanticExportV8(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        let base = try semanticExportV7(in: context, purpose: purpose)
    ...
    @MainActor
    private func semanticExportV9(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        let base = try semanticExportV8(in: context, purpose: purpose)
        ...
        return try purpose.encode(StoreSemanticEnvelopeV9(base: base, parties: parties, roles: roles, actors: actors, qualifications: qualifications, signoffs: signoffs))
    }

    @MainActor
    private func semanticExportV10(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        let base = try semanticExportV9(in: context, purpose: purpose)
        ...
        return try purpose.encode(StoreSemanticEnvelopeV10(
            base: base,
    ...
    @MainActor
    private func semanticExportV11(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        try purpose.encode(StoreSemanticEnvelopeV11(
            base: semanticExportV10(in: context, purpose: purpose),
    ...
    @MainActor
    private func semanticExportV12(in context: ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes) throws -> Data {
        try purpose.encode(StoreSemanticEnvelopeV12(
            base: semanticExportV11(in: context, purpose: purpose),
    ...
    @MainActor private func semanticExportV13(in context:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV13(base:semanticExportV12(in:context, purpose: purpose),visibilities:context.fetch(FetchDescriptor<EvidenceVisibilityRow>(sortBy:[SortDescriptor(\.visibilityID)])).map{try $0.value()},links:context.fetch(FetchDescriptor<ClaimEvidenceLinkRow>(sortBy:[SortDescriptor(\.linkID)])).map{try $0.value()},manifests:context.fetch(FetchDescriptor<AssuranceManifestRow>(sortBy:[SortDescriptor(\.manifestID)])).map{try $0.value()},attestations:context.fetch(FetchDescriptor<AttestationRow>(sortBy:[SortDescriptor(\.attestationID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV14(in context:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV14(base:semanticExportV13(in:context, purpose: purpose),transitions:context.fetch(FetchDescriptor<InspectionReviewTransitionRow>(sortBy:[SortDescriptor(\.transitionID)])).map{try $0.value()},dispositions:context.fetch(FetchDescriptor<ReviewDispositionRow>(sortBy:[SortDescriptor(\.dispositionID)])).map{try $0.value()},changeRequests:context.fetch(FetchDescriptor<ChangeRequestRow>(sortBy:[SortDescriptor(\.requestRevisionID)])).map{try $0.value()},policies:context.fetch(FetchDescriptor<CorrectiveActionPolicyRow>(sortBy:[SortDescriptor(\.releaseID)])).map{try $0.value()},events:context.fetch(FetchDescriptor<CorrectiveActionEventRow>(sortBy:[SortDescriptor(\.eventID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV15(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV15(base:semanticExportV14(in:c, purpose: purpose),manifests:c.fetch(FetchDescriptor<WorkPacketManifestRow>(sortBy:[SortDescriptor(\.manifestID)])).map{try $0.value()},claims:c.fetch(FetchDescriptor<WorkItemClaimRow>(sortBy:[SortDescriptor(\.claimID)])).map{try $0.value()},leases:c.fetch(FetchDescriptor<WorkLeaseRow>(sortBy:[SortDescriptor(\.leaseID)])).map{try $0.value()},releases:c.fetch(FetchDescriptor<WorkReleaseRow>(sortBy:[SortDescriptor(\.releaseID)])).map{try $0.value()},handoffs:c.fetch(FetchDescriptor<WorkHandoffRow>(sortBy:[SortDescriptor(\.handoffID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV16(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV16(base:semanticExportV15(in:c, purpose: purpose),checkpoints:c.fetch(FetchDescriptor<FieldDraftCheckpointRow>(sortBy:[SortDescriptor(\.draftID)])).map{try $0.value()},stages:c.fetch(FetchDescriptor<AttachmentStagingItemRow>(sortBy:[SortDescriptor(\.stageID)])).map{try $0.value()},sagas:c.fetch(FetchDescriptor<DraftCommitSagaRow>(sortBy:[SortDescriptor(\.sagaID)])).map{try $0.value()},reservations:c.fetch(FetchDescriptor<DraftContentReservationRow>(sortBy:[SortDescriptor(\.reservationID)])).map{try $0.value()},commitReceipts:c.fetch(FetchDescriptor<DraftCommitReceiptRow>(sortBy:[SortDescriptor(\.receiptID)])).map{try $0.value()},discardReceipts:c.fetch(FetchDescriptor<DraftDiscardReceiptRow>(sortBy:[SortDescriptor(\.receiptID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV17(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV17(base:semanticExportV16(in:c, purpose: purpose),promotedReleases:c.fetch(FetchDescriptor<PromotedPackageReleaseRow>(sortBy:[SortDescriptor(\.releaseRecordID)])).map{try $0.value()},sandboxRuns:c.fetch(FetchDescriptor<PackageSandboxRunRow>(sortBy:[SortDescriptor(\.runID)])).map{try $0.value()},promotionReceipts:c.fetch(FetchDescriptor<PackagePromotionReceiptRow>(sortBy:[SortDescriptor(\.receiptID)])).map{try $0.value()},activePointers:c.fetch(FetchDescriptor<ActivePackageRegistryPointerRow>(sortBy:[SortDescriptor(\.pointerID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV18(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV18(base:semanticExportV17(in:c, purpose: purpose),instruments:c.fetch(FetchDescriptor<InstrumentReferenceRow>(sortBy:[SortDescriptor(\.referenceID)])).map{try $0.value()},calibrations:c.fetch(FetchDescriptor<CalibrationStatusSnapshotRow>(sortBy:[SortDescriptor(\.snapshotID)])).map{try $0.value()},captures:c.fetch(FetchDescriptor<MeasurementCaptureRow>(sortBy:[SortDescriptor(\.captureID)])).map{try $0.value()},series:c.fetch(FetchDescriptor<MeasurementSeriesRow>(sortBy:[SortDescriptor(\.snapshotID)])).map{try $0.value()},assessments:c.fetch(FetchDescriptor<MeasurementQualityAssessmentRow>(sortBy:[SortDescriptor(\.assessmentID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV19(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let values=try privacyTransformValuesV19(in:c);return try purpose.encode(StoreSemanticEnvelopeV19(base:semanticExportV18(in:c, purpose: purpose),policies:values.policies,regions:values.regions,manifests:values.manifests,reviews:values.reviews))}
    @MainActor private func semanticExportV20(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let v=try clientCapabilityValuesV20(in:c);return try purpose.encode(StoreSemanticEnvelopeV20(base:semanticExportV19(in:c, purpose: purpose),profiles:v.profiles,decisions:v.decisions,policies:v.policies,dispositions:v.dispositions))}
    @MainActor private func semanticExportV21(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV21(base:semanticExportV20(in:c, purpose: purpose),receipts:c.fetch(FetchDescriptor<RecoverabilityVerificationReceiptRow>(sortBy:[SortDescriptor(\.receiptID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV22(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let releases=try c.fetch(FetchDescriptor<FieldReferenceReleaseRow>(sortBy:[SortDescriptor(\.releaseID)])).map{try $0.value()};let bindings=try c.fetch(FetchDescriptor<FieldReferenceBindingRow>(sortBy:[SortDescriptor(\.bindingID)])).map{row in guard let release=releases.first(where:{$0.releaseID==row.releaseID})else{throw StoreMigrationFailure.maintenanceRequired(.targetMismatch)};return try row.value(release:release)};return try purpose.encode(StoreSemanticEnvelopeV22(base:semanticExportV21(in:c, purpose: purpose),releases:releases,bindings:bindings))}
    @MainActor private func semanticExportV23(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV23(base:semanticExportV22(in:c, purpose: purpose),receipts:c.fetch(FetchDescriptor<AccessibleDocumentAssessmentReceiptRow>(sortBy:[SortDescriptor(\.receiptID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV24(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV24(base:semanticExportV23(in:c, purpose: purpose),identities:c.fetch(FetchDescriptor<SurveyDefinitionIdentityRow>(sortBy:[SortDescriptor(\.definitionID)])).map{try $0.value()},releases:c.fetch(FetchDescriptor<SurveyDefinitionReleaseRow>(sortBy:[SortDescriptor(\.releaseID)])).map{try $0.value()}))}
    @MainActor private func semanticExportV25(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV25(base:semanticExportV24(in:c, purpose: purpose),sessions:c.fetch(FetchDescriptor<SurveySessionRow>()).map{try $0.value()}.sorted{$0.sessionID.uuidString<$1.sessionID.uuidString},facts:c.fetch(FetchDescriptor<FactCaptureRow>()).map{try $0.value()}.sorted{$0.captureID.uuidString<$1.captureID.uuidString},subjects:c.fetch(FetchDescriptor<ProvisionalSubjectRow>()).map{try $0.value()}.sorted{$0.provisionalSubjectID.uuidString<$1.provisionalSubjectID.uuidString},promotions:c.fetch(FetchDescriptor<SubjectPromotionReceiptRow>()).map{try $0.value()}.sorted{$0.receiptID.uuidString<$1.receiptID.uuidString},publications:c.fetch(FetchDescriptor<SurveyPublicationSnapshotRow>()).map{try $0.value()}.sorted{$0.snapshotID.uuidString<$1.snapshotID.uuidString}))}
    @MainActor private func semanticExportV26(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV26(base:semanticExportV25(in:c, purpose: purpose),locators:c.fetch(FetchDescriptor<AssetLocatorRow>()).map{try $0.value()}.sorted{$0.locatorID.uuidString<$1.locatorID.uuidString},receipts:c.fetch(FetchDescriptor<LocatorBindingReceiptRow>()).map{try $0.value()}.sorted{$0.receiptID.uuidString<$1.receiptID.uuidString}))}
    @MainActor private func semanticExportV27(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV27(base:semanticExportV26(in:c, purpose: purpose),releases:c.fetch(FetchDescriptor<ScheduleDefinitionReleaseRow>()).map{try $0.value()}.sorted{$0.releaseID.uuidString<$1.releaseID.uuidString},events:c.fetch(FetchDescriptor<OccurrenceHistoryEventRow>()).map{try $0.value()}.sorted{$0.eventID.uuidString<$1.eventID.uuidString}))}
    @MainActor private func semanticExportV28(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV28(base:semanticExportV27(in:c, purpose: purpose),documents:c.fetch(FetchDescriptor<PlanDocumentRow>()).map{try $0.value()}.sorted{($0.planDocumentID.uuidString,$0.revision)<($1.planDocumentID.uuidString,$1.revision)},revisions:c.fetch(FetchDescriptor<PlanRevisionRow>()).map{try $0.value()}.sorted{$0.planRevisionID.uuidString<$1.planRevisionID.uuidString},placements:c.fetch(FetchDescriptor<PlanPlacementRow>()).map{try $0.value()}.sorted{($0.placementID.uuidString,$0.revision)<($1.placementID.uuidString,$1.revision)},receipts:c.fetch(FetchDescriptor<RebaseReceiptRow>()).map{try $0.value()}.sorted{$0.receiptID.uuidString<$1.receiptID.uuidString}))}
    @MainActor private func semanticExportV29(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV29(base:semanticExportV28(in:c, purpose: purpose),events:c.fetch(FetchDescriptor<AssetPoseEventRow>()).map{try $0.value()}.sorted{$0.eventID.uuidString<$1.eventID.uuidString},observations:c.fetch(FetchDescriptor<SpatialAnchorObservationRow>()).map{try $0.value()}.sorted{$0.observationID.uuidString<$1.observationID.uuidString}))}
    @MainActor private func semanticExportV30(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV30(base:semanticExportV29(in:c, purpose: purpose),contexts:c.fetch(FetchDescriptor<EvidenceContextRow>()).map{try $0.value()}.sorted{$0.contextID.uuidString<$1.contextID.uuidString},pairs:c.fetch(FetchDescriptor<PairedObservationLinkRow>()).map{try $0.value()}.sorted{$0.linkID.uuidString<$1.linkID.uuidString}))}
    @MainActor private func semanticExportV31(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV31(base:semanticExportV30(in:c, purpose: purpose),systems:c.fetch(FetchDescriptor<LightingSystemRow>()).map{try $0.value()}.sorted{$0.recordID.uuidString<$1.recordID.uuidString},observations:c.fetch(FetchDescriptor<LightingObservationRow>()).map{try $0.value()}.sorted{$0.recordID.uuidString<$1.recordID.uuidString},issues:c.fetch(FetchDescriptor<LightingIssueRow>()).map{try $0.value()}.sorted{$0.recordID.uuidString<$1.recordID.uuidString},plans:c.fetch(FetchDescriptor<MeasurementPlanRow>()).map{try $0.value()}.sorted{$0.recordID.uuidString<$1.recordID.uuidString},claims:c.fetch(FetchDescriptor<LightingClaimStateRow>()).map{try $0.value()}.sorted{$0.recordID.uuidString<$1.recordID.uuidString}))}
    @MainActor private func semanticExportV32(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV32(base:semanticExportV31(in:c, purpose: purpose),assistanceAcceptanceReceipts:c.fetch(FetchDescriptor<AssistanceAcceptanceReceiptRow>()).map{try $0.value()}.sorted{$0.receiptID.uuidString<$1.receiptID.uuidString}))}
    @MainActor private func semanticExportV33(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV33(base:semanticExportV32(in:c, purpose: purpose),clips:c.fetch(FetchDescriptor<TemporalEvidenceClipRow>()).map{try $0.value()}.sorted{$0.clipID.uuidString<$1.clipID.uuidString},anchors:c.fetch(FetchDescriptor<TimecodedEvidenceAnchorRow>()).map{try $0.value()}.sorted{$0.anchorID.uuidString<$1.anchorID.uuidString}))}
    @MainActor private func semanticExportV34(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV34(base:semanticExportV33(in:c, purpose: purpose),snapshots:c.fetch(FetchDescriptor<AcceptedLabelGenerationSnapshotRow>()).map{try $0.value()}.sorted{($0.workspaceID.rawValue.uuidString,$0.snapshotID.uuidString)<($1.workspaceID.rawValue.uuidString,$1.snapshotID.uuidString)}))}
    @MainActor private func semanticExportV35(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let contacts=try c.fetch(FetchDescriptor<ServiceContactPointRow>()).map{try $0.value()}.sorted{($0.workspaceID.rawValue.uuidString,$0.contactPointID.uuidString)<($1.workspaceID.rawValue.uuidString,$1.contactPointID.uuidString)},intents=try c.fetch(FetchDescriptor<SystemHandoffIntentRow>()).map{try $0.value()}.sorted{($0.workspaceID.rawValue.uuidString,$0.intentID.uuidString)<($1.workspaceID.rawValue.uuidString,$1.intentID.uuidString)};return try purpose.encode(StoreSemanticEnvelopeV35(base:semanticExportV34(in:c, purpose: purpose),contacts:contacts,handoffIntents:intents))}
    @MainActor private func semanticExportV36(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{try purpose.encode(StoreSemanticEnvelopeV36(base:semanticExportV35(in:c, purpose: purpose),envelopes:c.fetch(FetchDescriptor<ActivitySessionEnvelopeRow>()).map{try $0.value()}.sorted{($0.workspaceID.rawValue.uuidString,$0.activityID.uuidString)<($1.workspaceID.rawValue.uuidString,$1.activityID.uuidString)},transitions:c.fetch(FetchDescriptor<ActivityStateTransitionRow>()).map{try $0.value()}.sorted{$0.transitionID.uuidString<$1.transitionID.uuidString},installationTaskResults:c.fetch(FetchDescriptor<InstallationTaskResultRow>()).map{try $0.value()}.sorted(),installationAsBuiltSnapshots:c.fetch(FetchDescriptor<InstallationAsBuiltSnapshotRow>()).map{try $0.value()}.sorted{$0.snapshotID.uuidString<$1.snapshotID.uuidString},punchReviewBasisSnapshots:c.fetch(FetchDescriptor<PunchReviewBasisSnapshotRow>()).map{try $0.value()}.sorted{$0.basisID.uuidString<$1.basisID.uuidString}))}
    @MainActor private func semanticExportV37(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let entries=try c.fetch(FetchDescriptor<ManualWorkResourceRecordRow>()).map{try $0.value()}.sorted{($0.workspaceID.rawValue.uuidString.lowercased(),$0.entryID.uuidString.lowercased())<($1.workspaceID.rawValue.uuidString.lowercased(),$1.entryID.uuidString.lowercased())};return try purpose.encode(StoreSemanticEnvelopeV37(base:semanticExportV36(in:c, purpose: purpose),workResources:entries))}
    @MainActor private func semanticExportV38(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let calendars=try c.fetch(FetchDescriptor<ExceptionCalendarReleaseRow>()).map{try $0.value()}.sorted{$0.releaseID.uuidString.lowercased()<$1.releaseID.uuidString.lowercased()},overrides=try c.fetch(FetchDescriptor<ScheduleOverrideEventRow>()).map{try $0.value()}.sorted{$0.eventID.uuidString.lowercased()<$1.eventID.uuidString.lowercased()};return try purpose.encode(StoreSemanticEnvelopeV38(base:semanticExportV37(in:c, purpose: purpose),calendars:calendars,overrides:overrides))}
    @MainActor private func semanticExportV39(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let records=try c.fetch(FetchDescriptor<ServiceRequestRecordRow>()).map{try $0.value()}.sorted{($0.workspaceID.rawValue.uuidString,$0.recordID.uuidString,$0.revision)<($1.workspaceID.rawValue.uuidString,$1.recordID.uuidString,$1.revision)},dispositions=try c.fetch(FetchDescriptor<ServiceRequestDispositionEventRow>()).map{try $0.value()}.sorted{$0.eventID.uuidString<$1.eventID.uuidString},links=try c.fetch(FetchDescriptor<ServiceRequestWorkLinkEventRow>()).map{try $0.value()}.sorted{$0.eventID.uuidString<$1.eventID.uuidString};return try purpose.encode(StoreSemanticEnvelopeV39(base:semanticExportV38(in:c, purpose: purpose),records:records,dispositions:dispositions,workLinks:links))}
    @MainActor private func semanticExportV40(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{func ordered<T>(_ values:[T],key:(T)->(String,String,UInt64))->[T]{values.sorted{key($0)<key($1)}};let incidents=ordered(try c.fetch(FetchDescriptor<AssetServiceIncidentRow>()).map{try $0.value()}){($0.workspaceID.rawValue.uuidString,$0.eventID.uuidString,$0.revision)},impacts=ordered(try c.fetch(FetchDescriptor<ServiceImpactSegmentRow>()).map{try $0.value()}){($0.workspaceID.rawValue.uuidString,$0.eventID.uuidString,$0.revision)},causes=ordered(try c.fetch(FetchDescriptor<ServiceCauseAssertionRow>()).map{try $0.value()}){($0.workspaceID.rawValue.uuidString,$0.eventID.uuidString,$0.revision)},remedies=ordered(try c.fetch(FetchDescriptor<ServiceRemedyAssertionRow>()).map{try $0.value()}){($0.workspaceID.rawValue.uuidString,$0.eventID.uuidString,$0.revision)},repairs=ordered(try c.fetch(FetchDescriptor<ServiceRepairIntervalRow>()).map{try $0.value()}){($0.workspaceID.rawValue.uuidString,$0.eventID.uuidString,$0.revision)},restorations=ordered(try c.fetch(FetchDescriptor<ServiceRestorationAssertionRow>()).map{try $0.value()}){($0.workspaceID.rawValue.uuidString,$0.eventID.uuidString,$0.revision)},exposures=ordered(try c.fetch(FetchDescriptor<QualifiedServiceExposureRow>()).map{try $0.value()}){($0.workspaceID.rawValue.uuidString,$0.eventID.uuidString,$0.revision)};let events=try (incidents.map{try ServiceReliabilityCanonicalCodecV1.encode($0)}+impacts.map{try ServiceReliabilityCanonicalCodecV1.encode($0)}+causes.map{try ServiceReliabilityCanonicalCodecV1.encode($0)}+remedies.map{try ServiceReliabilityCanonicalCodecV1.encode($0)}+repairs.map{try ServiceReliabilityCanonicalCodecV1.encode($0)}+restorations.map{try ServiceReliabilityCanonicalCodecV1.encode($0)}+exposures.map{try ServiceReliabilityCanonicalCodecV1.encode($0)});return try purpose.encode(StoreSemanticEnvelopeV40(base:semanticExportV39(in:c, purpose: purpose),events:events))}
    @MainActor private func semanticExportV41(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let parts=try c.fetch(FetchDescriptor<LocalPartDefinitionRowV1>()).map{try $0.value()}.sorted{$0.partID.uuidString<$1.partID.uuidString},locations=try c.fetch(FetchDescriptor<StockStorageLocationRowV1>()).map{try $0.value()}.sorted{$0.locationID.uuidString<$1.locationID.uuidString},movements=try c.fetch(FetchDescriptor<StockMovementEventRowV1>()).map{try $0.value()}.sorted{$0.movementID.uuidString<$1.movementID.uuidString},uses=try c.fetch(FetchDescriptor<StockUseReceiptRowV1>()).map{try $0.value()}.sorted{$0.receiptID.uuidString<$1.receiptID.uuidString},reversals=try c.fetch(FetchDescriptor<StockUseReversalReceiptRowV1>()).map{try $0.value()}.sorted{$0.receiptID.uuidString<$1.receiptID.uuidString},returns=try c.fetch(FetchDescriptor<StockReturnReceiptRowV1>()).map{try $0.value()}.sorted{$0.receiptID.uuidString<$1.receiptID.uuidString},abandonments=try c.fetch(FetchDescriptor<AbandonUnverifiedStockRowV1>()).map{try $0.value()}.sorted{$0.dispositionID.uuidString<$1.dispositionID.uuidString};return try purpose.encode(StoreSemanticEnvelopeV41(base:semanticExportV40(in:c, purpose: purpose),parts:parts,locations:locations,movements:movements,uses:uses,reversals:reversals,returns:returns,abandonments:abandonments))}
    @MainActor private func semanticExportV42(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let plans=try c.fetch(FetchDescriptor<MyDayPlanRowV1>()).map{try $0.value()}.sorted{($0.key.stableKey,$0.revision,$0.planID.uuidString)<($1.key.stableKey,$1.revision,$1.planID.uuidString)},receipts=try c.fetch(FetchDescriptor<MyDayCarryoverReceiptRowV1>()).map{try $0.value()}.sorted{$0.receiptSHA256<$1.receiptSHA256};return try purpose.encode(StoreSemanticEnvelopeV42(base:semanticExportV41(in:c, purpose: purpose),plans:plans,carryoverReceipts:receipts))}
    @MainActor private func semanticExportV43(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let associations=try c.fetch(FetchDescriptor<EvidenceAssociationEventRowV1>()).map{try $0.value()}.sorted{($0.workspaceID,$0.evidenceID,$0.resultingEvidenceRevision,$0.associationEventID)<($1.workspaceID,$1.evidenceID,$1.resultingEvidenceRevision,$1.associationEventID)},sequences=try c.fetch(FetchDescriptor<EvidenceSequenceRevisionRowV1>()).map{try $0.value()}.sorted{($0.workspaceID.rawValue.uuidString,$0.sequenceID.uuidString,$0.revision)<($1.workspaceID.rawValue.uuidString,$1.sequenceID.uuidString,$1.revision)};return try purpose.encode(StoreSemanticEnvelopeV43(base:semanticExportV42(in:c, purpose: purpose),associations:associations,sequences:sequences))}
    @MainActor private func semanticExportV44(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let profiles=try validatedShopReportProfileValues(in:c);return try purpose.encode(StoreSemanticEnvelopeV44(base:semanticExportV43(in:c, purpose: purpose),profiles:profiles))}
    @MainActor private func semanticExportV45(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let sessions=try validatedRoundSessionValues(in:c);return try purpose.encode(StoreSemanticEnvelopeV45(base:semanticExportV44(in:c, purpose: purpose),sessions:sessions))}
    @MainActor private func semanticExportV46(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let profiles=try c.fetch(FetchDescriptor<ImportMappingProfileRowV1>()).map{try $0.value()}.sorted{$0.profileID.uuidString<$1.profileID.uuidString};let sessions=try c.fetch(FetchDescriptor<BulkSessionRowV1>()).map{try $0.value()}.sorted{$0.sessionID.uuidString<$1.sessionID.uuidString};let receipts=try c.fetch(FetchDescriptor<BulkCommitReceiptRowV1>()).map{try $0.value()}.sorted{$0.receiptID.uuidString<$1.receiptID.uuidString};try profiles.forEach{try $0.validate()};try sessions.forEach{try $0.validate()};try receipts.forEach{try $0.validate()};return try purpose.encode(StoreSemanticEnvelopeV46(base:semanticExportV45(in:c, purpose: purpose),profiles:profiles,sessions:sessions,receipts:receipts))}
    @MainActor private func semanticExportV47(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let rows=try (c.fetch(FetchDescriptor<EvidenceQualityRuleSetRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<EvidenceQualityAssessmentRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<EvidenceQualityWaiverRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<EvidenceQualityMutationReceiptRowV1>()).map(\.canonicalData)).sorted{$0.lexicographicallyPrecedes($1)};return try purpose.encode(StoreSemanticEnvelopeV47(base:semanticExportV46(in:c, purpose: purpose),rows:rows))}
    @MainActor private func semanticExportV48(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let rows=try (c.fetch(FetchDescriptor<CaptureInboxItemRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<CapturePromotionRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<SnippetRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<SnippetInsertionHistoryRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<FastSurveyInboxMutationReceiptRowV1>()).map(\.canonicalData)).sorted{$0.lexicographicallyPrecedes($1)};return try purpose.encode(StoreSemanticEnvelopeV48(base:semanticExportV47(in:c, purpose: purpose),rows:rows))}
    @MainActor private func semanticExportV49(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let rows=try (c.fetch(FetchDescriptor<ReinspectionPlanRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<UnchangedAttestationRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<ExceptionQueueAcknowledgementRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<ReinspectionExceptionMutationReceiptRowV1>()).map(\.canonicalData)).sorted{$0.lexicographicallyPrecedes($1)};return try purpose.encode(StoreSemanticEnvelopeV49(base:semanticExportV48(in:c, purpose: purpose),rows:rows))}
    @MainActor private func semanticExportV50(in c:ModelContext, purpose: StoreSemanticExportPurposeV1 = .canonicalBytes)throws->Data{let rows=try (c.fetch(FetchDescriptor<EntityAliasLinkRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<EntityConsolidationReceiptRowV1>()).map(\.canonicalData)+c.fetch(FetchDescriptor<EntityIdentityResolutionMutationReceiptRowV1>()).map(\.canonicalData)).sorted{$0.lexicographicallyPrecedes($1)};return try purpose.encode(StoreSemanticEnvelopeV50(base:semanticExportV49(in:c, purpose: purpose),rows:rows))}
    ...
    /// Aggregate journal schema 2 format: V1/V2 keep their frozen flat records
    /// digest; every V3...V53 candidate uses the bounded framed layer digest.
    /// Nested canonical bytes grow by base64 (4/3) per release and are never
    /// materialized on the skipped-release upgrade path.
    @MainActor
    private func aggregateSemanticDigest(
        in context: ModelContext,
        release: PersistentSchemaReleaseV1
    ) throws -> String {
    ...
    /// The original single-step journal keeps its recorded nested digests; only
    /// the aggregate journal (schema 2) uses the framed format.
    @MainActor
    private func adjacentSemanticDigest(
        in context: ModelContext,
        release: PersistentSchemaReleaseV1,
    ...
    private func backfillV5ObservationAndTimeMarker(
        ...
        _ = try semanticExportV5(in: context, purpose: .validationOnly)
    }
...
final class StoreRestoreGenerationAuthority {
    ...
    func switchCurrentGeneration(
        ...
        switch expected {
        case .legacy, .v2: break
        case .v3: throw StoreGenerationFailure.dataPointerInvalid
        }
        try pointer.validate()
        guard expected.generationID == Self.canonical(oldID),
              pointer.generationID == Self.canonical(newID), oldID != newID,
              try currentGenerationID() == oldID else {
            throw StoreGenerationFailure.dataPointerInvalid
        }
        _ = try Self.requiredDirectoryIdentity(parent: installedGenerationsDescriptor, name: Self.canonical(oldID))
        _ = try Self.requiredDirectoryIdentity(parent: installedGenerationsDescriptor, name: Self.canonical(newID))
        let captured = try Self.readRegularFileWithIdentity(parent: dataDescriptor, name: "current.json")
        guard captured.data == expectedCurrentPointerData else {
            throw StoreGenerationFailure.dataPointerInvalid
        }
        try replacePointer(name: "current.json", value: pointer, expectedData: expectedCurrentPointerData)
        guard try currentGenerationID() == newID else {
            throw StoreGenerationFailure.dataPointerInvalid
        }
    }
...
struct StoreGenerationFactory {
    ...
    private func switchCurrentGenerationLocked(
        ...
        case .legacy:
            ...
            try authority.switchCurrentGeneration(
                ...
                to: newID,
                pointer: pointer,
                expectedCurrentPointerData: envelope.data
            )
        case .v2:
            let pointer = try makeRestoreCurrentPointer(
                expectedOldID: oldID,
    ...
    private func acceptedGenerationEpoch(
        ...
        try autoreleasepool {
            ...
            case .v5:
                ...
                _ = try semanticExportV5(in: container.mainContext, purpose: .validationOnly)
                let states = try container.mainContext.fetch(
                    FetchDescriptor<WorkspaceMutationStateRow>()
                )
                guard states.count == 1,
            ...
            case .v6:
                ...
                _ = try semanticExportV6(in: container.mainContext, purpose: .validationOnly)
            case .v7:
                ...
                _ = try semanticExportV7(in: container.mainContext, purpose: .validationOnly)
            case .v8:
                ...
                _ = try semanticExportV8(in: container.mainContext, purpose: .validationOnly)
                let states = try container.mainContext.fetch(
                    FetchDescriptor<WorkspaceMutationStateRow>()
                )
                guard states.count == 1,
            ...
            case .v10:
                ...
                _ = try semanticExportV9(in: container.mainContext, purpose: .validationOnly)
            ...
            case .v11:
                ...
                _ = try semanticExportV11(in: container.mainContext, purpose: .validationOnly)
                let states = try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>())
                guard states.count == 1, let state = states.first, state.generationID == id else {
                    throw GenerationLeaseRegistryFailureV1.corruptRegistry
                }
            ...
            case .v12:
                ...
                _ = try semanticExportV12(in: container.mainContext, purpose: .validationOnly)
                let states = try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>())
                guard states.count == 1, let state = states.first, state.generationID == id else {
                    throw GenerationLeaseRegistryFailureV1.corruptRegistry
                }
                ...
                try journal.validateAll()
            case .v13:
                container=try makeV13Container(at:modelURL,migrate:false);_ = try requireV13Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV13(in:container.mainContext, purpose: .validationOnly)
            ...
            case .v14:
                container=try makeV14Container(at:modelURL,migrate:false);_ = try requireV14Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV14(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v15:
                container=try makeV15Container(at:modelURL,migrate:false);_ = try requireV15Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV15(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v16:
                container=try makeV16Container(at:modelURL,migrate:false);_ = try requireV16Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV16(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v17:
                container=try makeV17Container(at:modelURL,migrate:false);_ = try requireV17Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV17(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v18:
                container=try makeV18Container(at:modelURL,migrate:false);_ = try requireV18Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV18(in:container.mainContext, purpose: .validationOnly)
            case .v19:
                container=try makeV19Container(at:modelURL,migrate:false);_ = try requireV19Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV19(in:container.mainContext, purpose: .validationOnly)
            case .v20:
                container=try makeV20Container(at:modelURL,migrate:false);_ = try requireV20Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV20(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v21:
                container=try makeV21Container(at:modelURL,migrate:false);_ = try requireV21Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV21(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v22:
                container=try makeV22Container(at:modelURL,migrate:false);_ = try requireV22Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV22(in:container.mainContext, purpose: .validationOnly)
            case .v23:
                container=try makeV23Container(at:modelURL,migrate:false);_ = try requireV23Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV23(in:container.mainContext, purpose: .validationOnly)
            case .v24:
                container=try makeV24Container(at:modelURL,migrate:false);_ = try requireV24Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV24(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v25:
                container=try makeV25Container(at:modelURL,migrate:false);_ = try requireV25Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV25(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v26:
                container=try makeV26Container(at:modelURL,migrate:false);_ = try requireV26Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV26(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v27:
                container=try makeV27Container(at:modelURL,migrate:false);_ = try requireV27Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV27(in:container.mainContext, purpose: .validationOnly)
            case .v28:
                container=try makeV28Container(at:modelURL,migrate:false);_ = try requireV28Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV28(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v29:
                container=try makeV29Container(at:modelURL,migrate:false);_ = try requireV29Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV29(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v30:
                container=try makeV30Container(at:modelURL,migrate:false);_ = try requireV30Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV30(in:container.mainContext, purpose: .validationOnly)
            case .v31:
                container=try makeV31Container(at:modelURL,migrate:false);_ = try requireV31Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV31(in:container.mainContext, purpose: .validationOnly)
            case .v32:
                container=try makeV32Container(at:modelURL,migrate:false);_ = try requireV32Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV32(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v33:
                container=try makeV33Container(at:modelURL,migrate:false);_ = try requireV33Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV33(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v34:
                container=try makeV34Container(at:modelURL,migrate:false);_ = try requireV34Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV34(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v35:
                container=try makeV35Container(at:modelURL,migrate:false);_ = try requireV35Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV35(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
            case .v36:
                container=try makeV36Container(at:modelURL,migrate:false);_ = try requireV36Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV36(in:container.mainContext, purpose: .validationOnly)
                let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v37:
                 container=try makeV37Container(at:modelURL,migrate:false);_ = try requireV37Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV37(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v38:
                 container=try makeV38Container(at:modelURL,migrate:false);_ = try requireV38Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV38(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v39:
                 container=try makeV39Container(at:modelURL,migrate:false);_ = try requireV39Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV39(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v40:
                 container=try makeV40Container(at:modelURL,migrate:false);_ = try requireV40Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV40(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v41:
                 container=try makeV41Container(at:modelURL,migrate:false);_ = try requireV41Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV41(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v42:
                 container=try makeV42Container(at:modelURL,migrate:false);_ = try requireV42Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV42(in:container.mainContext, purpose: .validationOnly)
             case .v43:
                 container=try makeV43Container(at:modelURL,migrate:false);_ = try requireV43Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV43(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v44:
                 container=try makeV44Container(at:modelURL,migrate:false);_ = try requireV44Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV44(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v45:
                 container=try makeV45Container(at:modelURL,migrate:false);_ = try requireV45Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV45(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v46:
                 container=try makeV46Container(at:modelURL,migrate:false);_ = try requireV46Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV46(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v47:
                 container=try makeV47Container(at:modelURL,migrate:false);_ = try requireV47Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV47(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v48:
                 container=try makeV48Container(at:modelURL,migrate:false);_ = try requireV48Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV48(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v49:
                 container=try makeV49Container(at:modelURL,migrate:false);_ = try requireV49Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV49(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v50:
                 container=try makeV50Container(at:modelURL,migrate:false);_ = try requireV50Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV50(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v51:
                 container=try makeV51Container(at:modelURL,migrate:false);_ = try requireV51Marker(in:container.mainContext,expectedMigrationID:loaded.manifest.migrationID);_ = try semanticExportV51(in:container.mainContext, purpose: .validationOnly)
                 let states=try container.mainContext.fetch(FetchDescriptor<WorkspaceMutationStateRow>());guard states.count==1,let state=states.first,state.generationID==id else{throw GenerationLeaseRegistryFailureV1.corruptRegistry};let identity=try WorkspaceReplicaIdentityV1(workspaceID:WorkspaceID(rawValue:state.workspaceID),replicaID:ReplicaID(rawValue:state.activeReplicaID));let journal=try MutationJournalStoreV1(modelContext:container.mainContext,identity:identity,generationID:id,allowStateBootstrap:false);try journal.validateAll()
             case .v52:
    ...
    private func captureAggregateSourceEvidence(_ journal: StoreAggregateMigrationJournalV1) throws
        ...
        let evidence = try autoreleasepool { () throws -> (String, StoreMigrationValidatedTerminalImagesV1) in
            ...
            return (try aggregateSemanticDigest(in: container.mainContext,
    ...
    private func captureAggregatePriorMutationState(_ journal: StoreAggregateMigrationJournalV1,
        ...
        try autoreleasepool {
            ...
            let context = container.mainContext
            context.autosaveEnabled = false
            guard try aggregateSemanticDigest(in: context, release: journal.currentCandidateRelease) == journal.currentCandidateSemanticSHA256 else {
                throw StoreMigrationFailure.maintenanceRequired(.sourceMismatch)
            }
        ...
        if targetRelease == journal.targetRelease {
            ...
            try withFinalAggregateCandidate(journal, at: root, registry: makeGenerationLeaseRegistry(),
                ...
                try MutationJournalStoreV1.withAggregatePriorProjectionRestored(candidate) { context in
                    guard try aggregateSemanticDigest(in: context,
    ...
    private func resumeAggregateMigration(_ persisted: StoreAggregateMigrationJournalV1,
        ...
        func requireFinal(staging: Bool) throws {
            ...
            guard manifest.schemaVersion == 2, manifest.semanticDigestAlgorithm == .framedLayersV1,
                  manifest.predecessorGenerationID == journal.sourceGenerationID, manifest.migrationID == journal.migrationID,
                  manifest.storeSchemaRelease == journal.targetRelease,
                ...
                if journal.authorizedTargetRelease == nil {
                    if journal.currentCandidateRelease == journal.targetRelease {
                        try registry.withMigrationReservation(expected: journal) {
                            ...
                            let manifest = try StoreGenerationManifestV1(schemaVersion: 2, generationID: journal.targetGenerationID,
                                ...
                                semanticDigestAlgorithm: .framedLayersV1, frozenIdentityDigest: try frozenIdentityDigest(for: root), files: try generationFileDigests(at: root, durable: true))
                            let digest = try store.writeManifest(manifest)
    ...
    private func validateAggregatePublishedTarget(_ journal: StoreAggregateMigrationJournalV1) throws {
        ...
        guard let manifestDigest = journal.targetManifestSHA256 else { throw StoreMigrationFailure.invalidContract }
        let manifest = try StoreMigrationJournalStoreV1(applicationSupportURL: applicationSupportURL)
            .loadManifest(targetGenerationID: journal.targetGenerationID, expectedDigest: manifestDigest)
        guard manifest.schemaVersion == 2, manifest.semanticDigestAlgorithm == .framedLayersV1,
              manifest.storeSchemaRelease == journal.targetRelease,
    ...
    private func prepareFinalAggregateCandidate(_ journal: StoreAggregateMigrationJournalV1, at root: URL,
        ...
        try withFinalAggregateCandidate(journal, at: root, registry: registry, projectionProof: projectionProof) { candidate in
            ...
            return try candidate.withAuthorizedContext { context, _ in
                try aggregateSemanticDigest(in: context, release: journal.targetRelease)
            }
        }
    }
...
/// Validation must still encode every local envelope: its values may reject during
/// Encodable conformance. Its already validated predecessor does not need to be
...
private struct StoreSemanticExportPurposeV1 {
    ...
    static let canonicalBytes = Self(retainsCanonicalBytes: true)
    static let validationOnly = Self(retainsCanonicalBytes: false)
