...
actor IntegrationProjectionCheckpointStoreV1: IntegrationProjectionOperationalStoreV1 {
    ...
    private struct StoredProjection: Codable, Equatable, Sendable {
        ...
        func validate(expectedGenerationID: UUID, expectedWorkspaceID: WorkspaceID,
            ...
            try IntegrationProjectionCheckpointStoreV1.validateEvidenceMetadataEventPage(events)
            ...
            try IntegrationProjectionCheckpointStoreV1.validateSurveySessionEventPage(events)
            try IntegrationProjectionCheckpointStoreV1.validateAssetLocatorEventPage(events)
            try IntegrationProjectionCheckpointStoreV1.validateScheduleEventPage(events)
    ...
    private static func validateEvidenceMetadataEventPage(
        _ events: [IntegrationEventV1]
    ) throws {
        let grouped = Dictionary(grouping: events, by: \.sourceReceiptID)
        for page in grouped.values {
            let subjects = page.map(\.subject)
            let kinds = Set(subjects.map(\.kind))
            let present = kinds.intersection(IntegrationEventProjectionV1.evidenceMetadataKinds)
            guard !present.isEmpty else { continue }
            guard page.count == 2,
                  let first = page.first,
                  Set(subjects).count == 2,
                  kinds == IntegrationEventProjectionV1.evidenceMetadataKinds,
                  page.map(\.order.payloadOrdinal).sorted() == [0, 1],
                  page.allSatisfy({ event in
                      event.sourceMutationID == first.sourceMutationID
                          && event.sourceReceiptID == first.sourceReceiptID
                          && event.sourceReceiptSHA256 == first.sourceReceiptSHA256
                          && event.sourceWorkspaceRevision == first.sourceWorkspaceRevision
                          && event.order.sourceWorkspaceRevision == first.order.sourceWorkspaceRevision
                          && event.order.sourceReplicaID == first.order.sourceReplicaID
                          && event.order.sourceLocalSequence == first.order.sourceLocalSequence
                  }) else {
    ...
    private static func validateSurveySessionEventPage(_ events:[IntegrationEventV1])throws{let grouped=Dictionary(grouping:events,by:\.sourceReceiptID);for page in grouped.values{let subjects=page.map(\.subject),present=Set(subjects.map(\.kind)).intersection(IntegrationEventProjectionV1.surveySessionKinds);guard present.isEmpty || ([1,2].contains(page.count)&&Set(subjects).count==subjects.count&&subjects.allSatisfy{IntegrationEventProjectionV1.surveySessionKinds.contains($0.kind)})else{throw IntegrationEventFailureV1.divergentEvent}}}
    private static func validateAssetLocatorEventPage(_ events:[IntegrationEventV1])throws{let grouped=Dictionary(grouping:events,by:\.sourceReceiptID);for page in grouped.values{let subjects=page.map(\.subject),present=Set(subjects.map(\.kind)).intersection(IntegrationEventProjectionV1.assetLocatorKinds);guard present.isEmpty || ([2,3].contains(page.count)&&Set(subjects).count==subjects.count&&subjects.contains(where:{$0.kind == .assetLocator})&&subjects.contains(where:{$0.kind == .locatorBindingReceipt})&&subjects.allSatisfy{IntegrationEventProjectionV1.assetLocatorKinds.contains($0.kind)})else{throw IntegrationEventFailureV1.divergentEvent}}}
    private static func validateScheduleEventPage(_ events:[IntegrationEventV1])throws{let grouped=Dictionary(grouping:events,by:\.sourceReceiptID);for page in grouped.values{let subjects=page.map(\.subject),present=Set(subjects.map(\.kind)).intersection(IntegrationEventProjectionV1.scheduleKinds);guard present.isEmpty || (!page.isEmpty&&page.count<=ScheduleLimitsV1.maximumGeneratedOccurrences&&Set(subjects).count==subjects.count&&subjects.allSatisfy{IntegrationEventProjectionV1.scheduleKinds.contains($0.kind)})else{throw IntegrationEventFailureV1.divergentEvent}}}
    ...
    func recordDerivedConsumerEffects(events: [IntegrationEventV1], consumerID: String,
        ...
        try requireScope(consumerID: consumerID, workspaceID: workspaceID)
        try Self.validateEvidenceMetadataEventPage(events)
...
enum C32AssistanceCompatibility_Replication_IntegrationProjectionCheckpointStoreV1 {
