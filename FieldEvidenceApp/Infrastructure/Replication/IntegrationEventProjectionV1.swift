...
struct IntegrationEventProjectionV1: Sendable {
    ...
    func project(workspaceID: WorkspaceID, acceptedReceipts: [MutationReceiptV1]) throws -> [IntegrationEventV1] {
        ...
        for receipt in orderedReceipts {
            ...
            try Self.validateEvidenceMetadataReceiptShape(receipt)
    ...
    static func validatePackagePromotionReceiptShape(_ receipt:MutationReceiptV1)throws {
        let kinds=try receipt.postImages.map{try $0.identity.kind}
    ...
    func validatePackagePromotionReplay(_ receipts:[MutationReceiptV1])throws{let hasPromotion=try receipts.contains{try !$0.postImages.map{try $0.identity.kind}.filter{Self.packagePromotionKinds.contains($0)}.isEmpty};if hasPromotion{try PackageEvolutionIntegrationContractV1.validate(registry:registry)};try receipts.forEach{try Self.validatePackagePromotionReceiptShape($0)}}
    ...
    static let evidenceMetadataKinds: Set<WorkspaceEntityKindV1> = [
        .evidenceAssociationEvent,
        .evidenceSequenceRevision,
    ]
    static func validateEvidenceMetadataReceiptShape(_ receipt: MutationReceiptV1) throws {
        let identities = try receipt.postImages.map { try $0.identity }
        let present = Set(identities.map(\.kind)).intersection(evidenceMetadataKinds)
        guard !present.isEmpty else { return }
        let concurrency = try receipt.postImages.map { try $0.concurrencyIdentity }
        let expected = Dictionary(uniqueKeysWithValues: receipt.expectedRevision.entityRevisions.map {
            ($0.identity, $0.revision)
        })
        let resulting = Dictionary(uniqueKeysWithValues: receipt.resultingRevision.entityRevisions.map {
            ($0.identity, $0.revision)
        })
        guard receipt.postImages.count == 2,
              Set(identities.map(\.kind)) == evidenceMetadataKinds,
              Set(identities).count == 2,
              identities == concurrency,
              Set(expected.keys) == Set(concurrency),
              Set(resulting.keys) == Set(identities) else {
            throw IntegrationEventFailureV1.divergentEvent
        }
        for image in receipt.postImages {
            let identity = try image.identity
            guard let priorRevision = expected[identity],
                  priorRevision < UInt64.max,
                  image.revision == priorRevision + 1,
                  resulting[identity] == image.revision else {
                throw IntegrationEventFailureV1.divergentEvent
            }
        }
    }
    func validateEvidenceMetadataReplay(_ receipts: [MutationReceiptV1]) throws {
        var found = false
        for receipt in receipts {
            for image in receipt.postImages {
                if Self.evidenceMetadataKinds.contains((try image.identity).kind) {
                    found = true
                }
            }
        }
        if found {
            try EvidenceMetadataIntegrationContractV1.validate(registry: registry)
        }
        try receipts.forEach { try Self.validateEvidenceMetadataReceiptShape($0) }
    }
    ...
    static let lightingKinds:Set<WorkspaceEntityKindV1>=[.lightingSystem,.lightingObservation,.lightingIssue,.lightingMeasurementPlan,.lightingClaimState]
    static func validateLightingReceiptShape(_ receipt:MutationReceiptV1)throws{let identities=try receipt.postImages.map{try $0.identity};let present=Set(identities.map(\.kind)).intersection(lightingKinds);guard !present.isEmpty else{return};guard identities.count==1,let image=receipt.postImages.first,lightingKinds.contains(try image.identity.kind)else{throw IntegrationEventFailureV1.divergentEvent};let concurrency=try image.concurrencyIdentity;guard let expected=receipt.expectedRevision.entityRevisions.first(where:{$0.identity==concurrency})?.revision,expected<UInt64.max,image.revision==expected+1 else{throw IntegrationEventFailureV1.divergentEvent}}
    func validateLightingReplay(_ receipts:[MutationReceiptV1])throws{for receipt in receipts{try Self.validateLightingReceiptShape(receipt)}}
    static let temporalEvidenceKinds = C33TemporalEvidenceIntegrationEventBoundaryV1.projectedKinds
    static func validateTemporalEvidenceReceiptShape(_ receipt:MutationReceiptV1)throws{let identities=try receipt.postImages.map{try $0.identity};let present=Set(identities.map(\.kind)).intersection(temporalEvidenceKinds);guard !present.isEmpty else{return};guard Set(identities).count==identities.count,identities.allSatisfy({temporalEvidenceKinds.contains($0.kind)}),identities.count<=MutationReceiptV1.maximumPostImageCount else{throw IntegrationEventFailureV1.divergentEvent};for image in receipt.postImages{let concurrency=try image.concurrencyIdentity;guard let expected=receipt.expectedRevision.entityRevisions.first(where:{$0.identity==concurrency})?.revision,expected<UInt64.max,image.revision==expected+1 else{throw IntegrationEventFailureV1.divergentEvent}}}
...
struct C31LightingIntegrationProjectionMetadataV1: Codable, Equatable, Sendable {
    let consumer: String
    let sourceProjection: String
    let derivedOnly: Bool
    let bytesExcluded: Bool
    let actorsExcluded: Bool
    let privateLocatorsExcluded: Bool

    init(consumer: String) throws {
        guard !consumer.isEmpty else { throw IntegrationEventFailureV1.invalidValue }
        self.consumer = consumer
        sourceProjection = "C31_LIGHTING_REPORT_PROJECTION_V1"
        derivedOnly = true
        bytesExcluded = true
        actorsExcluded = true
        privateLocatorsExcluded = true
    }

    func validate() throws {
        guard !consumer.isEmpty,
              sourceProjection == "C31_LIGHTING_REPORT_PROJECTION_V1",
              derivedOnly, bytesExcluded, actorsExcluded,
              privateLocatorsExcluded else {
            throw IntegrationEventFailureV1.invalidValue
        }
    }
}
...
enum C32AssistanceCompatibility_Replication_IntegrationEventProjectionV1 {
