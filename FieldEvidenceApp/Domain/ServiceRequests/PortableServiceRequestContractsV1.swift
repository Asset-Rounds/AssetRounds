...
struct ServiceRequestDuplicateCandidateV1:Codable,Equatable,Hashable,Comparable,Sendable{
    ...
    func validate()throws{try record.validate();try ServiceRequestLimitsV1.id(sharedSiteID);if let sharedAssetID{try ServiceRequestLimitsV1.id(sharedAssetID)};try reasons.forEach{try $0.validate()};guard !reasons.isEmpty,reasons==reasons.sorted(),Set(reasons).count==reasons.count else{throw ServiceRequestFailureV1.invalidValue}}
...
struct ServiceRequestDuplicateProjectionV1:Codable,Equatable,Hashable,Sendable{
    ...
    func validate()throws{try ServiceRequestLimitsV1.digest(basisRequestSHA256);try ServiceRequestLimitsV1.digest(ruleReleaseSHA256);try ServiceRequestLimitsV1.digest(projectionSHA256);try candidates.forEach{try $0.validate()};guard suggestionOnly,candidates==candidates.sorted(),candidates.count<=ServiceRequestLimitsV1.maximumDuplicateCandidates,Set(candidates.map{$0.record.recordID}).count==candidates.count,Set(candidates.map(\.sharedSiteID)).count<=1,projectionSHA256==(try ServiceRequestCanonicalCodecV1.sha256(basis))else{throw ServiceRequestFailureV1.limitExceeded}}
