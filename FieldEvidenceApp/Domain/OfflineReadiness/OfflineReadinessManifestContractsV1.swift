...
struct OfflineReadinessAccessObservationV1: Codable, Equatable, Hashable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        try OfflineReadinessClosedCodingV1.exact(decoder, CodingKeys.allCases.map(\.rawValue))
        let c = try decoder.container(keyedBy: CodingKeys.self)
        self.init(protectedDataAvailable: try c.decode(Bool.self, forKey: .protectedDataAvailable))
    }
}
...
struct OfflineReadinessManifestV1: Codable, Equatable, Sendable {
    ...
    init(session: RoundSessionReferenceV1, expectedPackage: RoundPackageReleaseReferenceV1, observedPackage: RoundPackageReleaseReferenceV1?, selectedAssets: [RoundAssetSelectionV1], observedAssetIDs: [UUID], guidanceReferenceIDs: [String], availableGuidanceReferenceIDs: [String], contentRequirements: [OfflineReadinessContentRequirementV1], contentObservations: [OfflineReadinessContentObservationV1], expectedFieldReferences: [OfflineReadinessFieldReferenceRequirementV1], referenceObservations: [OfflineReadinessReferenceObservationV1], requiredBytes: Int64?, availableBytes: Int64?, storage: OfflineReadinessStorageObservationV1, protectedDataAvailable: Bool, checkedAt: Date, timeZoneIdentifier: String, clockState: OfflineReadinessClockStateV1, sourceSnapshotSHA256: String, priorSourceSnapshotSHA256: String?, requirements: [OfflineReadinessRequirementV1], status: OfflineReadinessStatusV1) throws {
        ...
        guard selectedAssets.count <= OfflineReadinessManifestLimitsV1.maximumAssets, selectedAssets == selectedAssets.sorted(by: { $0.assetID.uuidString < $1.assetID.uuidString }), Set(selectedAssets.map(\.assetID)).count == selectedAssets.count, observedAssetIDs.count <= OfflineReadinessManifestLimitsV1.maximumAssets, observedAssetIDs == observedAssetIDs.sorted(by: { $0.uuidString < $1.uuidString }), Set(observedAssetIDs).count == observedAssetIDs.count,
              ...
              contentRequirements.count <= OfflineReadinessManifestLimitsV1.maximumContentRequirements, contentRequirements == contentRequirements.sorted(by: { $0.reference.id < $1.reference.id }), Set(contentRequirements.map { $0.reference.id }).count == contentRequirements.count, contentObservations.count == contentRequirements.count, contentObservations == contentObservations.sorted(by: { $0.contentID < $1.contentID }), Set(contentObservations.map(\.contentID)) == Set(contentRequirements.map { $0.reference.contentID }),
              expectedFieldReferences.count <= OfflineReadinessManifestLimitsV1.maximumFieldReferences, expectedFieldReferences == expectedFieldReferences.sorted(by: { $0.releaseID.uuidString < $1.releaseID.uuidString }), Set(expectedFieldReferences.map(\.releaseID)).count == expectedFieldReferences.count,
              referenceObservations.count <= OfflineReadinessManifestLimitsV1.maximumFieldReferences, referenceObservations == referenceObservations.sorted(by: { $0.releaseID.uuidString < $1.releaseID.uuidString }), Set(referenceObservations.map(\.releaseID)).count == referenceObservations.count,
              ...
              requirements.count <= OfflineReadinessManifestLimitsV1.maximumRequirementRows, requirements == requirements.sorted(by: { $0.requirementID < $1.requirementID }), Set(requirements.map(\.requirementID)).count == requirements.count else { throw OfflineReadinessManifestFailureV1.invalidValue }
    ...
    init(from decoder: Decoder) throws {
        try OfflineReadinessClosedCodingV1.exact(decoder, CodingKeys.allCases.map(\.rawValue)); let c = try decoder.container(keyedBy: CodingKeys.self)
        guard try c.decode(Int.self, forKey: .schemaVersion) == Self.schemaVersion, try c.decode(String.self, forKey: .schema) == OfflineReadinessManifestLifecycleV1.schema, try c.decode(String.self, forKey: .persistenceMode) == OfflineReadinessManifestLifecycleV1.persistenceMode else { throw OfflineReadinessManifestFailureV1.invalidValue }
        ...
        guard value.manifestSHA256 == (try c.decode(String.self, forKey: .manifestSHA256)) else { throw OfflineReadinessManifestFailureV1.digestMismatch }; self = value
    }
}
