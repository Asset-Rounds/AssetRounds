...
struct FactCaptureV1:Codable,Equatable,Sendable{
    ...
    func validateSuccessor(of prior:[Self],session:SurveySessionV1,definition:SurveyDefinitionReleaseV1)throws{try validate(session:session,definition:definition);try prior.forEach{try $0.validate(session:session,definition:definition)};let refs=try prior.map(\.reference).sorted{$0.captureID.uuidString<$1.captureID.uuidString};guard predecessors==refs,!prior.isEmpty,prior.allSatisfy({$0.factID==factID&&$0.repeatCoordinates==repeatCoordinates}),revision==(prior.map(\.revision).max() ?? 0)+1,action == .resolveConflict ? prior.count>1:prior.count==1 else{throw SurveySessionFailureV1.unresolvedConflict}}
    ...
    func validateIntrinsic()throws{try definitionRelease.validate();try repeatCoordinates.forEach{try $0.validate()};try predecessors.forEach{try $0.validate()};try value?.validate();try capturedBy.validate();let coordinateKeys=repeatCoordinates.map(\.stableKey),predecessorIDs=predecessors.map(\.captureID),evidenceIDs=evidence.map(\.contentID);guard schemaVersion==Self.schemaVersion,captureID != UUID.zero,sessionID != UUID.zero,SurveyDefinitionLimitsV1.token(factID),coordinateKeys==coordinateKeys.sorted(),Set(coordinateKeys).count==coordinateKeys.count,Set(predecessorIDs).count==predecessorIDs.count,Set(evidenceIDs).count==evidenceIDs.count,capturedBy.workspaceID==workspaceID,capturedAt.isFiniteSurveyDate,revision>0,(action == .record)==predecessors.isEmpty,(action == .retract)==(value==nil),action != .retract || evidence.isEmpty,action != .resolveConflict || predecessors.count>1,captureSHA256==(try WorkspaceMutationCanonicalV1.sha256(basis))else{throw SurveySessionFailureV1.invalidDigest}}
