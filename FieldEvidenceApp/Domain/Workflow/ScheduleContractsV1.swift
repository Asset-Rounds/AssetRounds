...
enum ScheduleRecurrenceV1: Codable, Equatable, Hashable, Sendable {
    ...
    func validate() throws { switch self { case .fixedCalendar(let value): try value.validate(); case .completionRelative(let value): try value.validate() } }
}
...
struct ScheduleDefinitionReleaseV1: Codable, Equatable, Sendable {
    ...
    func validate() throws {
        ...
        guard schemaVersion==Self.schemaVersion,(1...ScheduleLimitsV1.maximumHorizonDays).contains(generationHorizonDays),
              ...
              endsAtUTC.map({$0>=startsAtUTC}) ?? true,workDefinition.definitionWorkspaceID==workspaceID,authoredBy.workspaceID==workspaceID,assignee.map({$0.workspaceID==workspaceID&&$0.responsibility == .assignedTo}) ?? true,
    ...
    func validateSuccessor(of old: Self) throws {
        ...
        let identityChanged = recurrence != old.recurrence || timeBasis != old.timeBasis ||
