...
enum MutationPostImageV1: Codable, Equatable, Sendable {
    ...
    case evidenceQuality(id:UUID,kind:WorkspaceEntityKindV1,concurrencyIdentity:WorkspaceEntityIdentityV1,revision:UInt64,semanticSHA256:String)
    ...
    case exceptionCalendarRelease(id:UUID,concurrencyIdentity:WorkspaceEntityIdentityV1,revision:UInt64,semanticSHA256:String)
    case scheduleOverrideEvent(id:UUID,concurrencyIdentity:WorkspaceEntityIdentityV1,revision:UInt64,semanticSHA256:String)
    ...
    case myDayPlan(id:UUID,concurrencyIdentity:WorkspaceEntityIdentityV1,revision:UInt64,semanticSHA256:String)
    case myDayCarryoverReceipt(id:UUID,concurrencyIdentity:WorkspaceEntityIdentityV1,revision:UInt64,semanticSHA256:String)
    ...
    case workResourceEntry(id:UUID,concurrencyIdentity:WorkspaceEntityIdentityV1,revision:UInt64,semanticSHA256:String)
    ...
    var identity: WorkspaceEntityIdentityV1 {
        get throws {
            ...
            case let .evidenceQuality(id,kind,_,_,_):return try .init(kind:kind,id:id)
            ...
            case let .exceptionCalendarRelease(id,_,_,_):return try .init(kind:.exceptionCalendarRelease,id:id)
            case let .scheduleOverrideEvent(id,_,_,_):return try .init(kind:.scheduleOverrideEvent,id:id)
            ...
            case let .myDayPlan(id,_,_,_):return try .init(kind:.myDayPlan,id:id)
            case let .myDayCarryoverReceipt(id,_,_,_):return try .init(kind:.myDayCarryoverReceipt,id:id)
            ...
            case let .workResourceEntry(id,_,_,_):return try .init(kind:.workResourceEntry,id:id)
    ...
    var concurrencyIdentity: WorkspaceEntityIdentityV1 {
        get throws {
            ...
            case let .evidenceQuality(_,kind,v,_,_):guard v.kind == kind else{throw WorkspaceMutationFailureV1.invalidReceipt};return v
            ...
            case let .exceptionCalendarRelease(_,v,_,_):guard v.kind == .exceptionCalendarRelease else{throw WorkspaceMutationFailureV1.invalidReceipt};return v
            case let .scheduleOverrideEvent(_,v,_,_):guard v.kind == .scheduleOverrideEvent else{throw WorkspaceMutationFailureV1.invalidReceipt};return v
            ...
            case let .myDayPlan(_,v,_,_):guard v.kind == .myDayPlan else{throw WorkspaceMutationFailureV1.invalidReceipt};return v
            case let .myDayCarryoverReceipt(_,v,_,_):guard v.kind == .myDayCarryoverReceipt else{throw WorkspaceMutationFailureV1.invalidReceipt};return v
...
extension EvidenceMetadataMutationV1 {
    var mutationPostImages: [MutationPostImageV1] {
        get throws {
            try validate()
            let associationIdentity = try Self.associationEntityIdentity(
            ...
            let sequenceIdentity = try WorkspaceEntityIdentityV1(
                ...
                id: sequenceSuccessor.sequenceID
            )
            guard let associationRevision = UInt64(exactly: associationEvent.resultingEvidenceRevision) else {
                throw WorkspaceMutationFailureV1.invalidCommand
            }
            let values: [MutationPostImageV1] = [
                .evidenceAssociationEvent(
                    id: associationIdentity.id,
                    concurrencyIdentity: associationIdentity,
                    revision: associationRevision,
                    semanticSHA256: try associationEvent.associationSHA256
                ),
                .evidenceSequenceRevision(
                    id: sequenceIdentity.id,
                    concurrencyIdentity: sequenceIdentity,
                    revision: sequenceSuccessor.revision,
                    semanticSHA256: sequenceSuccessor.sequenceSHA256
                ),
            ]
            let ordered = try values.sorted { try $0.identity.stableKey < $1.identity.stableKey }
            guard try ordered.map({ try $0.identity }) == affectedIdentities,
                  try ordered.map({ try $0.concurrencyIdentity }) == concurrencyIdentities else {
                throw WorkspaceMutationFailureV1.invalidCommand
            }
            return ordered
        }
...
extension ShopReportProfileMutationV1 {
    var mutationPostImage: MutationPostImageV1 {
        get throws {
            try .shopReportProfile(
                id: profile.profileID,
                concurrencyIdentity: concurrencyIdentity,
                revision: profile.revision,
                semanticSHA256: profile.profileSHA256
            )
        }
    }
}

struct ShopReportProfileMutationReceiptV1: Codable, Equatable, Sendable {
    let mutationSHA256: String
    let mutationReceiptSHA256: String
    let profileFrontier: ShopReportProfileReferenceV1
    let mutation: ShopReportProfileMutationV1
    let mutationReceipt: MutationReceiptV1

    init(mutation: ShopReportProfileMutationV1, mutationReceipt: MutationReceiptV1) throws {
        ...
        guard mutationReceipt.mutationID == mutation.mutationID,
              ...
              mutationReceipt.commandBodySHA256 == (try WorkspaceMutationCanonicalV1.sha256(
                WorkspaceCommandV1.applyShopReportProfile(mutation)
              )),
              ...
              mutationReceipt.resultingRevision.entityRevisions.first(where: { $0.identity == affected })?.revision == mutation.profile.revision,
              mutationReceipt.postImages == [image] else {
            throw WorkspaceMutationFailureV1.invalidReceipt
        }
        mutationSHA256 = try ShopReportProfileCanonicalCodecV1.sha256(mutation)
        mutationReceiptSHA256 = try mutationReceipt.canonicalSHA256()
        profileFrontier = try mutation.profile.reference
        self.mutation = mutation
        self.mutationReceipt = mutationReceipt
    }

    func validate() throws {
        guard self == (try Self(mutation: mutation, mutationReceipt: mutationReceipt)) else {
            throw WorkspaceMutationFailureV1.invalidReceipt
        }
    }

    private enum CodingKeys:String,CodingKey,CaseIterable {
        case mutationSHA256, mutationReceiptSHA256, profileFrontier, mutation, mutationReceipt
    }
    ...
    init(from decoder:Decoder)throws {
        ...
        let storedMutationSHA256 = try values.decode(String.self,forKey:.mutationSHA256)
        let storedReceiptSHA256 = try values.decode(String.self,forKey:.mutationReceiptSHA256)
        let storedFrontier = try values.decode(ShopReportProfileReferenceV1.self,forKey:.profileFrontier)
        let mutation = try values.decode(ShopReportProfileMutationV1.self,forKey:.mutation)
        let receipt = try values.decode(MutationReceiptV1.self,forKey:.mutationReceipt)
        try self.init(mutation:mutation,mutationReceipt:receipt)
        guard mutationSHA256 == storedMutationSHA256,
              mutationReceiptSHA256 == storedReceiptSHA256,
...
struct PortableReviewMutationReceiptV1: Codable, Equatable, Sendable {
    let mutationSHA256: String
    let mutationReceipt: MutationReceiptV1
    let importReceipt: ExternalReviewImportReceiptV1

    init(mutation: PortableReviewMutationV1, mutationReceipt: MutationReceiptV1) throws {
        try mutation.validate(); try mutationReceipt.validate(); try mutation.importReceipt.validate()
        ...
        guard mutationReceipt.mutationID == mutation.mutationID,
              mutationReceipt.identity.workspaceID == mutation.workspaceID,
              mutationReceipt.expectedRevision.workspaceRevision == mutation.plan.basisWorkspaceRevision,
              mutationReceipt.commandBodySHA256 == (try WorkspaceMutationCanonicalV1.sha256(WorkspaceCommandV1.applyPortableReview(mutation))),
              mutationReceipt.postImages == images,
              try mutation.concurrencyIdentities.allSatisfy({ expected[$0] == (try mutation.expectedRevision(for: $0)) }),
              try images.allSatisfy({ resulting[try $0.identity] == $0.revision }),
              mutation.importReceipt.appliedWorkspaceRevision == mutationReceipt.resultingRevision.workspaceRevision else {
            throw WorkspaceMutationFailureV1.invalidReceipt
        }
        mutationSHA256 = try mutation.canonicalSHA256()
        self.mutationReceipt = mutationReceipt
        importReceipt = mutation.importReceipt
    }
}
...
struct EvidenceMetadataMutationReceiptV1: Codable, Equatable, Sendable {
    let mutationSHA256: String
    let mutationReceipt: MutationReceiptV1
    let affectedIdentities: [WorkspaceEntityIdentityV1]
    let concurrencyIdentities: [WorkspaceEntityIdentityV1]
    ...
    init(
        ...
        let affected = try mutation.affectedIdentities
        let concurrency = try mutation.concurrencyIdentities
        let imageIdentities = try images.map { try $0.identity }
        let imageConcurrency = try images.map { try $0.concurrencyIdentity }
        let expected = Dictionary(uniqueKeysWithValues: mutationReceipt.expectedRevision.entityRevisions.map {
            ($0.identity, $0.revision)
        })
        let resulting = Dictionary(uniqueKeysWithValues: mutationReceipt.resultingRevision.entityRevisions.map {
            ($0.identity, $0.revision)
        })
        let evidenceKinds: Set<WorkspaceEntityKindV1> = [
            .evidenceAssociationEvent,
            .evidenceSequenceRevision,
        ]
        guard images.count == 2,
              Set(imageIdentities.map(\.kind)) == evidenceKinds,
              affected == imageIdentities,
              concurrency == imageConcurrency,
              Set(expected.keys) == Set(concurrency),
              Set(resulting.keys) == Set(affected),
              mutationReceipt.mutationID == mutation.mutationID,
              mutationReceipt.identity.workspaceID == mutation.workspaceID,
              mutationReceipt.commandBodySHA256 == (try WorkspaceMutationCanonicalV1.sha256(
                  WorkspaceCommandV1.applyEvidenceMetadata(mutation)
              )),
              try concurrency.allSatisfy({
                  expected[$0] == (try mutation.expectedRevision(for: $0))
              }),
              try images.allSatisfy({
                  resulting[try $0.identity] == $0.revision
              }),
              mutationReceipt.postImages == images else {
            throw WorkspaceMutationFailureV1.invalidReceipt
        }
        mutationSHA256 = try mutation.canonicalSHA256()
        self.mutationReceipt = mutationReceipt
        affectedIdentities = affected
        concurrencyIdentities = concurrency
        try validate()
    }

    func validate() throws {
        try mutationReceipt.validate()
        let images = mutationReceipt.postImages
        let affected = try images.map { try $0.identity }
        let concurrency = try images.map { try $0.concurrencyIdentity }
        let expected = Dictionary(uniqueKeysWithValues: mutationReceipt.expectedRevision.entityRevisions.map {
            ($0.identity, $0.revision)
        })
        let resulting = Dictionary(uniqueKeysWithValues: mutationReceipt.resultingRevision.entityRevisions.map {
        ...
        guard MutationEnvelopeV1.isSHA256(mutationSHA256),
              images.count == 2,
              Set(affected.map(\.kind)) == evidenceKinds,
              affectedIdentities == affected,
              concurrencyIdentities == concurrency,
              affected == concurrency,
              Set(affected).count == 2,
              Set(concurrency).count == 2,
              Set(expected.keys) == Set(concurrency),
              Set(resulting.keys) == Set(affected),
              try images.allSatisfy({ image in
                  let identity = try image.identity
                  guard let prior = expected[identity], prior < UInt64.max else { return false }
                  return image.revision == prior + 1 && resulting[identity] == image.revision
              }) else {
            throw WorkspaceMutationFailureV1.invalidReceipt
        }
    }
...
struct MyDayWorkspaceMutationReceiptV1: Codable, Equatable, Sendable {
    ...
    init(mutation: MyDayMutationV1, mutationReceipt: MutationReceiptV1) throws {
        try mutation.validate(); try mutationReceipt.validate()
        let affected = try mutation.affectedIdentities
        let concurrency = try mutation.concurrencyIdentities
        let images = try mutation.mutationPostImages
        ...
        guard mutationReceipt.mutationID == mutation.mutationID,
              mutationReceipt.identity.workspaceID == mutation.workspaceID,
              mutationReceipt.commandBodySHA256 == (try WorkspaceMutationCanonicalV1.sha256(WorkspaceCommandV1.applyMyDay(mutation))),
              mutationReceipt.postImages == images,
              try concurrency.allSatisfy({ expected[$0] == (try mutation.expectedRevision(for: $0)) }),
              try images.allSatisfy({ resulting[try $0.identity] == $0.revision }),
              affected == (try images.map { try $0.identity }) else {
            throw WorkspaceMutationFailureV1.invalidReceipt
        }
        mutationSHA256 = try WorkspaceMutationCanonicalV1.sha256(mutation)
        self.mutationReceipt = mutationReceipt
...
extension LightingDayInventoryWriteOperationV1 {
    var mutationPostImage: MutationPostImageV1 {
        get throws {
            let value = workflow
            return .lightingDayInventoryWorkflow(
                id: value.recordID,
...
struct OperationalContactMutationReceiptV1:Codable,Equatable,Sendable{
    let mutationSHA256:String;let mutationReceipt:MutationReceiptV1;let affectedIdentities:[WorkspaceEntityIdentityV1];let concurrencyIdentities:[WorkspaceEntityIdentityV1]
    init(mutation:OperationalContactMutationV1,mutationReceipt:MutationReceiptV1)throws{
        try mutation.validate();try mutationReceipt.validate();let affected=try mutation.affectedIdentities,concurrency=try mutation.concurrencyIdentities,images=try mutation.mutationPostImages
...
struct PartyContactSiteRoleImportMutationReceiptV1: Codable, Equatable, Sendable {
    let mutationSHA256: String
    let mutationReceipt: MutationReceiptV1
    let affectedIdentities: [WorkspaceEntityIdentityV1]
    let concurrencyIdentities: [WorkspaceEntityIdentityV1]
    ...
    init(
        ...
        let concurrency = try mutation.concurrencyIdentities
        let images = try mutation.mutationPostImages
        guard Set(mutationReceipt.expectedRevision.entityRevisions.map(\.identity)).count
                == mutationReceipt.expectedRevision.entityRevisions.count,
              Set(mutationReceipt.resultingRevision.entityRevisions.map(\.identity)).count
                == mutationReceipt.resultingRevision.entityRevisions.count else {
            throw WorkspaceMutationFailureV1.invalidReceipt
        }
        let expected = Dictionary(uniqueKeysWithValues: mutationReceipt.expectedRevision.entityRevisions.map {
            ($0.identity, $0.revision)
        ...
        guard mutationReceipt.mutationID == mutation.mutationID,
              mutationReceipt.identity.workspaceID == mutation.workspaceID,
              mutationReceipt.commandBodySHA256 == (try WorkspaceMutationCanonicalV1.sha256(
                  WorkspaceCommandV1.applyPartyContactSiteRoleImport(mutation)
              )),
              mutationReceipt.postImages == images,
              try concurrency.allSatisfy({ expected[$0] == (try mutation.expectedRevision(for: $0)) }),
              try images.allSatisfy({ resulting[try $0.identity] == $0.revision }),
...
struct WorkResourceMutationReceiptV1: Codable, Equatable, Sendable {
    let mutationSHA256: String
    let mutationReceipt: MutationReceiptV1
    let affectedIdentity: WorkspaceEntityIdentityV1

    init(mutation: WorkResourceMutationV1, mutationReceipt: MutationReceiptV1) throws {
        try mutation.validate(); try mutationReceipt.validate()
        let affected = try mutation.affectedIdentity
        let concurrency = try mutation.concurrencyIdentity
        let image = try mutation.mutationPostImage
        guard mutationReceipt.mutationID == mutation.mutationID,
              mutationReceipt.identity.workspaceID == mutation.workspaceID,
              mutationReceipt.commandBodySHA256 == (try WorkspaceMutationCanonicalV1.sha256(WorkspaceCommandV1.applyWorkResource(mutation))),
              mutationReceipt.postImages == [image],
              mutationReceipt.expectedRevision.entityRevisions.first(where: { $0.identity == concurrency })?.revision == mutation.postImage.expectedRevision,
              mutationReceipt.resultingRevision.entityRevisions.first(where: { $0.identity == affected })?.revision == mutation.postImage.revision else {
            throw WorkspaceMutationFailureV1.invalidReceipt
        }
        mutationSHA256 = try mutation.canonicalSHA256()
        self.mutationReceipt = mutationReceipt
...
struct ServiceRequestMutationReceiptV1: Codable, Equatable, Sendable {
    let mutationSHA256: String
    let mutationReceipt: MutationReceiptV1
    let postImages: [MutationPostImageV1]

    init(mutation: ServiceRequestMutationV1, mutationReceipt: MutationReceiptV1) throws {
        ...
        guard mutationReceipt.mutationID == mutation.mutationID,
              mutationReceipt.identity.workspaceID == mutation.workspaceID,
              mutationReceipt.commandBodySHA256 == (try WorkspaceMutationCanonicalV1.sha256(WorkspaceCommandV1.applyServiceRequest(mutation))),
              mutationReceipt.postImages == images,
              try mutation.concurrencyIdentities.allSatisfy({ expected[$0] == (try mutation.expectedRevision(for: $0)) }),
              try images.allSatisfy({ resulting[try $0.identity] == $0.revision }) else {
            throw WorkspaceMutationFailureV1.invalidReceipt
