...
enum FieldDraftMutationPayloadV1:Codable,Equatable,Sendable{
    ...
    case resolveConflict(ReviewedDraftConflictResolutionV1)
    case publishReadyStage(FieldDraftStagePublicationBundleV1)
    var workspaceID:WorkspaceID{switch self{case let .createCheckpoint(v),let .reviseCheckpoint(v):v.workspaceID;case let .appendStagingItem(v),let .reviseStagingItem(v):v.workspaceID;case let .appendCommitSaga(v),let .advanceCommitSaga(v):v.workspaceID;case let .appendContentReservation(v),let .reviseContentReservation(v):v.workspaceID;case let .applyCommitTerminal(v,_):v.workspaceID;case let .applyDiscardTerminal(v):v.workspaceID;case let .resolveConflict(v):v.successorCheckpoint.workspaceID;case let .publishReadyStage(v):v.workspaceID}}
    var mutationID:MutationIDV1{switch self{case let .createCheckpoint(v),let .reviseCheckpoint(v):v.mutationID;case let .appendStagingItem(v),let .reviseStagingItem(v):v.mutationID;case let .appendCommitSaga(v),let .advanceCommitSaga(v):v.mutationID;case let .appendContentReservation(v),let .reviseContentReservation(v):v.mutationID;case let .applyCommitTerminal(v,_):v.mutationID;case let .applyDiscardTerminal(v):v.mutationID;case let .resolveConflict(v):v.successorCheckpoint.mutationID;case let .publishReadyStage(v):v.mutationID}}
    var revision:UInt64{switch self{case let .createCheckpoint(v),let .reviseCheckpoint(v):v.draftRevision;case let .appendStagingItem(v),let .reviseStagingItem(v):v.revision;case let .appendCommitSaga(v),let .advanceCommitSaga(v):v.revision;case let .appendContentReservation(v),let .reviseContentReservation(v):v.revision;case let .applyCommitTerminal(v,_):v.committedCheckpoint.draftRevision;case let .applyDiscardTerminal(v):v.discardedCheckpoint.draftRevision;case let .resolveConflict(v):v.successorCheckpoint.draftRevision;case let .publishReadyStage(v):v.successorCheckpoint.draftRevision}}
    var affectedIdentities:[WorkspaceEntityIdentityV1]{get throws{let values:[WorkspaceEntityIdentityV1];switch self{case let .createCheckpoint(v),let .reviseCheckpoint(v):values=[try .init(kind:.fieldDraftCheckpoint,id:v.draftID)];case let .appendStagingItem(v),let .reviseStagingItem(v):values=[try .init(kind:.attachmentStagingItem,id:v.stageID)];case let .appendCommitSaga(v),let .advanceCommitSaga(v):values=[try .init(kind:.draftCommitSaga,id:v.sagaID)];case let .appendContentReservation(v),let .reviseContentReservation(v):values=[try .init(kind:.draftContentReservation,id:v.reservationID)];case let .applyCommitTerminal(v,_):values=[try .init(kind:.draftCommitSaga,id:v.retiredSaga.sagaID),try .init(kind:.fieldDraftCheckpoint,id:v.committedCheckpoint.draftID),try .init(kind:.draftCommitReceipt,id:v.receipt.receiptID)];case let .applyDiscardTerminal(v):values=[try .init(kind:.fieldDraftCheckpoint,id:v.discardedCheckpoint.draftID),try .init(kind:.draftDiscardReceipt,id:v.receipt.receiptID)];case let .resolveConflict(v):values=[try .init(kind:.fieldDraftCheckpoint,id:v.successorCheckpoint.draftID)];case let .publishReadyStage(v):values=[try .init(kind:.attachmentStagingItem,id:v.readyItem.stageID),try .init(kind:.fieldDraftCheckpoint,id:v.successorCheckpoint.draftID)]};return values.sorted{$0.stableKey<$1.stableKey}}}
    var predecessorIdentity:WorkspaceEntityIdentityV1?{get throws{switch self{case .createCheckpoint,.appendStagingItem,.appendCommitSaga,.appendContentReservation,.applyCommitTerminal,.applyDiscardTerminal,.publishReadyStage:return nil;case let .resolveConflict(v):return try .init(kind:.fieldDraftCheckpoint,id:v.expectedCheckpoint.draftID);case let .reviseCheckpoint(v):return try .init(kind:.fieldDraftCheckpoint,id:v.draftID);case let .reviseStagingItem(v):return try .init(kind:.attachmentStagingItem,id:v.stageID);case let .advanceCommitSaga(v):return try v.predecessorSagaID.map{try .init(kind:.draftCommitSaga,id:$0)};case let .reviseContentReservation(v):return try .init(kind:.draftContentReservation,id:v.reservationID)}}}
    func validate()throws{switch self{case let .createCheckpoint(v):try v.validate();guard v.draftRevision==1 else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .reviseCheckpoint(v):try v.validate();guard v.draftRevision>1 else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .appendStagingItem(v):try v.validate();guard v.revision==1 else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .reviseStagingItem(v):try v.validate();guard v.revision>1 else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .appendCommitSaga(v):try v.validate();guard v.predecessorSagaID==nil else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .advanceCommitSaga(v):try v.validate();guard v.predecessorSagaID != nil else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .appendContentReservation(v):try v.validate();guard v.revision==1 else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .reviseContentReservation(v):try v.validate();guard v.revision>1 else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .applyCommitTerminal(v,expectedSagaRevision):try v.validate();guard expectedSagaRevision>0,expectedSagaRevision<UInt64.max,v.retiredSaga.revision==expectedSagaRevision+1 else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .applyDiscardTerminal(v):try v.validate();case let .resolveConflict(v):try v.validate();case let .publishReadyStage(v):try v.validate()}}
}
struct FieldDraftMutationV1:Codable,Equatable,Sendable{static let schemaVersion=1;let schemaVersion:Int;let workspaceID:WorkspaceID;let expectedRevision:UInt64;let expectedBaseCanonicalRevision:UInt64;let mutationID:MutationIDV1;let postImage:FieldDraftMutationPayloadV1;let continuationBinding:RepetitiveCaptureDestinationContinuationBindingV1?;init(workspaceID:WorkspaceID,expectedRevision:UInt64,expectedBaseCanonicalRevision:UInt64,mutationID:MutationIDV1,postImage:FieldDraftMutationPayloadV1,continuationBinding:RepetitiveCaptureDestinationContinuationBindingV1?=nil)throws{schemaVersion=Self.schemaVersion;self.workspaceID=workspaceID;self.expectedRevision=expectedRevision;self.expectedBaseCanonicalRevision=expectedBaseCanonicalRevision;self.mutationID=mutationID;self.postImage=postImage;self.continuationBinding=continuationBinding;try validate()};func validate()throws{try postImage.validate();if let continuationBinding{guard case let .createCheckpoint(checkpoint)=postImage,expectedRevision==0,expectedBaseCanonicalRevision==checkpoint.baseCanonicalRevision else{throw WorkspaceMutationContractFailureV1.invalidPlan};try RepetitiveCaptureDestinationContinuationV1.validateCheckpoint(checkpoint,binding:continuationBinding)};guard schemaVersion==Self.schemaVersion,workspaceID==postImage.workspaceID,mutationID==postImage.mutationID else{throw WorkspaceMutationContractFailureV1.invalidPlan};switch postImage{case let .applyCommitTerminal(bundle,expectedSaga):guard expectedRevision>0,expectedRevision<UInt64.max,bundle.committedCheckpoint.draftRevision==expectedRevision+1,bundle.committedCheckpoint.baseCanonicalRevision==expectedBaseCanonicalRevision,expectedSaga>0 else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .applyDiscardTerminal(bundle):guard expectedRevision>0,expectedRevision<UInt64.max,bundle.discardedCheckpoint.draftRevision==expectedRevision+1,bundle.discardedCheckpoint.baseCanonicalRevision==expectedBaseCanonicalRevision else{throw WorkspaceMutationContractFailureV1.invalidPlan};case let .publishReadyStage(bundle):guard expectedRevision==bundle.expectedCheckpoint.draftRevision,expectedBaseCanonicalRevision==bundle.expectedCheckpoint.baseCanonicalRevision else{throw WorkspaceMutationContractFailureV1.invalidPlan};default:let predecessor=try postImage.predecessorIdentity;guard predecessor==nil ? expectedRevision==0&&postImage.revision==1:expectedRevision>0&&expectedRevision<UInt64.max&&postImage.revision==expectedRevision+1 else{throw WorkspaceMutationContractFailureV1.invalidPlan}};if case let .reviseCheckpoint(value)=postImage{guard value.baseCanonicalRevision==expectedBaseCanonicalRevision else{throw WorkspaceMutationContractFailureV1.invalidPlan}};if case let .resolveConflict(value)=postImage{guard value.expectedCheckpoint.draftRevision==expectedRevision,value.expectedCheckpoint.baseCanonicalRevision==expectedBaseCanonicalRevision else{throw WorkspaceMutationContractFailureV1.invalidPlan}}};var affectedIdentities:[WorkspaceEntityIdentityV1]{get throws{try postImage.affectedIdentities}};var affectedIdentity:WorkspaceEntityIdentityV1{get throws{let values=try affectedIdentities;guard values.count==1,let value=values.first else{throw WorkspaceMutationContractFailureV1.invalidPlan};return value}};var concurrencyIdentities:[WorkspaceEntityIdentityV1]{get throws{if let continuationBinding{return try continuationBinding.concurrencyIdentities};let values:[WorkspaceEntityIdentityV1];switch postImage{case let .applyCommitTerminal(bundle,_):guard let predecessor=bundle.retiredSaga.predecessorSagaID else{throw WorkspaceMutationContractFailureV1.invalidPlan};values=[try .init(kind:.draftCommitSaga,id:predecessor),try .init(kind:.fieldDraftCheckpoint,id:bundle.committedCheckpoint.draftID),try .init(kind:.draftCommitReceipt,id:bundle.receipt.receiptID)];case let .applyDiscardTerminal(bundle):values=[try .init(kind:.fieldDraftCheckpoint,id:bundle.discardedCheckpoint.draftID),try .init(kind:.draftDiscardReceipt,id:bundle.receipt.receiptID)];case let .resolveConflict(value):values=[try .init(kind:.fieldDraftCheckpoint,id:value.expectedCheckpoint.draftID)]+[value.reviewedTargetBasis.existingIdentity].compactMap{$0};case .publishReadyStage:values=try affectedIdentities;default:let affected=try affectedIdentities;guard affected.count==1,let identity=affected.first else{throw WorkspaceMutationContractFailureV1.invalidPlan};values=[try postImage.predecessorIdentity ?? identity]};return values.sorted{$0.stableKey<$1.stableKey}}};var concurrencyIdentity:WorkspaceEntityIdentityV1{get throws{let values=try concurrencyIdentities;guard values.count==1,let value=values.first else{throw WorkspaceMutationContractFailureV1.invalidPlan};return value}};func expectedRevision(for identity:WorkspaceEntityIdentityV1)throws->UInt64{if let continuationBinding{return try continuationBinding.expectedRevision(for:identity)};switch postImage{case let .applyCommitTerminal(_,expectedSaga):if identity.kind == .draftCommitSaga{return expectedSaga};if identity.kind == .fieldDraftCheckpoint{return expectedRevision};if identity.kind == .draftCommitReceipt{return 0};case .applyDiscardTerminal:if identity.kind == .fieldDraftCheckpoint{return expectedRevision};if identity.kind == .draftDiscardReceipt{return 0};case let .resolveConflict(value):let checkpointIdentity=try WorkspaceEntityIdentityV1(kind:.fieldDraftCheckpoint,id:value.expectedCheckpoint.draftID);if identity==checkpointIdentity{return expectedRevision};if identity==value.reviewedTargetBasis.existingIdentity{return value.reviewedTargetBasis.targetRevision};case let .publishReadyStage(bundle):if identity == (try WorkspaceEntityIdentityV1(kind:.attachmentStagingItem,id:bundle.readyItem.stageID)){return 0};if identity == (try WorkspaceEntityIdentityV1(kind:.fieldDraftCheckpoint,id:bundle.expectedCheckpoint.draftID)){return expectedRevision};default:return expectedRevision};throw WorkspaceMutationContractFailureV1.invalidPlan};func canonicalSHA256()throws->String{try validate();return try WorkspaceMutationCanonicalV1.sha256(self)}}

extension FieldDraftMutationV1 {
    var requiresExplicitConcurrencyBinding: Bool {
        if continuationBinding != nil { return true }
        if case .resolveConflict = postImage { return true }
        return false
    }

    /// Exact key absence is bound to the original workspace revision, in
    /// addition to the checkpoint entity lock carried by this mutation.
    func validateReviewedTargetWorkspaceRevision(_ revision: UInt64) throws {
        if case let .resolveConflict(value) = postImage,
           let expected = value.reviewedTargetBasis.expectedWorkspaceRevision,
           expected != revision {
            throw WorkspaceMutationContractFailureV1.invalidPlan
        }
    }
...
enum TemporalEvidenceMutationPayloadV1: Codable, Equatable, Sendable {
    ...
    func validate()throws{switch self{case .acceptClip(let value,let review,let predecessor):try value.validateIntrinsic();try review.validate();guard review.workspaceID==value.workspaceID,review.clipID==value.clipID,review.decision == .accept,review.reviewedAt==value.acceptedAt else{throw WorkspaceMutationContractFailureV1.invalidPlan};if let predecessor{try predecessor.validateIntrinsic();guard value.workspaceID==predecessor.workspaceID,value.supersedesClipID==predecessor.clipID,value.revision==predecessor.revision+1,value.original==predecessor.original,value.originalProvenance==predecessor.originalProvenance,value.locator==predecessor.locator else{throw WorkspaceMutationContractFailureV1.invalidPlan}}else{guard value.revision==1,value.supersedesClipID==nil else{throw WorkspaceMutationContractFailureV1.invalidPlan}}
        ...
        case .registerDerivative(let successor,let derivative,let predecessor,let priorDerivative):try predecessor.validateIntrinsic();try successor.validateIntrinsic();try derivative.validate(clip:predecessor);if let priorDerivative{try priorDerivative.validate(clip:predecessor);guard derivative.supersedesDerivativeID==priorDerivative.derivativeID,derivative.revision==priorDerivative.revision+1 else{throw WorkspaceMutationContractFailureV1.invalidPlan}}else{guard derivative.revision==1,derivative.supersedesDerivativeID==nil else{throw WorkspaceMutationContractFailureV1.invalidPlan}};guard successor.supersedesClipID==predecessor.clipID,successor.revision==predecessor.revision+1,successor.original==predecessor.original,successor.target==predecessor.target,successor.derivativeReferences.contains(where:{$0.derivativeID==derivative.derivativeID&&$0.derivativeSHA256==derivative.derivativeSHA256}),successor.retentionReference==predecessor.retentionReference else{throw WorkspaceMutationContractFailureV1.invalidPlan}
...
struct RoundSessionMutationV1: Codable, Equatable, Sendable {
    ...
    private init(
        ...
        guard expectedRevision < UInt64.max, workspaceID == session.workspaceID,
              ...
              session.predecessor.map({ $0.revision == expectedRevision }) ?? (expectedRevision == 0) else { throw RoundSessionFailureV1.staleRevision }
