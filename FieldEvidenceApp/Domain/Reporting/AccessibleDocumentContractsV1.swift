...
struct AccessibleDocumentNodeV1: Codable, Equatable, Sendable {
    ...
    func validate() throws {
        ...
        try tableHeaderNodeIDs.forEach(AccessibleDocumentValidationV1.id);try evidenceLinks.forEach{try $0.validate()}
...
struct AccessibleDocumentSemanticTreeV1: Codable, Equatable, Sendable {
    ...
    func validate()throws{
        ...
        try nodes.forEach{try $0.validate()};let byID=Dictionary(uniqueKeysWithValues:nodes.map{($0.nodeID,$0)});guard nodes.filter({$0.parentNodeID == nil}).count==1 else{throw AccessibleDocumentFailureV1.missingParent}
