...
struct ScheduleCompatibilityPolicyV1:Codable,Equatable,Sendable{
    ...
    let calendarOverrideAndBasisPersistence="EMBEDDED_EXISTING_V27_RECORD_FAMILY"
    let allDaysCompatibilityParity=true
    let unknownNewerSchedulePayloadDisposition="FAIL_CLOSED"
    let downgradeDisposition=Self.downgradeDisposition
    let forwardFixRequiredAfterFirstV27Write=true
    static let current=Self()
    func validate()throws{guard Set(Self.readablePersistentWriterVersions).count==27,Self.readablePersistentWriterVersions.last==Self.currentPersistentWriterVersion,Set(Self.readableBackupWriterVersions).count==Self.readableBackupWriterVersions.count,Self.readableBackupWriterVersions.last==Self.currentBackupWriterVersion,exceptionPersistence=="EMBEDDED_HISTORY_PAYLOAD_ONLY",dueAndReminderPersistence=="DERIVED_ONLY",calendarOverrideAndBasisPersistence=="EMBEDDED_EXISTING_V27_RECORD_FAMILY",allDaysCompatibilityParity,unknownNewerSchedulePayloadDisposition=="FAIL_CLOSED",downgradeDisposition==Self.downgradeDisposition,forwardFixRequiredAfterFirstV27Write,unknownVersionsFailClosed else{throw CompatibilityContractErrorV1.invalidSupportTable}}
}
...
enum C52ServiceRequestBoundary_ReleasedDataCompatibilityPolicyV1 {
