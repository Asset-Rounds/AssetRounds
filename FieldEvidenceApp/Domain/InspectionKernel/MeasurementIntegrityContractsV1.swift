...
struct MeasurementSeriesV1:Codable,Equatable,Sendable,MeasurementIntegrityValidatableV1{
    ...
    func validate()throws{try protocolReference.validate();try samples.forEach{try $0.validate()};try derivedFact?.validate();guard expectedSampleCount>0,expectedSampleCount<=MeasurementIntegrityLimitsV1.maximumSampleCount else{throw MeasurementIntegrityFailureV1.invalidValue};let ordinals=samples.map(\.sampleOrdinal);let expectedOrdinals=Array(1...expectedSampleCount);let observedPrefix=Array(expectedOrdinals.prefix(samples.count));let aggregateShape=aggregationPolicy == .none ? derivedFact==nil : derivedFact != nil;let finalShape=state == .finalized ? (observedSampleCount==expectedSampleCount && ordinals==expectedOrdinals && aggregateShape) : (observedSampleCount<=expectedSampleCount && derivedFact==nil);guard schemaVersion==Self.schemaVersion,snapshotID != UUID.zero,seriesID != UUID.zero,revision>0,(revision == 1)==(supersedesSnapshotID==nil),supersedesSnapshotID != snapshotID,observedSampleCount==samples.count,ordinals==observedPrefix,Set(samples.map(\.captureID)).count==samples.count,Set(ordinals).count==ordinals.count,finalShape,derivedFact.map{$0.workspaceID==workspaceID && $0.protocolReleaseID==protocolReference.releaseID} ?? true,seriesSHA256==(try MeasurementIntegrityCanonicalCodecV1.sha256(digestBasis)) else{throw MeasurementIntegrityFailureV1.invalidValue}}
