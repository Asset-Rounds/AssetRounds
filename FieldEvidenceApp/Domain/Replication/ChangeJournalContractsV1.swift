...
enum PortableReviewChangeJournalPolicyV1 { static let commandKind:WorkspaceCommandKindV1 = .applyPortableReview;static let replaysExactResponseBytes=true;static let postimagesUseOnlyExistingC14Families=true;static let historyDiscardAndQuarantineAreSessionOnly=true }
enum WorkResourceChangeJournalPolicyV1 { static let commandKind:WorkspaceCommandKindV1 = .applyWorkResource;static let appendOnlyCanonicalPostimage=true;static let directCostIsEmbeddedNotASecondLedger=true }
enum PartsStockChangeJournalPolicyV1 { static let commandKind:WorkspaceCommandKindV1 = .applyPartsStock; static let appendOnlyMovements = true; static let stockAndWorkResourceUseIsAtomic = true }
enum MyDayChangeJournalPolicyV1 { static let commandKind:WorkspaceCommandKindV1 = .applyMyDay; static let sourceAndTargetCarryoverIsAtomic = true; static let readinessAndDueAreDerived = true }
...
enum PortableReviewJournalContractV1{static func validate(envelope:MutationEnvelopeV1,receipt:MutationReceiptV1,entityChanges:[EntityChangeV1])throws{guard case let .applyPortableReview(m)=envelope.command else{return};try m.validate();let affected=try m.affectedIdentities,images=try m.mutationPostImages;guard envelope.commandKind == .applyPortableReview,envelope.mutationID==m.mutationID,receipt.mutationID==m.mutationID,receipt.postImages==images,entityChanges.map(\.identity)==affected,entityChanges.map(\.postImage)==images else{throw ChangeJournalFailureV1.tamperedBatch};_ = try PortableReviewMutationReceiptV1(mutation:m,mutationReceipt:receipt)}}
enum WorkResourceJournalContractV1{static func validate(envelope:MutationEnvelopeV1,receipt:MutationReceiptV1,entityChanges:[EntityChangeV1])throws{guard case let .applyWorkResource(m)=envelope.command else{return};try m.validate();let affected=try m.affectedIdentities,images=try m.mutationPostImages;guard envelope.commandKind == .applyWorkResource,envelope.mutationID==m.mutationID,receipt.mutationID==m.mutationID,receipt.postImages==images,entityChanges.map(\.identity)==affected,entityChanges.map(\.postImage)==images else{throw ChangeJournalFailureV1.tamperedBatch};_ = try WorkResourceMutationReceiptV1(mutation:m,mutationReceipt:receipt)}}
enum PartsStockJournalContractV1{static func validate(envelope:MutationEnvelopeV1,receipt:MutationReceiptV1,entityChanges:[EntityChangeV1])throws{guard case let .applyPartsStock(m)=envelope.command else{return};try m.validate();let affected=try m.affectedIdentities,images=try m.mutationPostImages;guard envelope.commandKind == .applyPartsStock,envelope.mutationID==m.mutationID,receipt.mutationID==m.mutationID,receipt.postImages==images,entityChanges.map(\.identity)==affected,entityChanges.map(\.postImage)==images else{throw ChangeJournalFailureV1.tamperedBatch}}}
enum MyDayJournalContractV1{static func validate(envelope:MutationEnvelopeV1,receipt:MutationReceiptV1,entityChanges:[EntityChangeV1])throws{guard case let .applyMyDay(m)=envelope.command else{return};try m.validate();let affected=try m.affectedIdentities,images=try m.mutationPostImages;guard envelope.commandKind == .applyMyDay,envelope.mutationID==m.mutationID,receipt.mutationID==m.mutationID,receipt.postImages==images,entityChanges.map(\.identity)==affected,entityChanges.map(\.postImage)==images else{throw ChangeJournalFailureV1.tamperedBatch};_ = try MyDayWorkspaceMutationReceiptV1(mutation:m,mutationReceipt:receipt)}}
...
enum ScheduleJournalContractV1{static func validate(envelope:MutationEnvelopeV1,receipt:MutationReceiptV1,entityChanges:[EntityChangeV1])throws{guard case let .applySchedule(mutation)=envelope.command else{return};try mutation.validate();let affected=try mutation.affectedIdentities,images=try mutation.mutationPostImages;guard envelope.commandKind == .applySchedule,envelope.mutationID==mutation.mutationID,receipt.mutationID==mutation.mutationID,receipt.postImages==images,entityChanges.map(\.identity)==affected,entityChanges.map(\.postImage)==images else{throw ChangeJournalFailureV1.tamperedBatch};switch mutation.payload{case .appendExceptionCalendarRelease:guard entityChanges.count==1,entityChanges[0].identity.kind == .exceptionCalendarRelease else{throw ChangeJournalFailureV1.tamperedBatch};case .appendOverrideEvent:guard entityChanges.count==1,entityChanges[0].identity.kind == .scheduleOverrideEvent else{throw ChangeJournalFailureV1.tamperedBatch};default:break};_ = try ScheduleMutationReceiptV1(mutation:mutation,mutationReceipt:receipt)}}
enum PlanJournalContractV1{static func validate(envelope:MutationEnvelopeV1,receipt:MutationReceiptV1,entityChanges:[EntityChangeV1])throws{guard case let .applyPlan(mutation)=envelope.command else{return};try mutation.validate();let affected=try mutation.affectedIdentities,images=try mutation.mutationPostImages;guard envelope.commandKind == .applyPlan,envelope.mutationID==mutation.mutationID,receipt.mutationID==mutation.mutationID,receipt.postImages==images,entityChanges.map(\.identity)==affected,entityChanges.map(\.postImage)==images else{throw ChangeJournalFailureV1.tamperedBatch};_ = try PlanMutationReceiptV1(mutation:mutation,mutationReceipt:receipt)}}
...
enum EntityIdentityResolutionJournalContractV1 {
    static func validate(
        envelope: MutationEnvelopeV1,
        receipt: MutationReceiptV1,
        entityChanges: [EntityChangeV1]
    ) throws {
        guard case let .applyEntityIdentityResolution(command) = envelope.command else { return }
        try command.validate()
        let targets = try command.affectedIdentitiesForCanonicalWriter()
        let expectedPairs = command.expectedRevision.entityRevisions.map { ($0.identity, $0.revision) }
        guard Set(expectedPairs.map(\.0)).count == expectedPairs.count else { throw ChangeJournalFailureV1.tamperedBatch }
        let expected = Dictionary(uniqueKeysWithValues: expectedPairs)
        let receiptIdentities = try receipt.postImages.map { try $0.identity }
        guard envelope.commandKind == .applyEntityIdentityResolution,
              envelope.mutationID == command.mutationID,
              receipt.mutationID == command.mutationID,
              receipt.postImages.count == 1,
              entityChanges.count == 1,
              ...
              entityChanges.map(\.identity) == targets,
              entityChanges.map(\.postImage) == receipt.postImages,
              receipt.postImages.map(\.semanticSHA256).sorted() == command.payload.semanticSHA256s.sorted(),
              let identity = receiptIdentities.first,
...
enum C52ServiceRequestBoundary_ChangeJournalContractsV1 {
