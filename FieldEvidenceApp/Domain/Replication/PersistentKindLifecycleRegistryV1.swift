...
enum SurveyDefinitionPersistentKindPolicyV1 {
    ...
    static func validate(_ descriptors:[PersistentKindDescriptorV1])throws{let byID=Dictionary(uniqueKeysWithValues:descriptors.map{($0.stableKindID,$0)});guard durableKindIDs.allSatisfy({byID[$0].map{PersistentKindLifecycleRegistryV1.hasIndependentRepresentationWrite($0.subject)} == true}),derivedKindIDs.allSatisfy({id in guard let descriptor=byID[id] else{return false};return descriptor.kindClassification == .derived && descriptor.storage == .derivedProjection && !PersistentKindLifecycleRegistryV1.hasIndependentRepresentationWrite(descriptor.subject) && descriptor.temporalEvidence.disposition == .nonpersistentNoCanonicalWrite})else{throw PersistentKindLifecycleFailureV1.invalidLifecyclePolicy}}
