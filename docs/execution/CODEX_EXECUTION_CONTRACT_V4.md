...
No new approval message is required. Complete failing logs and checksum-verified artifacts normally classify a run as hosted infrastructure only when failure is confined to GitHub runner provisioning, pinned toolchain/runtime readiness, exact-Simulator boot/migration, GitHub Actions service behavior, or artifact transport before every compiler diagnostic, product test assertion, selector mismatch, and product-code failure. Pre-repository provisioning failure may instead use authoritative GitHub run/job/step metadata plus complete Actions logs proving no repository command executed. Upload/transport failure requires complete logs proving evidence validation and checksum creation succeeded first. An aggregate-total assertion failure is eligible only for the next K, never same-head redispatch, when every constituent ceiling, exact selector, evidence check, checksum, and upload passed and hosted readiness scheduling is the diagnosed cause. That K may only reduce/overlap infrastructure scheduling while preserving the exact aggregate start, endpoint, values, and assertion; the failed run remains non-green. Ambiguity and every other product/compiler/test/acceptance failure are ineligible.

For one immutable gate identity—phase, card, workflow, required ref role(s), product implementation candidate E, and first failed run/head—name failed verification head F and permit at most two linear diagnosed support heads K1/K2. Owner-directed S0 K1 installed this rule through the five authority documents and workflow named in CURRENT_TASK. The present owner-directed S0 guardrail closure may amend those same six paths once in K2; afterward AGENTS, plan, runbook, and this contract are immutable again. Later gate-scoped K may change only a causally required subset of CURRENT_TASK, `.github/workflows/ios-ci.yml`, and the four smoke/timeout scripts; CURRENT_TASK may record/repin but cannot broaden/reset authority. Derive K/dispatch counters from reachable commit ancestry and every GitHub exact-ref/head candidate, including pre-existing candidates.

Each K directly parents the failed head. Preserve HANDOFF, selector, project/app/tests/fixtures/assets, product behavior, test selectors, runner/Xcode/Simulator selection, frozen N8/P12/F25 values, existing product/card acceptance, workflow trigger/permissions/action pins, selector/tier/result validation, exact build/test/UI commands/flags/destinations/result paths, `set -euo pipefail`, process-group timeout/TERM/KILL/exit-124 behavior, command/pipeline exit propagation, total-budget start before checkout, endpoint after checksum, accounting/assertion, and artifact/checksum/upload enforcement. Only causally required infrastructure readiness/wait/transport scheduling and additive nonempty checksummed diagnostics/acceptance may change; existing evidence cannot be removed or weakened.

Fresh G0 proves immutable gate/evidence, `K^=F`, exact `F..K` allowlisted diff/pins, clean state, history-derived counters, ref state, and no protected E..K product/selector change. Use non-force exact-ref-gated pushes; never rerun a failed run ID. A provider failure before repository execution may use one same-head redispatch without K; green exact-F then satisfies the unchanged ordinary gate. Each K1/K2 otherwise receives one initial dispatch per required ref; only an identical mechanically eligible non-acceptance infrastructure signature under the eligibility paragraph, containing no compiler diagnostic, test result/assertion, selector, acceptance, or product failure, permits one no-code same-head fresh-runner redispatch. A diagnosed cause requiring a lane-path correction may instead consume the next K without using that retry; creation of the next K retires the failed head. Any aggregate-total or other acceptance failure at K2, ineligible K2 result, non-green separately eligible K2 retry, ambiguity, ref divergence, unlisted path, or evidence weakening stops technically without further approval; no K3 exists. Once K exists, only green exact-K required artifacts are accepted; K becomes the verification/integration head while E remains implementation evidence and subsequent M/P uses K. If an open card's K exposes product failure, ordinary unused I2 may start from latest support head; a closed boundary card never reopens.

Candidate history is closed per exact workflow/ref/head: zero candidates dispatches the initial run; one active waits; one green accepts; one eligible non-acceptance terminal failure dispatches the sole unused retry; and an eligible diagnosed correction instead creates the next K and retires the failed head. Exactly two same-head candidates are valid only when the first is that eligible non-acceptance failure and the second is active/green; wait for or accept the second. A terminal non-green second run, other order/status, or more than two candidates stops. An authorized retry is not an ambiguous duplicate.

Exact S0 bridge: phase run `31579936670` at `K1=0da62482db7e3f90742876ef05574de9e9569e32` remains failed. Its 90-entry checksum-verified artifact proves readiness, setup/artifact, build, unit, UI, selectors, result bundles, screenshot, evidence validation, checksums, and upload passed at unchanged constituent ceilings; only aggregate `727 > 720` failed. It may authorize only direct-child K2 through the five authority documents plus workflow. K2 starts the exact selected UDID earlier, overlaps migration only with unchanged Xcode inspection, bounds request-to-ready by the same 300 seconds, and preserves the aggregate start before checkout, endpoint after checksum, and P12 `120/180/180/240/720`. K2 is final; no K3 exists.
