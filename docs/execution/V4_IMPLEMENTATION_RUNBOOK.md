...
Install this exact file as `docs/execution/V4_IMPLEMENTATION_RUNBOOK.md`. It contains **36 strictly ordered Codex coding cards plus two owner-only release gates**. One persistent `/goal` may complete the coding program through S9.1 while every card remains separately scoped, committed, and CI-verified. Program autopilot performs verified fast-forward `main` integration and exact-main CI at each phase boundary. A mechanically proven hosted-CI failure may use only the finite autonomous infrastructure lane below; it never expands product-card scope.
...
At S0 phase start, the owner fills every applicable first-card `CURRENT_TASK.md` field and freezes program autopilot, the exact phase/branch map, phase autopilot, and the ordered S0 span. Later phase starts may be hydrated by program autopilot only after the preceding phase's accepted exact-main verification/phase-close head, including an accepted infrastructure `K` when the standing lane applies. For any transition, Codex may hydrate only the immediate next runbook card and must use this closed rule:
...
1. The first card of S0 starts on the owner-prepared branch. Each later phase starts on its frozen branch created by program autopilot from accepted green exact-`main`; later cards continue the same phase branch from the prior accepted verification head plus one transition commit.
2. Owner or authorized program autopilot installs phase-start `A` containing CURRENT_TASK alone. For each card, Codex performs fresh read-only G0: verify `P`, ancestry, exact `M..A`, selected hashes/card, program/phase map, dirty paths, envelope, Full access posture, pins, selectors, and budget. A committed boundary HANDOFF forbids rerunning the card but permits Step 7 and, after mechanically eligible hosted-CI failure evidence, Step 8. Infrastructure post-commit G0 proves `K^=F`, the exact declared `F..K` diff, clean state, matching pins, protected-tree/selector equality, expected remote refs, and full failed-run artifact diagnosis.
3. If G0 passes, Codex implements only the card and creates implementation commit `I`; the sole recovery instead creates exactly `I3`. Only when separately authorized may Codex push and dispatch unsigned CI.
4. Product/card success normally requires a green run with `head_sha == I`. One diagnosed card-scoped fix `I2` and one rerun are allowed; a product/compiler/test/acceptance non-green rerun stops. Historical S0.1 used its recorded `I3` exception. A terminal result that is mechanically eligible hosted infrastructure follows Step 8 independently and never consumes or resets the product allowance.
5. On a non-boundary card after green evidence, Codex appends HANDOFF. Only when phase autopilot is enabled, the same-phase transition flag is `yes`, and the immediate next runbook card is inside the exact authorized span may Codex apply Section 3's closed rule and create one transition commit containing exactly that HANDOFF append plus the next CURRENT_TASK. Immediately before its non-force push, the remote phase ref must still equal the accepted verification head `V` (`V=E` normally; exact green K when the lane applies); immediately afterward it must equal the transition commit. Any mismatch stops. Then run fresh G0. This commit is never implementation evidence and may not move while card CI is pending.
...
7. On a final card after green evidence, Codex appends HANDOFF and commits/pushes only that append as phase-close `C` when boundary integration is authorized. Prove phase ref `C`. Remote `main=P` permits one non-force fast-forward to `C`; `main=C` resumes; any other value stops. Any named workflow-dispatch run with `ref=main` and `head_sha=C` is the existing candidate: accept success, wait/inspect queued/in-progress, and dispatch once only when none exists. A terminal non-success is classified under Step 8; ineligible product/compiler/test/acceptance failure stops. Reprove remote phase and main refs at the exact accepted verification head. Unless S9.1, valid next-branch recovery is local absent or at that head/exact A and remote absent or exact A; hydrate CURRENT_TASK-only A from the accepted head, non-force create remote when local A/remote absent, or resume exact remote A; every other state stops. Run fresh G0. After S9.1, report the exact accepted main head/run in the goal final response and stop for owner-only S9.2/S9.3.
8. Autonomous hosted-CI infrastructure recovery: no new approval message is required. Eligibility normally requires complete logs and checksum-verified artifacts proving failure solely in hosted runner provisioning, pinned toolchain/runtime readiness, exact-Simulator boot/migration, GitHub Actions service behavior, or artifact transport before compiler/product tests. Pre-repository provisioning failure may instead use authoritative GitHub metadata and complete Actions logs proving no repository command executed; upload/transport failure requires logs proving required-evidence validation and checksum creation already succeeded. A failed aggregate-total assertion is eligible only for the next K, never same-head redispatch, when every constituent ceiling, exact selector, evidence check, checksum, and upload passed and hosted readiness scheduling is the diagnosed cause. That K may only reduce/overlap readiness scheduling while preserving the exact aggregate start, endpoint, values, and assertion; the failed run remains non-green. Ambiguity and every other product/compiler/test/acceptance failure are ineligible. Preserve product implementation candidate E and set failed verification head F. Owner-directed S0 K1 installed this rule through the five authority documents plus workflow. The present owner-directed S0 guardrail closure may amend those same six paths once in K2; afterward AGENTS/plan/runbook/contract are immutable again. Later gate-scoped K changes only a causally required subset of CURRENT_TASK, `.github/workflows/ios-ci.yml`, and the four smoke/timeout scripts. CURRENT_TASK cannot broaden/reset authority.

   One immutable gate identity is phase, card, workflow, required ref role(s), E, and first failed run/head. Derive the maximum two linear K heads and every dispatch/redispatch from reachable Git ancestry plus all GitHub exact-ref/head candidate history, including pre-existing candidates; prose counters are descriptive only. Each K directly parents the failed head. Preserve HANDOFF, selector, project/app/tests/fixtures/assets, product semantics, runner/Xcode/Simulator selector, N8/P12/F25 values, existing product/card acceptance, workflow trigger/permissions/action pins, selector/tier/result validation, exact build/test/UI commands/flags/destinations/result paths, `set -euo pipefail`, process-group timeout/TERM/KILL/exit-124 behavior, command/pipeline exit propagation, total-budget start before checkout, endpoint after checksum, accounting/assertion, and artifact/checksum/upload enforcement. Only causally required infrastructure readiness/wait/transport scheduling and additive nonempty checksummed diagnostics/acceptance may change. G0 proves immutable gate/evidence, exact F..K allowlisted diff/pins, counters, ref state, and E..K protected-tree equality. Use non-force exact-ref-gated pushes; never rerun a failed run ID.

   A provider failure before repository execution may consume one same-head redispatch without K; green exact-F then satisfies the unchanged ordinary gate. Each K1/K2 otherwise gets one initial dispatch per required ref. Only an identical mechanically eligible non-acceptance infrastructure signature under the eligibility paragraph, containing no compiler diagnostic, test result/assertion, selector, acceptance, or product failure, permits one no-code same-head fresh-runner redispatch. A diagnosed cause requiring a lane-path correction may instead use the next K without consuming that retry; creation of the next K retires the failed head. K2 may take one same-head retry only for a separately eligible non-acceptance signature; any aggregate-total or other acceptance failure at K2, ineligible K2 result, non-green K2 retry, ambiguity, ref divergence, unlisted path, or evidence weakening stops technically without approval and no K3 exists. Once K exists, only green exact-K artifacts are accepted; K becomes the verification/integration head while E remains implementation evidence and next M/P uses K. If an open card's K exposes product failure, exit the lane and use ordinary unused I2 from the latest support head; a closed boundary card never reopens.

   Candidate history is closed independently per exact workflow/ref/head: zero candidates dispatches the initial run; one active waits; one green accepts; one eligible non-acceptance terminal failure dispatches the sole unused retry; and an eligible diagnosed correction instead creates the next K and retires the failed head. Exactly two same-head candidates are valid only when the first is that eligible non-acceptance failure and the second is active/green; wait for or accept the second. A terminal non-green second run, any other order/status, or more than two candidates stops. An authorized retry is not an ambiguous duplicate.

9. Current S0 infrastructure recovery: retain `E=I3=ac310dd37700d65165200f742aaec1d48d0a34d6`, HANDOFF-only `C=15348e92466a79efd02bfb71f5ffee043e615829`, authority head `R=1e0322cc072fda28718468bcd1b0dbe1f5d0accc`, failed main runs `31567299022` at C and `31571125674` at R, and unchanged completed HANDOFF. K1 `0da62482db7e3f90742876ef05574de9e9569e32` directly parents R and installed the 300-second readiness step. Exact phase run `31579936670` at K1 remains failed: its 90-entry checksummed artifact proves readiness passed under 300, setup/artifact/build/unit/UI enforcement passed at unchanged `120/180/180/240`, exact selectors/result bundles/screenshot/evidence/checksums/upload passed, and only aggregate `727 > 720` failed. Retire K1's unused phase retry. Create direct-child K2 changing exactly the five authority documents plus `.github/workflows/ios-ci.yml`. K2 moves exact Simulator selection before unchanged required Xcode inspection, records the request epoch, accepts initial selected-UDID state only `Shutdown` or `Booted`, starts `Shutdown` nonblockingly under the existing process-group timeout helper and same 300-second ceiling, overlaps migration only with those unchanged checks, then gives `bootstatus -b` only the positive remainder of that 300 seconds and proves request-to-ready `<=300`. Preserve the aggregate start before checkout, endpoint after checksum, P12 `120/180/180/240/720`, selectors, commands, evidence, and protected bytes. Post-K2/resume G0 proves exact `K1..K2`, pins, clean state, S1 absent, and exactly one ref state: phase=K1/main=R, phase=K2/main=R, or phase/main=K2. Push only the missing next ref. Require green exact-head phase K2 before main advances and green exact-main K2 before S1. K2 is final; any K2 aggregate overrun cannot retry, and no K3 exists. Only green exact-main K2 makes it S0 phase-close/S1 base; then hydrate only S1.1 with `P=M=K2`. Do not amend HANDOFF or rerun any failed run.

Every card gets one product implementation decision and exact-head verification. Every phase gets fast-forward integration and exact-main verification. The independent finite infrastructure lane may move the verification head without changing product implementation and never resets the product-fix budget. One persistent program goal runs sequentially; no two cards write the checkout concurrently.
...
S0 creates `run-with-timeout.sh`; it accepts a positive integer ceiling followed by one command and its arguments, launches that command in its own process group, forwards the exit status, and on expiry sends TERM, waits at most five seconds, sends KILL if needed, and exits 124. The workflow validates selector shape before build and validates selector resolution/execution from the resulting `.xcresult` after tests, wraps build/test/enabled-UI scripts with those ceilings, and fails when elapsed setup+lane-authorized infrastructure readiness+build+test+UI+evidence-validation/checksum time exceeds the total. Artifact upload is outside the selected total but remains inside the 30-minute job hard stop.
...
- Owner-authorized recovery outcome: preserve the complete `I2` S0.1 project/app/tests/scripts/selector; after failed runs `31562160165` at `I` and `31562792005` at `I2`, a fresh four-document authority `A` with `M=I2` may authorize exactly one `I3`: remove exact-UDID `xcrun simctl bootstatus "$CI_SIMULATOR_UDID" -b` from `Scripts/test-smoke.sh` and add it to `Scripts/build-smoke.sh` immediately before `xcodebuild`; obtain one green exact-`I3` P12 result without any other implementation change. Preserve selector bytes, P12 `120/180/180/240/720`, workflow/ref/input, all other script semantics, and every prohibition. Dispatch exactly once at `I3`; non-green stops.
- Hosted-CI boundary recovery: preserve successful exact-`I3` run `31566371521`, HANDOFF-only `C`, authority head `R`, and failed boundary runs as history. Apply lifecycle Steps 8–9 only. This is CI verification/integration support, not another S0.1 implementation outcome; no product/card/selector/tier/HANDOFF byte changes.
...
- Terminal/next: phase-end fast-forward/exact-main CI; next S7.1 only after the required repository-local fixture input for frozen product ID `com.palatis3.fieldrecord.sub.solo.monthly.v1` and fixed grace/family settings is already resolved; otherwise program autopilot stops for owner input. No App Store SKU exists unless the commitment gate passed.
...
- Terminal/next: phase-end fast-forward/exact-main CI; next S9.1 only after required release inputs are resolved, otherwise program autopilot stops for owner input.
...
- Terminal/next: phase-end fast-forward/exact-main CI; then stop for owner S9.2.
...
head_sha equals I or the one allowed product diagnosed-fix I2. A mechanically
proven hosted-infrastructure failure uses only the finite Steps 8–9 K lane.
...
and continue. Stop on divergence, ambiguity, missing input, any product/compiler/test failure,
any acceptance failure outside the exact aggregate-total infrastructure class, or exhausted infrastructure recovery. Never force-push, sign, upload,
deploy, or submit. Stop after S9.1 for the
owner-only S9.2/S9.3 gates.
```

## 12. Completion rule

A card completes only with accepted product implementation evidence, green exact verification-head CI, and recorded handoff. A phase completes only after verified fast-forward integration and green CI on the exact resulting `main` verification/phase-close SHA. When the infrastructure lane applies, record implementation `E` and verification head `K` separately and use accepted `K` as the next base. Current S0 requires green exact-main K2 while S0.1 remains evidenced by I3 and its existing HANDOFF. Coding automation completes after S9.1's exact-main gate; product release completes only after owner-recorded S9.2/S9.3 evidence. A Windows edit, stale run, divergent branch, plan-only artifact, or upload without physical verification is not completion.
