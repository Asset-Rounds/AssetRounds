...
- Allowed/forbidden: exactly the seven canonical inventory/coverage JSON instances, `FieldEvidenceApp/Features/Issues/WorkCoordinator.swift`, `FieldEvidenceApp/Infrastructure/Finalization/FinalizationService.swift`, `FieldEvidenceApp/Infrastructure/Finalization/FinalizationRecoveryService.swift`, `S10_1BrandInventoryTests.swift`, and `S10_1BrandInventoryUITests.swift`; activation/authority bytes are owner-directed correction authority and selector/HANDOFF remain standing exceptions. App mutation is limited to the closed repeated work/recheck lineage grammar diagnosed by hosted runs `31919914831` and `31922598904`: work admission plus matching precommit, replay, and recovery authority. No other app, project, asset, feature, fixture, release-metadata, or design-token mutation. Explicit envelope override: 10 product/document files and 2 test files because the V4.1 validator requires the seven one-to-one canonical instances, the three earlier-card-owned lineage corrections, and a UI baseline-capture lane.
- Exact delta: documentation, machine-readable coverage, the diagnosed repeated-lineage corrections, and baseline evidence only. Inventory uses stable route/state IDs and points to existing source identifiers/fixtures; planned later tokens are `planned`, every visual row is `baseline_frozen`, and candidate/review fields remain blank or `NOT_RUN`/`NOT_REVIEWED`. The corrected candidate—not either defective tap path—is the frozen S9 behavioral baseline.
