...
- H411 owner-approved S10.4 execution exception: S10.4 owner amendment H411 (2026-09-08): all fourteen existing shards may consume one immutable, qualified exact-head build, with the five required native unit methods performed once per selected payload and referenced truthfully by each consumer. All seven minimum-profile logical shards may be assembled from independently reproducible, source-defined segments whose disjoint owned states cover all 67 inventory states and whose public prerequisites and continuously executed critical journeys satisfy all six common tasks. Preserve 938 visual cells, 84 accessibility rows, strict native AX/contrast, original checksummed source/environment/producer/consumer evidence, genuine human visual review and the S10 E/K/C lifecycle. Freeze one head and payload before qualification and matrix collection. No consumer rebuild/fallback, cross-head evidence, copied Simulator state or invented local unit pass is allowed. GitHub remains the formal minimum-runtime route; Bitrise formal current-light/current-dark retains its existing exact-head equivalence gate. CURRENT_TASK H411 controls the exact finite implementation envelope and readiness gates; the amendment is prospective until the executable contracts and hosted qualification are complete.
- H411 explicit additional support envelope, outside the unchanged four evidence-document/two-test cap: `.github/workflows/ios-ci.yml`, `.github/workflows/ios-ci-worker.yml`, `Scripts/s10-4-segment-plan.json`, `Scripts/s10-4-segment-assembler.sh`, `Scripts/test-smoke.sh`, `Scripts/ui-smoke.sh`, `Scripts/s10-4-build-payload.py`, `docs/design/s10/authority/s10.4-automation-amendment-v1/manifest.json`, `docs/design/s10/authority/s10.4-automation-amendment-v1/s10-visual-regression.schema.json`, `docs/design/s10/authority/s10.4-automation-amendment-v1/validate-s10-contracts.ps1`, and required `docs/execution/CURRENT_TASK.md`/`docs/execution/S10_4_CI_OPERATING_BRIEF.md` evidence/pins. The existing two test paths remain the only unit/UI lab tests. Use the smallest causal subset; no app/project/fixture/asset or later-card scope is added. The payload script contains only the finite shared-build seal/verification logic, not a generalized CI framework.
- H412 owner-approved S10.4 execution efficiency: implement one reviewed source-defined parameterized controller and factual audit rather than per-head helper graphs or fixed historical run counts. Preserve append-only originals, durable exclusive intent/direct run IDs, fresh repository/ref/head/main-P and capacity checks, no active duplicate/cancel/rerun/fallback, audited retry reasons, own-head five native producer methods, immutable payload/source/expiry and exact segment dependencies. Prestage operational configuration and collector review while producer work runs; never launch a dependent route before its required gate. Audit completed originals once and reuse verified hashes while rechecking mutable API state/expiry. Continue one combined integration review through incremental findings rather than repeat unchanged broad scans.
- H412 additional support envelope, outside the unchanged four evidence-document/two-lab-test cap: exactly `Scripts/s10-4-ci.py` and `Scripts/test-s10-4-ci.py`, in addition to H411's existing finite support paths. These standard-library Python files implement only S10.4 dispatch, collection and factual audit/control checks; they add no product behavior, native test target, dependency or acceptance substitute. The H412 authority transaction may also change `AGENTS.md`, `docs/execution/S10_4_CI_OPERATING_BRIEF.md`, selected S10.4 text in this runbook, resulting runbook/activation pins and append-only provenance in `docs/design/s10/s10-activation.json` and `docs/design/s10/authority/s10.4-automation-amendment-v1/manifest.json`, required `docs/execution/CURRENT_TASK.md` evidence/pins, and causally affected active identities in the existing unit lab test. No other card or product-plan/environment/rights/baseline/release truth changes.
- H412 public preparation and diagnostic policy: measure shorter public setup only after explicit entry/exit/replay/dependency proof preserves all67owned states, continuously executed critical journeys, profile-specific fixes and unchanged durable data. No copied Simulator/database, injected entitlement/receipt or guessed observation; cached values cannot cross invalidating actions/waits. Correct only demonstrated current diagnostic eligibility/export/incident-window/retained-operand/integrity gaps, preserving original errors, strict AX/contrast, crash/source/PID/Simulator binding, finite budgets, truncation and non-atomic/missing-value truth. Do not suppress warnings or infer their crash causality. Startup retains the existing exact GitHub macos-26/Xcode26.6-17F113/iPhoneSE3/iOS18.0-22A3351 route and fresh isolated Simulator; no alternate provider/runtime/cache, increased watchdog or speculative sleep is authorized. Policy, implementation review and hosted qualification are separate gates. Batch compatible diagnosed corrections, qualify affected routes, then freeze final-matrix bytes and stop optional optimization; all14shards/67states/938visual cells/84accessibility rows/six common tasks/native checks/human review remain unchanged.
- H414 owner-approved finite app-diagnostic exception (2026-09-09): admit only the reviewed DEBUG passive path/lifecycle observation in `FieldEvidenceApp/Features/Signs/SignsRootView.swift`, exact current Reduce Transparency launch opt-in in `FieldEvidenceAppUITests/S10_3BrandMigrationUITests.swift`, and causal active contracts in `FieldEvidenceAppTests/S10_4AutomatedBrandLabTests.swift`. The app exception is outside the ordinary four-evidence/two-test cap. Record only fixed scalar fields while the exact opt-in token and a current recheck-due issue are present; preserve the original NavigationStack binding, path/state writes, actions, queries, waits, assertions and released product literals. No native Back interception, custom binding, private API, route serialization, new state, fixture, runtime, provider or repair is authorized. The observation can affect DEBUG timing and may miss coalesced transitions; neither missing output nor source review establishes a handler result or navigation fix. The finite authority/pin transaction may change `AGENTS.md`, this selected S10.4 material, `docs/execution/S10_4_CI_OPERATING_BRIEF.md`, `docs/execution/CURRENT_TASK.md`, and resulting runbook/activation/manifest identities in `docs/design/s10/s10-activation.json` and `docs/design/s10/authority/s10.4-automation-amendment-v1/manifest.json`. The product plan and all noncausal bytes remain unchanged. Qualify the actual new head with an unsigned immutable producer, all five native methods and affected existing routes; complete the separately owner-approved H413 required-nine matrix and genuine new-head visual review after coherently rebasing that evidence-policy transaction. Existing fourteen-profile source catalog and original old-head evidence remain intact; no historical native or human result transfers.
- H413 owner-approved S10.4 required-nine acceptance: retain all fourteen runnable profiles, 938 catalog visual slots and 84 catalog accessibility rows. Require exactly all seven current profiles plus minimum `minimum-os` and minimum `double-length`, totaling 603 required visual cells and 54 required accessibility rows. Preserve minimum `rtl`, `rtl-string`, `tall`, `accented`, and `bounded` as explicit `DEFERRED` coverage totaling 335 visual cells and 30 accessibility rows; never record those deferred cells or rows as `PASS` or `NOT_APPLICABLE`. Each required profile still proves all 67 states, all six continuously executed critical journeys, strict native AX/contrast/geometry with only existing narrow exceptions, all five native producer methods, complete original integrity, and genuine human visual review. Preserve every prior run, failure, artifact, test, fix, audit, and available catalog entry. Deferral cannot conceal a demonstrated defect affecting a required profile. H413 supersedes only the selected S10.4 fourteen-profile acceptance count; every other card and product, iOS 18 deployment/runtime, physical, and release invariant remains unchanged.
- H413 evidence lifecycle: keep all required native consumers on the frozen exact `ProductHead E` and its selected qualified immutable payload; no cross-head promotion is allowed. A diagnosed native correction must qualify its own new exact product head and collect the complete retained nine-profile matrix. After retained same-E closure and genuine human review, apply the coherent nonshipping authority/schema/validator/manifest/evidence amendment as descendant `EvidenceHead K`, then later bookkeeping `C`. The permitted E..policy-K paths are exactly `AGENTS.md`, `docs/product/BUILD_PLAN_V4.md`, `docs/execution/CODEX_EXECUTION_CONTRACT_V4.md`, this runbook, `docs/execution/S10_4_CI_OPERATING_BRIEF.md`, `docs/execution/CURRENT_TASK.md`, `docs/design/s10/s10-activation.json`, the amendment `manifest.json`, both amendment overlay schemas, the amendment validator, the three canonical S10 accessibility/token/visual evidence documents, and exactly `Scripts/s10-4-ci.py` plus `Scripts/test-s10-4-ci.py` for the diagnosed nonshipping collector correction. This is the sixteen-path envelope; no app or native execution code may change in policy K. Validators may bind native/catalog/unit proof to `ProductHead E` and revised acceptance to `EvidenceHead K`; policy staging or review alone does not establish acceptance or require a new native build solely for policy bytes.
...
- H418 owner-selected S10.4 acceptance: require the seven current-runtime full logical shards (469 visual cells/42 automated accessibility rows), defer all seven minimum full shards explicitly (469/42), and preserve the complete fourteen-profile/938/84 catalog. Double is nonblocking. Separately require `s10.4.minimum-core-smoke.v1` on the exact GitHub minimum-os/none shared-consumer tuple with ordered launch, sign-saved, capture-review, report-saved, report-reopened and settings checkpoints, native pass, seven inspection screenshots and complete source/run/producer/payload/expiry/command/attachment/original bindings. The smoke closes zero catalog cells/rows, never claims full-shard/full-segment completion and adds no human gate. Current-profile human review remains genuine and mandatory; all physical/manual fields remain `NOT_RUN`. Compose the replacement policy after actual native E and selected own-head producer qualification, preserving the exact twenty source paths, eventual twenty-three paths, historical audit recovery and `E -> K -> C`.
