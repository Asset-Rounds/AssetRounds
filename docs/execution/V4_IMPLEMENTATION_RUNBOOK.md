...
8. Autonomous hosted-CI infrastructure recovery: no new approval message is required. Eligibility normally requires complete logs and checksum-verified artifacts proving failure solely in hosted runner provisioning, pinned toolchain/runtime readiness, exact-Simulator boot/migration, GitHub Actions service behavior, or artifact transport before compiler/product tests. Pre-repository provisioning failure may instead use authoritative GitHub metadata and complete Actions logs proving no repository command executed; upload/transport failure requires logs proving required-evidence validation and checksum creation already succeeded. Ambiguity is ineligible. Preserve product implementation candidate E and set failed verification head F. The current owner-directed S0 K1 may install this rule by changing exactly the five authority documents plus workflow in CURRENT_TASK. After it, AGENTS/plan/runbook/contract are immutable under the lane; later K changes only a causally required subset of CURRENT_TASK, `.github/workflows/ios-ci.yml`, and the four smoke/timeout scripts. CURRENT_TASK cannot broaden/reset authority.

   One immutable gate identity is phase, card, workflow, required ref role(s), E, and first failed run/head. Derive the maximum two linear K heads and every dispatch/redispatch from reachable Git ancestry plus all GitHub exact-ref/head candidate history, including pre-existing candidates; prose counters are descriptive only. Each K directly parents the failed head. Preserve HANDOFF, selector, project/app/tests/fixtures/assets, product semantics, runner/Xcode/Simulator selector, N8/P12/F25 values, existing product/card acceptance, workflow trigger/permissions/action pins, selector/tier/result validation, exact build/test/UI commands/flags/destinations/result paths, `set -euo pipefail`, process-group timeout/TERM/KILL/exit-124 behavior, command/pipeline exit propagation, total-budget accounting, and artifact/checksum/upload enforcement. Only causally required infrastructure readiness/wait/transport handling and additive nonempty checksummed diagnostics/acceptance may change. G0 proves immutable gate/evidence, exact F..K allowlisted diff/pins, counters, ref state, and E..K protected-tree equality. Use non-force exact-ref-gated pushes; never rerun a failed run ID.

   A provider failure before repository execution may consume one same-head redispatch without K; green exact-F then satisfies the unchanged ordinary gate. Each K1/K2 otherwise gets one initial dispatch per required ref. Only an identical mechanically eligible infrastructure signature under the eligibility paragraph, containing no compiler diagnostic, test result/assertion, selector, acceptance, or product failure, permits one no-code same-head fresh-runner redispatch. A distinct eligible cause may use the next K. K2 may take its one same-head retry only for that identical signature; any ineligible K2 result or non-green K2 retry, ambiguity, ref divergence, unlisted path, or evidence weakening stops technically without approval and no K3 exists. Once K exists, only green exact-K artifacts are accepted; K becomes the verification/integration head while E remains implementation evidence and next M/P uses K. If an open card's K exposes product failure, exit the lane and use ordinary unused I2 from the latest support head; a closed boundary card never reopens.

   Candidate history is closed independently per exact workflow/ref/head: zero candidates dispatches the initial run; one active waits, one green accepts, and one eligible terminal failure dispatches the sole unused retry. Exactly two are valid only when the first is that eligible failure and the second is active/green; wait for or accept the second. A terminal second run, any other order/status, or more than two candidates stops. An authorized retry is not an ambiguous duplicate.

9. Current S0 infrastructure recovery: retain `E=I3=ac310dd37700d65165200f742aaec1d48d0a34d6`, HANDOFF-only `C=15348e92466a79efd02bfb71f5ffee043e615829`, authority head `R=1e0322cc072fda28718468bcd1b0dbe1f5d0accc`, failed main runs `31567299022` at C and `31571125674` at R, and unchanged completed HANDOFF. Create direct-child `K1` from R changing exactly the five execution-authority documents plus `.github/workflows/ios-ci.yml`. The workflow gives exact-UDID `simctl bootstatus -b` its own fixed 300-second hosted-infrastructure step before the unchanged P12 180-second build wrapper and requires `simulator-boot.log`; P12 `120/180/180/240/720` and its total remain unchanged. Post-K/resume G0 proves exact `R..K1`, protected bytes, pins, clean state, S1 absent, and exactly one ref state: initial phase/main=R, phase-only K1/R, or integrated K1/K1. Push only the missing next ref; otherwise resume closed exact-head candidate history without re-push or duplicate dispatch. Require green exact-head UI-enabled phase CI before main advances and green exact-main UI-enabled CI before S1. Only green exact-main K1 makes it S0 phase-close/S1 base; then hydrate only S1.1 with `P=M=K1`. Do not amend HANDOFF or rerun either failed run.
...
and continue. Stop on divergence, ambiguity, missing input, product/compiler/test/
acceptance failure, or exhausted infrastructure recovery. Never force-push, sign, upload,
...
A card completes only with accepted product implementation evidence, green exact verification-head CI, and recorded handoff. A phase completes only after verified fast-forward integration and green CI on the exact resulting `main` verification/phase-close SHA. When the infrastructure lane applies, record implementation `E` and verification head `K` separately and use accepted `K` as the next base. Current S0 requires green exact-main `K1` while S0.1 remains evidenced by `I3` and its existing HANDOFF. Coding automation completes after S9.1's exact-main gate; product release completes only after owner-recorded S9.2/S9.3 evidence. A Windows edit, stale run, divergent branch, plan-only artifact, or upload without physical verification is not completion.
