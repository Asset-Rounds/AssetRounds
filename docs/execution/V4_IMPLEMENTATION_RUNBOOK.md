...
- Outcome: freeze the one-to-one six-stage S10 contract, complete released route/state and common-task inventory, map every state to the approved token catalog, correct the one diagnosed prebaseline S9 lineage defect, and capture immutable pre-migration screenshots of that corrected unpublished candidate before any brand/presentation byte changes.
- Allowed/forbidden: exactly the seven canonical inventory/coverage JSON instances, `FieldEvidenceApp/Features/Issues/WorkCoordinator.swift`, `S10_1BrandInventoryTests.swift`, and `S10_1BrandInventoryUITests.swift`; activation/authority bytes are owner-directed correction authority and selector/HANDOFF remain standing exceptions. The sole app mutation is the exact terminal `issue_still_visible` open-lineage acceptance correction diagnosed by hosted run `31919914831`; no other app, project, asset, feature, fixture, release-metadata, or design-token mutation. Explicit envelope override: 8 product/document files and 2 test files because the V4.1 validator requires the seven one-to-one canonical instances, the one prebaseline behavior correction, and a UI baseline-capture lane.
- Exact delta: documentation, machine-readable coverage, the one diagnosed `WorkCoordinator` lineage correction, and baseline evidence only. Inventory uses stable route/state IDs and points to existing source identifiers/fixtures; planned later tokens are `planned`, every visual row is `baseline_frozen`, and candidate/review fields remain blank or `NOT_RUN`/`NOT_REVIEWED`. The corrected candidate—not the defective tap path—is the frozen S9 behavioral baseline.
