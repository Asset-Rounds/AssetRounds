...
8. Autonomous hosted-CI infrastructure recovery: no approval message is required. An exact terminal non-green run is eligible only when complete logs and checksum-verified artifacts prove its first causative failure is hosted-runner provisioning, pinned toolchain/runtime readiness, exact-Simulator readiness, Actions service/transport, or lane-owned harness scheduling/accounting, with no compiler diagnostic, product test result/assertion, selector mismatch, or product failure. Pre-repository failure may use authoritative metadata and complete logs proving no repository command ran; upload failure requires logs proving validation/checksum creation succeeded first. The sole legacy no-manifest exception is exact K2 phase run `31583957094`/head `400f8827f5451b9a44f4d846340b51377554069d`/artifact `9136413551` digest `sha256:284632a9587fa296a36b1c57c9e9320779f022a4d68b8ff5a592305105bfaedd`; it authorizes K3 only and is consumed. K3 changes evidence ordering so later post-checkout classification requires checksum-verified evidence. Setup `<=120` is eligible only as first failure before product commands; aggregate total is eligible only after every constituent/product/selector/evidence/checksum/upload gate passed. Build/unit/UI ceiling failures and every other acceptance failure are ineligible. Failed assertions remain non-green; ambiguity is ineligible.

   For immutable gate identity phase/card/workflow/ordered-ref-roles/product head E/first-failed-run+head, support heads are a direct-child sequence `K1,K2,...` with no numeric ceiling or approval gate. Fresh G0 derives the next index from reachable lineage plus all exact-ref/head candidates; freezes canonical gate/failure/repair records under AGENTS' literal grammar and closed registries; hashes them; freezes one diagnosis/minimal delta; proves `K^=F`; and proves no earlier K consumed the same/overlapping operation+mechanism or repair key. Volatile ref/run/head/display/time/UDID/duration/log/cascade values are provenance only. K3+ commits require one exact gate/index/failed-run/head/ref/failure/repair trailer each; G0 recomputes them and rejects disagreement. Legacy K1/K2 are indices 1/2. CURRENT_TASK cannot reset or relabel history.

   Each K contains mechanical CURRENT_TASK evidence/pins plus exactly one correction across the smallest causal subset. K3 is the sole installer exception: it may once amend AGENTS/plan/runbook/contract/CURRENT_TASK/workflow/build-smoke to install static schema/refreeze, non-accepting failure-evidence ordering, and exactly one K2 causal correction. After K3 the four static sources and all smoke/timeout scripts refreeze; later K may use only CURRENT_TASK plus marker-delimited infrastructure-recovery workflow bodies, and one-delta/no-adjacent-hardening is absolute. Preserve HANDOFF, selector, product/project/tests/fixtures/assets, runner/Xcode/Simulator selection, N8/P12/F25 values, trigger/permissions/action pins, exact build/test/UI commands/selectors/destinations/results, selector/tier/result validation, fail-fast/timeout/exit behavior, evidence/checksum/upload enforcement, and raw aggregate start-before-checkout/end-after-checksum. Readiness remains `<=300` and fully counted in raw total. G0 proves the direct delta, cumulative E..K protected-tree/workflow-spine equality, and attribution of every lane hunk to one unique failure/repair.

   Per exact workflow/ref/head, zero candidates dispatches once; one active waits; one green accepts. One terminal failure takes exactly one route: one same-head fresh-runner candidate only for pre-repository provider or post-checksum upload/transport failure with no indicated repository delta, or the next K for a distinct eligible failure/repair and unique correction. Retry consumption is gate-wide: before the no-code retry, canonicalize an unconsumed failure/repair; the first terminal candidate plus its exact GitHub receipt consumes both across every ref, and its retry is their sole disposition. GitHub history is immediately authoritative; CURRENT_TASK mirrors it only at the next otherwise-authorized commit. Setup/aggregate failures never redispatch same-head. Creating K retires every unused dispatch/retry at F. Exactly two candidates are valid only as that consumed provider/transport failure followed by active/green/terminal; green accepts, and terminal can create another K only for a distinct eligible failure/repair. Any consumed failure/repair recurrence on any ref, a second retry, ambiguity/ineligibility, another ordering, or more than two candidates stops technically. Failed run IDs are never rerun. CURRENT_TASK freezes ordered ref predecessors/cursor; each moves once by non-force fast-forward only after prior roles are green, and an exact already-moved role resumes without re-push. Accept only green exact-K complete artifacts. E remains product evidence; accepted K becomes verification/integration/next base. An open card may use ordinary unused I2 after a product failure; a closed boundary card never reopens. Technical stops never request authority text, and numeric index is never a stop.

9. Current S0 bridge: retain `E=I3=ac310dd37700d65165200f742aaec1d48d0a34d6`, HANDOFF-only `C=15348e92466a79efd02bfb71f5ffee043e615829`, authority head `R=1e0322cc072fda28718468bcd1b0dbe1f5d0accc`, failed main runs `31567299022` at C and `31571125674` at R, and immutable HANDOFF. K1 `0da62482db7e3f90742876ef05574de9e9569e32` and K2 `400f8827f5451b9a44f4d846340b51377554069d` are failed support heads. K1 run `31579936670` failed only aggregate `727 > 720` after readiness, setup/artifact, build/unit/UI, selectors, evidence, checksums, and upload passed. K2 phase run `31583957094` first failed setup `232 > 120` after its boot request put Simulator readiness on setup's critical path; build/unit/UI did not run, downstream missing evidence/checksum was deterministic, total `233 <= 720`, and upload succeeded. Never accept or redispatch those runs. Create direct-child K3 with exactly the seven CURRENT_TASK paths. Restore toolchain/scheme/deployment/Simulator selection and setup enforcement before readiness; run the bounded exact-UDID readiness step as a tracked GitHub background step concurrently with the unchanged exact-destination build; remove only build-smoke's redundant readiness command; explicitly wait for readiness before tests; and preserve P12 `120/180/180/240/720`, aggregate endpoints, selector, product/tests, and exact xcodebuild invocation. Post-K3/resume G0 proves trailers/pins/diff/protected spine, S1 absent, and one closed ref state: phase=K2/main=R, phase=K3/main=R, or phase/main=K3 after green phase. The first current K at index 3 or later with green exact-head phase and green exact-main CI becomes S0 phase-close/S1 `P=M`; I3 remains product evidence. A distinct eligible later failure/repair may create the next K; a repeated/overlapping failure or repair or other technical stop does not request authority text. Do not amend HANDOFF.

Every card gets one product implementation decision and exact-head verification. Every phase gets fast-forward integration and exact-main verification. The independent monotonic evidence-bounded infrastructure lane may move the verification head without changing product implementation and never resets the product-fix budget. One persistent program goal runs sequentially; no two cards write the checkout concurrently.
...
proven hosted-infrastructure failure uses only the monotonic evidence-bounded Steps 8–9 K lane and needs no approval message.
...
an ineligible or ambiguous infrastructure classification, a repeated or overlapping consumed failure/repair, absence of one unique allowed-path delta, invalid ref/candidate provenance, or another technical lane stop. A numeric K index is never a stop. Never force-push, sign, upload,
...
A card completes only with accepted product implementation evidence, green exact verification-head CI, and recorded handoff. A phase completes only after verified fast-forward integration and green CI on the exact resulting `main` verification/phase-close SHA. When the infrastructure lane applies, record implementation `E` and verification head `K` separately and use accepted `K` as the next base. Current S0's next candidate is K3; the first current support head at index 3 or later with green exact-head phase and green exact-main CI becomes S0 verification/phase-close and S1 `P=M`, while S0.1 remains evidenced by I3 and its existing HANDOFF. Coding automation completes after S9.1's exact-main gate; product release completes only after owner-recorded S9.2/S9.3 evidence. A Windows edit, stale run, divergent branch, plan-only artifact, or upload without physical verification is not completion.
