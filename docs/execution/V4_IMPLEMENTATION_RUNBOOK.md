...
At S0 phase start, the owner fills every applicable first-card `CURRENT_TASK.md` field and freezes program autopilot, the exact phase/branch map, phase autopilot, and the ordered S0 span. Later phase starts may be hydrated by program autopilot only after the preceding phase's accepted exact-main CI, including successful exact-main `R` for the sole S0 boundary recovery below. For any transition, Codex may hydrate only the immediate next runbook card and must use this closed rule:
...
- Set current card ID/order/position/boundary/tier/selectors from the immediate runbook card. Within a phase set `M` to prior accepted `I`/`I2` and expected `M..A` to HANDOFF plus CURRENT_TASK. At phase start set `M=P` and expected `M..A` to CURRENT_TASK alone; include predecessor main run evidence. The sole S0.1 implementation recovery instead sets `M` to failed `I2`, declares the exact four-document `M..A`, preserves `P`, and records both failed exact-head runs. The separate sole S0 boundary recovery keeps historical `P/M/A/I3/C` evidence immutable and uses post-commit `R^=C` plus exact four-document `C..R`; it never treats `R` as card authority or implementation. Keep ordinary card `A`, and boundary-recovery `R`, as `OBSERVE AT G0` values that cannot self-record their SHA. Card-specific UI mode, selector, tier, timeouts, commands, paths, delta, acceptance, and exact current/ref arguments change only to frozen immediate-card or explicitly pinned recovery values.
...
- S0.1 alone uses bootstrap SHA `B` because no iOS project CI exists yet. At phase start, owner- or program-autopilot-prepared `A` normally changes only CURRENT_TASK. Before implementation or CI, approved D-004/D-005/D-006 may install the declared finite authority/config/workflow amendment when CURRENT_TASK names exact cumulative `M..A` paths and repins changed hashes. The one owner-authorized post-`I2` S0.1 recovery may install a new `A` that directly parents `I2`, changes exactly `AGENTS.md`, `BUILD_PLAN_V4.md`, this runbook, and CURRENT_TASK, sets `M=I2`, repins exact bytes, and authorizes only the frozen two-script `I3` move and one final P12 dispatch stated below. The separate sole S0 boundary recovery may install one `R` directly after completed HANDOFF-only `C`, changing exactly those same four authority documents; fresh G0 proves `C..R`, remote phase/main=`C`, S1 absent, unchanged implementation/HANDOFF/workflow/selector/P12 bytes, and all repinned source hashes. `R` receives no card CI and is never implementation evidence. A later ordinary same-phase `A` must parent prior green implementation and contain exactly HANDOFF plus CURRENT_TASK; a next-phase `A` normally parents C, but S1.1 after successful S0 recovery must parent accepted `R`, contain CURRENT_TASK only, and set `P=M=R`. Fresh G0 proves immutable carry-forward fields and allows only frozen immediate-card or explicitly pinned recovery fields to differ. It validates expected selector/tier while allowing `ci-selection.json` absent for initial S0.1 or still equal predecessor; after G0, replacing it is the first implementation-support mutation only when CURRENT_TASK authorizes a selector change.
...
1. The first card of S0 starts on the owner-prepared branch. Each later phase starts on its frozen branch created by program autopilot from accepted green exact-`main`; later cards continue the same phase branch from prior green implementation plus one transition commit.
2. Owner or authorized program autopilot installs phase-start `A` containing CURRENT_TASK alone, except the one pre-S0 authority/config/workflow amendment declared by CURRENT_TASK, the one exact four-document S0.1 implementation recovery amendment, and the one exact four-document S0 boundary recovery `R`. For each card or implementation recovery, Codex performs fresh read-only G0: verify `P`, ancestry, exact `M..A`, selected hashes/card, program/phase map, dirty paths, envelope, Full access posture, pins, selectors, and budget. For boundary recovery, the post-commit G0 instead proves `R^=C`, one exact four-document `C..R`, clean state, repins, remote phase/main=`C`, S1 absent, and unchanged completed-card evidence/implementation/HANDOFF/workflow/selector/tier/timeouts. A committed boundary HANDOFF forbids rerunning the card but permits only Step 7 or the exact Step 8 exception.
...
4. Success requires a green run with `head_sha == I`. One diagnosed card-scoped fix `I2` and one rerun are allowed. A second non-green run stops. The sole exception is the owner-authorized S0.1 recovery: after its exact four-document authority amendment and fresh G0 with `M=I2`, exactly one `I3` and one final exact-head P12 dispatch are allowed; a non-green recovery run stops with no further edit or dispatch.
5. On a non-boundary card after green evidence, Codex appends HANDOFF. Only when phase autopilot is enabled, the same-phase transition flag is `yes`, and the immediate next runbook card is inside the exact authorized span may Codex apply Section 3's closed rule and create one transition commit containing exactly that HANDOFF append plus the next CURRENT_TASK. Immediately before its non-force push, the remote phase ref must still equal accepted `I`/`I2` (or the exact S0.1 recovery `I3`); immediately afterward it must equal the transition commit. Any mismatch stops. Then run fresh G0. This commit is never implementation evidence and may not move while card CI is pending.
...
7. On a final card after green evidence, Codex appends HANDOFF and commits/pushes only that append as phase-close `C` when boundary integration is authorized. Prove phase ref `C`. Remote `main=P` permits one non-force fast-forward to `C`; `main=C` resumes; any other value stops. Any named workflow-dispatch run with `ref=main` and `head_sha=C` is the existing candidate: accept success, wait/inspect queued/in-progress, dispatch once only when none exists, and stop on every terminal non-success (the workflow itself enforces UI-input equality). Reprove remote phase and main refs both `C`. Unless S9.1, valid next-branch recovery is local absent or at C/exact A and remote absent or exact A; hydrate CURRENT_TASK-only A from C, non-force create remote when local A/remote absent, or resume exact remote A; every other state stops. Run fresh G0. After S9.1, report C/run in the goal final response and stop for owner-only S9.2/S9.3.
8. Sole S0 boundary recovery: after exact `I3` card success, completed HANDOFF-only `C=15348e92466a79efd02bfb71f5ffee043e615829`, and failed exact-main run `31567299022` at `C` before `xcodebuild` during hosted `CoreLocationMigrator`, the owner may authorize one authority-only `R` directly after `C`. `C..R` changes exactly `AGENTS.md`, the plan, this runbook, and CURRENT_TASK; HANDOFF and all implementation/project/app/test/script/selector/workflow bytes are unchanged. After the Step 2 recovery G0, require remote phase/main=`C` and S1 absent; non-force push phase `C→R`, verify; non-force fast-forward `main C→R`, verify. Never rerun `31567299022`; it is not an `R` candidate. Reuse an exact workflow-dispatch candidate at `ref=main/head_sha=R`, or dispatch once with `run_ui_smoke=true` only when none exists. Every terminal non-success at `R` stops permanently with no edit, rerun, or further recovery. Matching success makes `R` the accepted S0 phase-close; reprove phase/main=`R`, then create/resume only `phase/s1-shell-design` using the normal collision rule with base `R`, hydrate only S1.1 in a CURRENT_TASK-only `A` directly after `R`, set S1 `P=M=R`, record the `R` run evidence, and run fresh G0. Do not amend the completed S0.1 HANDOFF.

Every card gets one implementation-head CI decision; every phase normally gets one fast-forward integration and one exact-main CI decision. The recorded S0.1 `I3` recovery receives exactly one final implementation-head decision, and the recorded S0 boundary `R` recovery receives exactly one additional authority-head integration decision without reopening the card. No other exception exists. One persistent program goal runs sequentially; no two cards write the checkout concurrently.
...
- Owner-authorized boundary recovery: after successful exact-`I3` run `31566371521`, HANDOFF-only `C=15348e92466a79efd02bfb71f5ffee043e615829`, and failed exact-main run `31567299022` at `C`, apply lifecycle Step 8 only. This is authority/integration recovery, not another S0.1 implementation outcome; no card path, selector, tier, timeout, workflow, or HANDOFF byte changes.
...
except for the one pinned S0.1 I3 recovery; its final non-green run stops.
...
accepted I/I2 (or exact S0.1 recovery I3), use non-force push only, and verify the ref equals the new commit.
...
and continue. Stop on divergence, a non-green required run, ambiguity, or missing
input, except only when CURRENT_TASK pins the sole S0 authority-only Step 8
recovery; its non-green run stops permanently. Never force-push, sign, upload,
...
A card completes only with green exact-implementation-head CI and recorded handoff. A phase completes only after verified fast-forward integration and green CI on the exact resulting `main` SHA; for the sole S0 boundary recovery, this means successful exact-main CI at authority-only `R`, while the card remains evidenced by `I3` and its existing HANDOFF. Coding automation completes after S9.1's exact-main gate; product release completes only after owner-recorded S9.2/S9.3 evidence. A Windows edit, stale run, divergent branch, plan-only artifact, or upload without physical verification is not completion.
