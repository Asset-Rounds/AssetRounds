...
- iOS 26.1 fallback recovery authority: exact-head runs `31853962973`, `31854610306`, and `31855173723` all passed build plus `4/4` units and reached the real `SubscriptionStoreView` purchasing state on iOS 26.2, but bounded queries through the app, SpringBoard, and `com.apple.ios.StoreKitUIService` respectively exposed no payment-dialog controls. This satisfies the owner-authorized iOS 26.1 fallback condition without weakening the system cancel/confirm proof. The exact GitHub `macos-15` image manifest `20260727.0377.1` provides Xcode 26.1.1 build `17B100`, iOS 26.1, and iPhone 17. The infrastructure correction changes only this file plus `.github/workflows/ios-ci.yml`; every product/project/test/fixture/selector byte and acceptance rule remains unchanged.
- iOS 18.6 fallback recovery authority: exact-head run `31859401967` at `42d1638eea8f6fdec23cac180691a396a7f44d38` passed build plus `4/4` units on iOS 26.1 and reached the real `SubscriptionStoreView` purchasing state, but the checksummed XCResult again contained no StoreKit UI-service process or payment-dialog controls. Apple documents payment-sheet automation with `SKTestSession` dialogs enabled, so this repeated iOS 26 CLI signature is a pinned Simulator failure. The same exact runner artifact proves installed/available iOS 18.6 build `22G86` with an available iPhone 16 Pro. The infrastructure correction changes only this file plus `.github/workflows/ios-ci.yml`; Xcode 26.1.1, product/project/test/fixture/selector bytes, commands, watchdogs, and real StoreKit-system cancel/confirm acceptance remain unchanged.
- Xcode 16.4 fallback recovery authority: exact-head run `31860651032` at `dea11765e8a4b9d1c62400921f16a367c0e37fd4` passed the Xcode 26.1.1/iOS 18.6 build plus `5/5` units and reached real purchase initiation after a bounded no-publication window, but `com.apple.ios.StoreKitUIService` remained pid `0` and no payment-dialog control existed. Artifact `ios-ci-31860651032-1` / ID `9240601449` / digest `sha256:d7829372fac0735032527fad3b0e507b9611ef89ef44d239bab11fcac4d2e6af` passed `1059/1059` checksum validation. This disproves app-state publication and the Xcode 26 runtime family as causal repair routes. The official `macos-15` image provides Xcode 16.4 build `16F6` at `/Applications/Xcode_16.4.app/Contents/Developer`, and the exact failed artifact proves installed/available iOS 18.5 build `22F77` with an available iPhone 16 Pro. The diagnosed infrastructure correction changes only this file plus `.github/workflows/ios-ci.yml`; product/project/test/fixture/selector bytes, commands, watchdogs, and real StoreKit-system cancel/confirm acceptance remain unchanged.
...
- Workflow: `.github/workflows/ios-ci.yml` / `C78AF03E25BBF2101044950C6E3588BE7AC10916E18B71F08A25BE87E5B937C9` / `workflow_dispatch`.
...
- Runner / Xcode / project: `macos-15 / Xcode 16.4 Build version 16F6 / /Applications/Xcode_16.4.app/Contents/Developer / FieldEvidenceApp.xcodeproj / FieldEvidenceApp / Debug / iOS 18.0`.
- Simulator selector: `iPhone 16 Pro / iOS 18.5`; resolve exactly one ephemeral-job UDID.
...
- Fresh iOS 26.1 fallback G0 must prove this two-path infrastructure correction directly parents exact failed head `b0a5c545e261942cc39468a339e9a2a8da0a2dc7`, changes only `docs/execution/CURRENT_TASK.md` plus `.github/workflows/ios-ci.yml`, pins the installed `macos-15` / Xcode 26.1.1 / iOS 26.1 / iPhone 17 route and workflow hash above, preserves the exact selector object and every product/project/test/fixture byte, and leaves remote `main=P`.
- Fresh Xcode 16.4 fallback G0 must prove this two-path infrastructure correction directly parents exact failed head `dea11765e8a4b9d1c62400921f16a367c0e37fd4`, changes only `docs/execution/CURRENT_TASK.md` plus `.github/workflows/ios-ci.yml`, pins the installed `macos-15` / Xcode 16.4 build `16F6` / iOS 18.5 build `22F77` / iPhone 16 Pro route and workflow hash above, preserves the exact selector object and every product/project/test/fixture byte, and leaves remote `main=P`.
