...
- Task-start authority A is the direct-child S7.2 transition commit observed after it exists at fresh G0; this file deliberately never self-records that future SHA.
- Required `M..A`: exactly one direct-child commit changing only append-only `docs/execution/HANDOFF.md` plus this replacement `docs/execution/CURRENT_TASK.md`; remote `phase/s7-commerce=A`, remote `main=P`, and no other dirty path at fresh G0.
- Required pre-G0 owner authority: the pinned plan/runbook require exact recovery copy for StoreKit cancelled, pending, unverified, and failed outcomes but contain no literal strings. Implementation remains read-only until the owner supplies all four exact messages and one direct-child authority correction records them here. This is a product-truth input, not an implementation approval gate.
...
- Owner-copy gate: exact cancelled/pending/unverified/failed recovery strings are intentionally not guessed in this transition. A direct-child CURRENT_TASK-only owner-authority correction must freeze all four before G0 can pass.
...
- Fresh G0 must prove clean `phase/s7-commerce`, `A^=M`, exact one-commit HANDOFF-plus-CURRENT_TASK-only `M..A`, remote S7 phase=A, remote main=P, all pins, accepted S7.1 run/artifact/checksum evidence, accepted predecessor S7.1 selector byte-exact, and the eight-production/two-test expanded envelope inside the default 10/5 cap.
- G0 must additionally prove the four exact owner-frozen recovery messages are present in this file. Until then, remain read-only: do not replace the selector, edit implementation/tests, or dispatch S7.2 CI.
