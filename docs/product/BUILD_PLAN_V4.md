...
The canonical `V4_IMPLEMENTATION_RUNBOOK.md` expands this into **36 strictly ordered Codex coding cards plus two owner-only release gates**: 1+1+2+7+5+4+6+5+4+1 cards across S0–S9. One persistent `/goal` may execute the coding program through S9.1 under the frozen phase/branch map. Every card requires accepted implementation evidence and green exact-head verification; every phase requires a verified fast-forward `main` update and green exact-main CI before the next phase branch. S9.2/S9.3 remain owner-only. Ambiguous transition, unresolved input, unexpected ref movement, non-fast-forward boundary, any product/compiler/test failure, any acceptance failure outside the exact aggregate-total infrastructure class, or exhausted hosted-infrastructure recovery stops the train.
...
Historical S0 boundary evidence is immutable: accepted S0.1 product implementation `I3=ac310dd37700d65165200f742aaec1d48d0a34d6` and run `31566371521`; HANDOFF-only `C=15348e92466a79efd02bfb71f5ffee043e615829`; failed exact-main run `31567299022` at `C`; authority-only `R=1e0322cc072fda28718468bcd1b0dbe1f5d0accc`; and failed exact-main run `31571125674` at `R`. Both failures exhausted the 180-second build wrapper inside exact-UDID `simctl bootstatus -b` during hosted Simulator migration before `xcodebuild`; neither is a product, compiler, test, selector, or acceptance failure, neither may be rerun, and neither is green evidence.

Standing hosted-CI infrastructure recovery needs no new approval message. Eligibility normally requires complete logs and checksum-verified artifacts proving failure solely in hosted runner provisioning, pinned toolchain/runtime readiness, exact-Simulator boot/migration, GitHub Actions service behavior, or artifact transport before compilation/product tests. Pre-repository provisioning failure may instead use authoritative GitHub metadata plus complete Actions logs proving no repository command ran; upload/transport failure requires logs proving evidence validation and checksum creation succeeded first. A failed aggregate-total assertion is eligible only for the next K, never same-head redispatch, when every constituent ceiling, exact selector, required-evidence check, checksum, and upload passed and hosted readiness scheduling is the diagnosed cause; that K may only reduce/overlap infrastructure scheduling while preserving the exact aggregate start, endpoint, values, and assertion. The failed run remains non-green. Ambiguity and every other product/compiler/test/acceptance failure are ineligible. For one immutable gate identity (phase, card, workflow, required ref roles, product implementation candidate `E`, first failed run/head), use failed verification head `F` plus at most two linear diagnosed support heads K1/K2. Owner-directed S0 K1 installed this rule through the five authority documents and workflow. The present owner-directed S0 guardrail closure may amend those same six paths once in K2; afterward AGENTS/plan/runbook/execution contract are immutable again and CURRENT_TASK remains record/repin-only. Later gate-scoped K may change only a causally required subset of CURRENT_TASK, `.github/workflows/ios-ci.yml`, and the four smoke/timeout scripts, with CURRENT_TASK unable to broaden/reset the rule. Derive K/dispatch counters from reachable commits and GitHub exact-ref/head history.

Every K directly parents the failed head. Preserve HANDOFF, selector, project/app/tests/fixtures/assets, product behavior, selectors, runner/Xcode/Simulator selection, N8/P12/F25 values, existing product/card acceptance, workflow trigger/permissions/action pins, selector/tier/result validation, exact build/test/UI invocations, shell fail-fast/pipefail and timeout/exit propagation, total-budget start before checkout, endpoint after checksum, accounting/assertion, and artifact/checksum/upload enforcement. Only causally necessary infrastructure readiness/wait/transport scheduling and additive nonempty checksummed diagnostics/acceptance may change; existing evidence may not be removed or weakened. G0 proves immutable gate identity, eligible evidence, exact `F..K` allowlisted diff/pins, history-derived counters, and `E..K` protected-tree equality. Pushes are non-force and exact-ref gated; failed run IDs are never rerun. A provider failure before repository execution may consume one same-head redispatch without K; green exact-F then satisfies the unchanged ordinary gate. Each K1/K2 otherwise gets one initial dispatch per required ref and one same-head redispatch only for an identical mechanically eligible non-acceptance infrastructure signature containing no compiler diagnostic, test result/assertion, selector, acceptance, or product failure. A diagnosed cause requiring a lane-path correction may instead use the next K without consuming the retry; creation of that K retires the prior head's retry. If K2 remains non-green after a separately eligible non-acceptance retry, has an aggregate-total or other acceptance failure, or encounters ambiguity, ref divergence, an unlisted path, or evidence weakening, stop technically without requesting approval and create no K3. Once K exists, only green exact-K evidence is accepted; record E and K separately and use K for subsequent M/phase P. An open card may exit to unused ordinary I2 if K exposes a product failure; a closed boundary card never reopens.

Per exact workflow/ref/head, candidate history is closed: zero candidates dispatches the initial run; one active waits; one green accepts; one eligible non-acceptance terminal failure dispatches the sole unused retry; and an eligible diagnosed correction instead creates the next K and permanently retires the failed head. Exactly two same-head candidates are valid only when the first is that eligible non-acceptance failure and the second is active/green; wait for or accept the second. A terminal non-green second run, other order/status, or more than two candidates stops. An authorized retry is therefore not an ambiguous duplicate.

For the current S0 recovery, K1 `0da62482db7e3f90742876ef05574de9e9569e32` directly parents R and installed the 300-second exact-UDID readiness step. Exact phase run `31579936670` at K1 remains failed: its 90-entry checksum-verified artifact proves readiness passed under 300; setup/artifact, build, unit, and UI enforcement passed at unchanged `120/180/180/240`; exact selectors, result bundles, screenshot, evidence validation, checksums, and upload passed; and only aggregate `727 > 720` failed. This is the sole current aggregate bridge and cannot be accepted or redispatched. Create direct-child K2 from K1 changing exactly `AGENTS.md`, this plan, the runbook, the execution contract, CURRENT_TASK, and `.github/workflows/ios-ci.yml`. K2 starts the exact selected UDID earlier and overlaps migration only with unchanged required Xcode inspection. It records the request epoch, rejects every initial state except `Shutdown` or `Booted`, gives the later readiness barrier only the positive remainder of the same 300 seconds, and proves request-to-ready `<=300`; it leaves the total start before checkout, endpoint after checksum, P12 `120/180/180/240/720`, selectors, commands, and every protected byte unchanged. K2 is the final support head; no K3 exists. Fresh G0 must prove exact `K1..K2`, clean state, matching pins, absent S1, and one closed ref state: phase=`K1`/main=`R` initially, phase=`K2`/main=`R` after phase advance, or phase/main=`K2` after integration. Require green exact-head phase K2 before main advances and green exact-main K2 before S1. Green exact-main K2 makes it the S0 verification/phase-close and S1 phase-main/card base while I3 remains S0.1 implementation evidence; then create only `phase/s1-shell-design`, hydrate S1.1 with `P=M=K2`, and resume the frozen program. Never amend the completed HANDOFF.
...
I or the one allowed product diagnosed-fix I2. A proven hosted-infrastructure
failure instead uses only the finite §18 K lane and never consumes the I2 budget.
...
continue. Stop on a non-fast-forward, unexpected ref movement, ambiguity,
unresolved input, any product/compiler/test failure, any acceptance failure outside
the exact §18 aggregate-total infrastructure class, or exhausted §18
infrastructure recovery.
Never use local Xcode/Simulator or
