...
The canonical `V4_IMPLEMENTATION_RUNBOOK.md` expands this into **36 strictly ordered Codex coding cards plus two owner-only release gates**: 1+1+2+7+5+4+6+5+4+1 cards across S0–S9. One persistent `/goal` executes the coding program through S9.1 under the frozen phase/branch map. Every card requires accepted implementation evidence and green exact-head verification; every phase requires a verified non-force fast-forward `main` update and green exact-main CI before the next phase branch. S9.2/S9.3 remain owner-only. Missing or ambiguous evidence, temporarily unavailable services, unexpected ref state, compiler/test failure, repeated infrastructure failure, and watchdog expiry keep the current card active for read-only diagnosis, waiting, a fresh-runner dispatch, or a smallest authorized direct-child correction. They do not consume or terminate program authority.
...
or enter S9.2/S9.3. Stop coding after green exact-main S9.1 for the owner-only gates.
```
