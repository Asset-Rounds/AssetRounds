...
The canonical `V4_IMPLEMENTATION_RUNBOOK.md` expands this into **36 strictly ordered Codex coding cards plus two owner-only release gates**: 1+1+2+7+5+4+6+5+4+1 cards across S0–S9. One persistent `/goal` may execute the coding program through S9.1 under the frozen phase/branch map. Every card requires green implementation-SHA CI; every phase requires a verified fast-forward `main` update and green exact-main CI before the next phase branch. S9.2/S9.3 remain owner-only. A failed required run, ambiguous transition, unresolved owner input, unexpected ref movement, or non-fast-forward boundary stops the train except for the sole exact S0 authority-only boundary recovery installed below; its own non-green run stops permanently.
...
One separate bounded S0 phase-boundary recovery is owner-authorized after exact-head S0.1 recovery run `31566371521` succeeded at `I3=ac310dd37700d65165200f742aaec1d48d0a34d6`, HANDOFF-only phase-close `C=15348e92466a79efd02bfb71f5ffee043e615829` was fast-forwarded to `main`, and exact-main run `31567299022` at `C` failed before `xcodebuild` while the hosted Simulator remained in `CoreLocationMigrator`. Starting only when local HEAD, remote phase, and remote `main` all equal `C` and S1 is absent, Codex may create one authority-only `R` directly after `C` changing exactly `AGENTS.md`, this plan, the runbook, and CURRENT_TASK. `R` preserves every product, project, app, test, script, selector, workflow, P12 `120/180/180/240/720` value, program-map entry, prohibition, and completed HANDOFF byte. After a fresh G0 proves `R^=C`, exact `C..R`, repinned authority bytes, clean state, remote phase/main=`C`, and S1 absent, Codex may non-force advance the phase ref `C→R`, then non-force fast-forward `main C→R`. Run `31567299022` is never rerun and is not an `R` candidate. Reuse any exact workflow-dispatch candidate at `ref=main/head_sha=R`, or dispatch exactly once with UI enabled only when none exists. A terminal non-success at `R` stops with no fix, rerun, or further recovery. Only matching success makes `R` the accepted S0 phase-close and S1 phase-main/card base; then Codex may create only `phase/s1-shell-design`, hydrate only S1.1 in CURRENT_TASK from `R`, and resume the frozen program. `R` is never S0.1 implementation evidence and the completed S0.1 HANDOFF is not amended.
...
run, ambiguity, or unresolved input, except only when CURRENT_TASK pins the sole
S0 authority-only boundary recovery above; that recovery's non-green run stops.
