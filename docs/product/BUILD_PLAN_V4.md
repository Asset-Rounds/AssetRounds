...
Standing hosted-CI infrastructure recovery needs no new approval message. Eligibility normally requires complete logs and checksum-verified artifacts proving failure solely in hosted runner provisioning, pinned toolchain/runtime readiness, exact-Simulator boot/migration, GitHub Actions service behavior, or artifact transport before compilation/product tests. Pre-repository provisioning failure may instead use authoritative GitHub metadata plus complete Actions logs proving no repository command ran; upload/transport failure requires logs proving evidence validation and checksum creation succeeded first. Ambiguity is ineligible. For one immutable gate identity (phase, card, workflow, required ref roles, product implementation candidate `E`, first failed run/head), use failed verification head `F` plus at most two linear diagnosed support heads K1/K2. Current owner-directed S0 K1 may install this rule by changing the five authority documents and workflow exactly as CURRENT_TASK names. Of those five authority documents, AGENTS/plan/runbook/execution contract are immutable afterward; CURRENT_TASK remains record/repin-only. Later K may change only a causally required subset of CURRENT_TASK, `.github/workflows/ios-ci.yml`, and the four smoke/timeout scripts, with CURRENT_TASK unable to broaden/reset the rule. Derive K/dispatch counters from reachable commits and GitHub exact-ref/head history.

Every K directly parents the failed head. Preserve HANDOFF, selector, project/app/tests/fixtures/assets, product behavior, selectors, runner/Xcode/Simulator selection, N8/P12/F25 values, existing product/card acceptance, workflow trigger/permissions/action pins, selector/tier/result validation, exact build/test/UI invocations, shell fail-fast/pipefail and timeout/exit propagation, total-budget accounting, and artifact/checksum/upload enforcement. Only causally necessary infrastructure readiness/wait/transport handling and additive nonempty checksummed diagnostics/acceptance may change; existing evidence may not be removed or weakened. G0 proves immutable gate identity, eligible evidence, exact `F..K` allowlisted diff/pins, history-derived counters, and `E..K` protected-tree equality. Pushes are non-force and exact-ref gated; failed run IDs are never rerun. A provider failure before repository execution may consume one same-head redispatch without K; green exact-F then satisfies the unchanged ordinary gate. Each K1/K2 otherwise gets one initial dispatch per required ref and one same-head redispatch only for an identical mechanically eligible infrastructure signature containing no compiler diagnostic, test result/assertion, selector, acceptance, or product failure. A distinct cause may use the next K. If K2 remains non-green after its allowed retry, has an ineligible failure, or encounters ambiguity, ref divergence, an unlisted path, or evidence weakening, stop technically without requesting approval and create no K3. Once K exists, only green exact-K evidence is accepted; record E and K separately and use K for subsequent M/phase P. An open card may exit to unused ordinary I2 if K exposes a product failure; a closed boundary card never reopens.

Per exact workflow/ref/head, candidate history is closed: zero candidates dispatches the initial run; one active waits, one green accepts, and one eligible terminal failure dispatches the sole unused retry. Exactly two are valid only when the first is that eligible failure and the second is active/green; wait for or accept the second. A terminal second run, other order/status, or more than two candidates stops. An authorized retry is therefore not an ambiguous duplicate.

For the current S0 recovery, create one direct-child `K1` from `R` changing exactly `AGENTS.md`, this plan, the runbook, the execution contract, CURRENT_TASK, and `.github/workflows/ios-ci.yml`. The sole operational correction gives exact-UDID Simulator readiness its own fixed 300-second hosted-infrastructure step before the unchanged P12 180-second build wrapper, records a required nonempty `simulator-boot.log`, and leaves P12 `120/180/180/240/720` plus its total budget unchanged. HANDOFF, selector, scripts, project, app, and tests remain byte-identical to `R`. Fresh G0 must prove exact `R..K1`, clean state, matching pins, absent S1, and one closed ref state: phase/main=`R` initially, phase=`K1`/main=`R` after phase advance, or phase/main=`K1` after integration. Push each ref only from its preceding state; otherwise resume its closed candidate history without re-push or duplicate dispatch. Require green exact-head UI-enabled phase CI before the main advance and green exact-main UI-enabled CI before S1. Each ref has only the standing same-head infrastructure retry. Green exact-main `K1` makes it the accepted S0 verification/phase-close and S1 phase-main/card base while `I3` remains S0.1 implementation evidence; then create only `phase/s1-shell-design`, hydrate only S1.1, and resume the frozen program. Never amend the completed HANDOFF.
...
unresolved input, product/compiler/test/acceptance failure, or exhausted §18
