...
Execution route: Codex authors on Windows; ordinary cards run every Xcode build, automated test, and Simulator check on the pinned GitHub-hosted macOS workflow. Only S10.4 may use the exact owner-authorized hybrid Build Hub route after a nonaccepting same-shard GitHub-to-Bitrise exact-environment equivalence gate: Bitrise may supply formal evidence only for current-runtime `default-light` and `default-dark`, while GitHub remains the formal route for every minimum-profile shard, including iPhone SE (3rd generation) / iOS 18.0 build `22A3351`. That route reuses one exact-head `build-for-testing` payload only when its complete immutable `xctestrun` plus `Build/Products` closure and sorted checksum manifest verify before every `test-without-building` consumer; rebuild, fallback, mixed-head payload, and cross-machine Simulator/evidence reuse are forbidden. Provider-local screenshots remain independently checksummed and reviewable; screenshot byte equality across providers is not asserted. Hardware/release verification runs through TestFlight on a physical iPhone. V4's minimum deployment target is iOS 18.0; changing it requires a plan revision. “Local-first” describes app data, not the build computer. No owner-operated Mac, local Xcode, local Simulator, XcodeBuildMCP, or local signing identity is required.
...
S10.4 owner amendment H413 (2026-09-09) supersedes only H411's fourteen-profile acceptance cardinality for the selected S10.4 card. The complete fourteen-profile catalog remains available and runnable with 938 visual slots and 84 accessibility rows. Formal S10.4 acceptance requires exactly nine profiles: all seven current profiles plus minimum-runtime `minimum-os` and `double-length`, totaling 603 visual cells and 54 accessibility rows. Minimum `rtl`, `rtl-string`, `tall`, `accented`, and `bounded` remain preserved as 335 visual cells and 30 accessibility rows with explicit `DEFERRED` disposition, never `PASS` or `NOT_APPLICABLE`. Each required profile still closes all 67 states and all six continuously executed critical journeys with strict native AX/contrast/geometry, only the existing narrow exceptions, a qualified immutable payload and all five native producer methods, complete original integrity, and genuine human visual review. Preserve every prior run, failure, artifact, test, fix, audit, approved use right, frozen package/baseline byte, and available catalog entry. Deferral cannot conceal a demonstrated product defect affecting retained coverage. This selected-card amendment changes no product behavior, iOS 18.0 minimum deployment target, runtime or provider truth, physical-device proof, release gate, signing, upload, or submission authority.
...
- Monthly product: `com.palatis3.fieldrecord.sub.solo.monthly.v1`.
...
- Layout and text support every system Dynamic Type category; required actions may reflow/scroll but never clip or disappear. S8.2 exercises default and largest accessibility layouts in CI, S10.4 repeats the branded automated matrix, and S10.5 records the owner-bridged physical-iPhone golden flow rather than claiming a full device matrix.
...
### S10 AssetRounds brand-system refresh

After the accepted unsigned S9.1 candidate and before owner-operated release, S10 may replace that unpublished candidate with the reviewed **AssetRounds Brand Handoff V4.1** visual system. This is a presentation, identity, accessibility, and release-evidence phase—not a new feature phase. It preserves every S0–S9 behavior, navigation route, persistent model and byte contract, privacy boundary, commerce rule, supported-device rule, report meaning, backup/restore rule, and device-local truth.

The permanent master identity is **AssetRounds**, the broad descriptor is **Field Inspections**, and the current vertical/App Store title remains **AssetRounds: Sign Inspection** pending the existing professional-clearance gate. The approved mark is the open orbit, center asset, and green waypoint from the frozen V4.1 package. Runtime integration uses exactly the package's three AppIcon appearances and two original/template symbol image sets. Source-reference boards, research, scorecards, manifests, validators, and rejected experiments never enter the app bundle.

S10 refines Worklight Precision through semantic system surfaces plus the reviewed brand values: accent teal `#006D75` light / `#2BB8C2` dark, deep teal `#0B4E53` light / `#8ADDE3` dark, checkpoint green `#147D47` light / `#53D78B` dark, limited brand canvas `#F3F9F9` light / `#061E26` dark, ink `#11181C` light / `#F7FAFA` dark, and slate `#47565D` light / `#B8C5C8` dark. Warning/error remain system semantic colors, no condition is color-only, evidence stays untinted and aspect-correct, live text remains Dynamic Type/localizable, and familiar actions remain native controls/SF Symbols. Standard SwiftUI controls may adopt the current OS appearance; custom decorative glass, branded camera/share/settings replacements, ornamental textures, and motion that survives Reduce Motion are forbidden.

Brand evidence follows six separately accepted cards: S10.1 inventory/activation, S10.2 component system plus the exact 12 runtime assets and AppIcon setting, S10.3 full released-screen migration, S10.4 automated appearance/accessibility/localization lab, S10.5 owner-bridged physical experience, and S10.6 release/store/privacy/legal evidence lock. Each stage distinguishes accepted product head E, descendant evidence head K, and later receipt/bookkeeping C. A plan or package score never substitutes for exact CI, human visual review, physical proof, rights attestation, or trademark clearance.
...
Each S-row is one ordered **phase train** containing the named Codex cards. One persistent `/goal` may complete all authorized coding phases through S10.6, but exactly one card is active at a time. Every card still owns one observable result plus at most one bounded alternate/failure family and requires exact implementation evidence plus successful exact-head CI. Each phase freezes main-base SHA `P` (`B` for S0, prior accepted exact-main verification/phase-close SHA thereafter). After a green card, phase autopilot prepares only the immediate next same-phase card using HANDOFF plus CURRENT_TASK and fresh G0. After a green boundary card, program autopilot creates phase-close `C`, verifies the phase, advances `main` only by a non-force fast-forward from `P` to the exact green phase head (or resumes when already exact), requires UI-enabled exact-main CI, and creates only the frozen next phase branch before fresh G0. Recovery is persistent: one CI candidate runs at a time, terminal evidence is diagnosed, and Codex either dispatches a fresh-runner candidate at the same exact head or creates the smallest direct-child product or CI-harness correction and verifies again. No candidate count, correction count, repeated failure signature, or watchdog expiry consumes authority. Product scope, card/phase order, selectors, acceptance, evidence, and external-mutation prohibitions remain closed. The private solo repository needs no server branch protection under this exact-ref state machine.

| Phase | Ordered task-sized units and exact terminal boundary | Owned launch smokes |
|---|---|---|
| S0 Contract/repo lock | `S0.1` creates the checked-in project/shared scheme, synchronized source groups, minimal app/test scaffold, build/test/UI scripts, `run-with-timeout.sh`, and exact-schema `Scripts/ci-selection.json`; CI validates the tier/input, enforces non-accepting watchdogs, and fails closed on missing evidence. Green unsigned implementation-SHA build/install/launch is the boundary. Bootstrap `B` is S0's immutable initial phase-main/card base. Accepted product implementation `I3` remains product evidence. K1 and K2 remain failed history. K3 `e569951909c2ee8aff4253bae0d442550c229a16` passed exact-head phase run `31590956549`; exact-main run `31591804256` failed only the historical 300-second Simulator-readiness watchdog. Direct-child K4 replaces the exhausted-stop policy and watchdog contract without changing product/project/test bytes, then persistent exact-head phase/main verification continues until green. The first green exact-head current support head becomes the accepted S0 verification/phase-close head and S1 base. | baseline |
...
| S9 Unsigned candidate | `S9.1` freezes the unsigned-CI-verified RC/privacy/metadata package and inactive owner-only TestFlight workflow; no upload. Its accepted exact-main head is the base for the owner-approved unpublished-candidate brand replacement. | 12 plus candidate evidence |
| S10 Brand refresh | `S10.1` freezes activation, complete released-screen/state inventory, S9 baselines, common tasks, budgets, and store plan. `S10.2` integrates the exact runtime brand assets, AppIcon setting, semantic tokens, and reusable components in isolation. `S10.3` migrates every frozen released state without behavior change. `S10.4` closes automated Light/Dark/contrast/AX/pseudolanguage/Reduce Motion/Reduce Transparency/minimum-OS evidence. `S10.5` closes owner-bridged physical accessibility, field, durability, and performance evidence. `S10.6` locks store/privacy/supply-chain/legal evidence for the final branded candidate. Only afterward may owner S9.2/S9.3 sign/upload/submit. | visual/accessibility/store evidence plus owner physical proof |

The canonical `V4_IMPLEMENTATION_RUNBOOK.md` expands this into **42 strictly ordered Codex coding cards plus two owner-only release gates**: 1+1+2+7+5+4+6+5+4+1+6 cards across S0–S10. One persistent `/goal` executes the coding program through S10.6 under the frozen phase/branch map. Every card requires accepted implementation evidence and green exact-head verification; every phase requires a verified non-force fast-forward `main` update and green exact-main CI before the next phase branch. S10 additionally records product E, descendant evidence K, and later receipt C without self-reference. S9.2/S9.3 remain owner-only after S10.6. Missing or ambiguous evidence, temporarily unavailable services, unexpected ref state, compiler/test failure, repeated infrastructure failure, and watchdog expiry keep the current card active for read-only diagnosis, waiting, a fresh-runner dispatch, or a smallest authorized direct-child correction. They do not consume or terminate program authority.
...
| S7 | Use the frozen monthly product ID/group/14-day-trial fixture, fixed 16-day paid-to-paid grace, and Family Sharing off; the checked-in local `.storekit` fixture is CI-only. Deterministic tests use reserved nonresolving HTTPS fixtures `https://example.invalid/terms`, `https://example.invalid/privacy`, and `https://example.invalid/support`; production configuration remains absent and fails closed until S9 release inputs exist. Unsigned local StoreKit implementation needs no Paid Apps Agreement, bank/tax completion, or active App Store product. Product views never hardcode price or eligibility. |
| S9 | S9.1 records a complete pending/provided release-input manifest and builds every repository-derived unsigned RC/privacy/metadata/workflow artifact. Before S9.2, the owner supplies Apple Account/Developer Program access; title clearance; six-of-ten commitment evidence before production SKU creation/activation; Paid Apps Agreement/bank/tax and verified Small Business Program status; live monthly product/group/storefront/14-day offer/grace configuration; owner-controlled domain/support email/privacy/terms/support pages; final App Store record; GitHub `app-store-connect` environment secrets on a supported repository/plan posture; owner-only manual dispatch restricted to the reviewed exact `main` SHA under the private-solo ref rule; and signing/App Store Connect credentials. Do not claim a private-repository environment required reviewer unless the selected plan supports it. Pending live values block S9.2, not completion of the unsigned S9.1 coding artifact. |
| S10 | Freeze the reviewed Brand Handoff V4.1 ZIP and manifest digests, unpublished-candidate route, exact S9 base/run evidence, six-card authority, current/minimum device profiles, release locale and six pseudolanguage profiles, accepted owner use grant, and an owner-operated physical distribution bridge. S10.1–S10.4 continue without signing. S10.5 requires the owner to install the exact product E on physical hardware through the frozen bridge and return installation/accessibility/field/durability/performance evidence. S10.6 remains blocked until separate dated trademark/name/claim/URL clearance and every release hard gate are supplied. Codex never fabricates or performs the owner signing/upload/submission step. |
...
3. **Independently reviewed executable device-local vertical slice** — force-quit, storage failure, whole-sign delete/Erase All, report replacement, StoreKit sandbox, the named offline entitlement smoke, the second-pack fixture, and the branded S10 physical golden flow are launch evidence. Forced media-loss/corruption, derived-projection failure, security/abuse exercises, schema migration, and old/new report/portable-archive/backup compatibility are separately authorized post-V4 evidence tasks, not hidden S0–S10 launch scope. The nonshipping exterior-light fixture must run through the same pack/runner/snapshot/renderer interfaces with zero new production entity, route, service, permission, or renderer branch. This proves the bounded content-pack seam for nouns, copy, evidence purposes, and issue labels; it does not claim a generic workflow or untested observation engine. A partial spike informs the build but cannot release the architecture evidence cap.
...
The mandatory installed companions are `../execution/CODEX_EXECUTION_CONTRACT_V4.md` and `../execution/V4_IMPLEMENTATION_RUNBOOK.md`. The repository receives its short root `AGENTS.md`, one current-card `CURRENT_TASK.md`, a known-bug register, the supplied unsigned CI workflow, and small smoke scripts. The runbook is a catalog, not blanket product authority: `CURRENT_TASK.md` pins one selected card plus the immutable program/phase map and transition/integration state. Authorized execution includes exact phase-branch work, CI/evidence, same-phase transition, non-force fast-forward `main` integration, exact-main CI, and creation/hydration of only the frozen next phase branch. A preflight inconsistency is resolved by rereading, fetching, waiting, gathering diagnostics, or an authorized mechanical correction; it is not an approval-message gate. Full access never expands product scope. S10 evidence heads follow the frozen E/K/C path without changing product bytes after E. Signing, TestFlight upload, deployment, App Store mutation, and S9.2/S9.3 remain owner-only.

No additional owner message is required for implementation or verification recovery inside the current card and frozen program order. Candidate history is append-only with no retry or cardinality ceiling. If an exact workflow/ref/head candidate is green with complete checksum-verified evidence, accept it. If one is queued or running, wait. Otherwise inspect all terminal candidates and artifacts, then dispatch exactly one new `workflow_dispatch` candidate on a fresh runner at the same branch ref, exact head, inputs, pinned runner/toolchain/Simulator selector, and tier. Never use Actions rerun on a failed run ID. Repeat until green or until evidence identifies a repository correction. Recurrence, overlapping signatures, missing evidence, and previous repair labels are audit provenance, not consumption gates.

For a diagnosed product, compiler, test, or card-acceptance failure, create one direct-child task-scoped correction commit changing only the current card's allowed product paths and required authority evidence/pins, then verify again. Successive implementation heads are `I`, `I2`, `I3`, and so on with no numeric ceiling. The first exact-green head with complete evidence becomes accepted product implementation `E`. A correction never weakens outcome, GOLDEN, ALT-1, selector semantics, tests, evidence, or acceptance. During S8.1 or S8.2, keep that S8 card active; a proof-discovered production defect may create a direct-child correction limited to the exact earlier-card-owned paths that introduced the defect, and the unchanged S8 selector must exercise the corrected seam before the S8 proof reruns. Card order and selector identity stay fixed rather than temporarily reopening an earlier card or turning verification into opportunistic redesign.

For a diagnosed hosted-runner, pinned-toolchain/runtime, exact-Simulator, Actions service/transport, or lane-harness failure, either redispatch the same exact head on a fresh runner or create one direct-child `K(n+1)` containing one minimal diagnosed correction. K may change only required CURRENT_TASK evidence/pins and the smallest causally required subset of `.github/workflows/ios-ci.yml`, `Scripts/build-smoke.sh`, `Scripts/test-smoke.sh`, `Scripts/ui-smoke.sh`, and `Scripts/run-with-timeout.sh`. Preserve product/project/test/fixture/asset bytes, selector semantics, pinned environment, exact product commands/destinations, timeout exit behavior, evidence/checksum/upload enforcement, and raw aggregate accounting. Successive K heads may refine the same mechanism; hashes, trailers, failure records, and repair records remain provenance and never consume authority.

CI watchdogs are exact and non-accepting, but expiry feeds the same diagnosis/retry/correction loop rather than ending authority. Simulator readiness is `900` seconds and the job watchdog is 90 minutes; workflow concurrency never cancels an in-progress candidate. Tier IDs remain compatibility labels rather than literal minute counts. Tier values are setup/build/unit/UI/total seconds: N8 `300/600/900/0/2400`, P12 `300/600/900/900/3300`, and F25 `300/900/1200/1800/4500`. Artifact upload remains after the raw setup-through-checksum total and within the job watchdog. Missing evidence or exceeded watchdogs keep a candidate non-green. They never justify weakening a selector, product command, assertion, checksum, or required artifact.

CURRENT_TASK preserves ordered ref roles and exact accepted predecessors. At a boundary, verify phase CI first, re-fetch immediately before mutation, and advance phase/main only by verified non-force fast-forward to the exact green verification head. If a ref already equals that head, resume idempotently. Unexpected movement triggers read-only ancestry/provenance inspection and either waiting or recreation of unaccepted work as a direct child of the valid lineage; never force-push, create a merge commit, or silently reconcile unrelated history. If exact-main verification exposes a product failure before the next phase starts, the boundary card stays active for successive scoped corrections and fresh phase/main verification. Prior HANDOFF entries remain append-only.

Historical S0 evidence remains factual. Accepted product implementation `I3=ac310dd37700d65165200f742aaec1d48d0a34d6` and run `31566371521` remain product evidence; HANDOFF-only `C=15348e92466a79efd02bfb71f5ffee043e615829`, authority head `R=1e0322cc072fda28718468bcd1b0dbe1f5d0accc`, K1 `0da62482db7e3f90742876ef05574de9e9569e32`, K2 `400f8827f5451b9a44f4d846340b51377554069d`, and their failed runs remain immutable provenance. K3 `e569951909c2ee8aff4253bae0d442550c229a16` passed exact-head phase run `31590956549`. Exact-main K3 run `31591804256` failed only the historical 300-second readiness watchdog after the unchanged product build passed. Direct-child K4 installs this persistent policy and the new watchdog values; it changes no protected product/project/test bytes. Exact-head phase and main verification then continue through the same loop until green; accepted verification head K becomes S0 phase close and S1 `P=M`, while I3 remains product implementation evidence.

Program hydration uses already frozen facts rather than asking again: S7 uses product ID `com.palatis3.fieldrecord.sub.solo.monthly.v1`, a 14-day introductory-offer fixture, 16-day paid-to-paid grace, Family Sharing off, and the three reserved `example.invalid` HTTPS fixtures named in the owner-preparation table. S9.1 may prehydrate app/test bundle IDs and the frozen product ID. S10 prehydrates the owner-approved V4.1 ZIP/manifest digests, unpublished-candidate replacement route, exact S9 base and green phase/main runs, palette, runtime asset allowlist, current/minimum device profiles, release locale, six pseudolanguage profiles, owner use grant, and planned owner-operated physical bridge. The adopted title still requires its stated professional clearance before it becomes final metadata. Unspecified live URLs, support email, title/trademark clearance, App Store record, version/build, privacy answers, credentials, signing, physical observations, or release state are never invented; they remain explicit release inputs, while Codex continues every independent S9.1 or S10 artifact and validation it can derive from repository truth. S9.1 and S10 create no production SKU, secret, signing identity, upload, deployment, or submission.

Recommended opening in the Codex task:

```text
/goal Complete every remaining coding card through S10.6 in strict runbook and
frozen phase-map order. Do not enter /plan, combine, skip, pre-implement, or add
adjacent product scope. Run fresh G0 for each card, implement only its frozen
outcome/GOLDEN/ALT-1, and accept only green exact-head CI with complete evidence.
Use one CI candidate at a time. On terminal failure, diagnose logs/artifacts and
either dispatch a fresh-runner candidate at the same exact head or create the
smallest direct-child current-card/CI-harness correction; repeat without a retry,
I, or K ceiling. After green evidence append HANDOFF, hydrate only the immediate
next card, and continue. At every boundary non-force fast-forward main to the exact
green phase head, require exact-main CI, create only the frozen next phase branch,
and continue. Never weaken acceptance, force-push, sign, upload, deploy, submit,
or enter S9.2/S9.3. Stop coding after green exact-main S10.6 for the owner-only gates.
```
...
S10.4 owner amendment H418 (2026-09-10) supersedes H413's required-nine acceptance cardinality prospectively for this selected card. Require all seven current-runtime profiles as full logical shards, totaling 469 visual cells and 42 automated accessibility rows, with all 67 states, all six continuous common-task journeys, strict native AX/contrast/geometry, complete original integrity, and genuine human review. Preserve the fourteen-profile/938-cell/84-row catalog while all seven minimum full profiles remain explicit `DEFERRED` coverage totaling 469 cells and 42 rows. Double is nonblocking. Separately require the six-checkpoint `s10.4.minimum-core-smoke.v1` ordinary iOS 18.0 functional smoke. It receives zero catalog-cell/row credit, never claims full-shard/full-segment closure, and adds no human-approval gate. Its successful native result, seven screenshots, public functional assertions, exact source/run/producer/payload/expiry/command/checkpoint/attachment bindings and complete checksummed originals are mandatory. PDF preview plus an available Share PDF control is the complete share claim; no share-sheet, export-byte or delivery claim follows. The deployment target, product behavior, provider/runtime truth, physical proof and release authority remain unchanged.
