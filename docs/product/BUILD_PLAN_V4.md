...
Standing hosted-CI infrastructure recovery needs no new approval message. An exact terminal non-green run is eligible only when complete logs and checksum-verified artifacts prove its first causative failure is hosted-runner provisioning, pinned toolchain/runtime readiness, exact-Simulator readiness, Actions service/transport, or lane-owned harness scheduling/accounting, with no compiler diagnostic, product test result/assertion, selector mismatch, or product failure. Pre-repository failure may use authoritative metadata plus complete logs proving no repository command ran. Upload failure requires logs proving validation and checksum creation succeeded first. The sole legacy no-manifest exception is exact K2 phase run `31583957094`/head `400f8827f5451b9a44f4d846340b51377554069d`/artifact `9136413551` digest `sha256:284632a9587fa296a36b1c57c9e9320779f022a4d68b8ff5a592305105bfaedd`; it authorizes K3 only and is consumed. K3 changes evidence ordering so every later post-checkout classification requires checksum-verified evidence. Budget eligibility is closed: setup `<=120` may authorize a correction only as the first failure before product commands, and aggregate total may authorize one only after every constituent/product/selector/evidence/checksum/upload gate passed. Build/unit/UI ceiling failures and every other acceptance failure are ineligible. All failed assertions remain non-green; ambiguity is ineligible.

For immutable gate identity phase/card/workflow/ordered-ref-roles/product head E/first-failed-run+head, support heads form a direct-child monotonic sequence `K1,K2,...`. There is no numeric ceiling or approval gate. Before each next K, fresh G0 derives the index from reachable lineage plus all exact-ref/head candidates; freezes separate canonical gate/failure/repair records under AGENTS' literal grammar and closed registries; hashes them; freezes one diagnosis and one minimal delta; proves the new K directly parents failed head F; and proves no prior K consumed the same/overlapping operation+mechanism or repair key. Volatile ref/run/head/display/time/UDID/duration/log/cascade values are provenance only and cannot mint a failure. CURRENT_TASK cannot reset/relabel history. K3 and later commits carry exact gate/index/failed-run/head/ref/failure/repair trailers.

One K contains only mechanical CURRENT_TASK evidence/pins plus exactly one operational correction across the smallest causally required subset. The current owner-directed K3 conversion is the sole installer exception: it may amend AGENTS, plan, runbook, execution contract, CURRENT_TASK, workflow, and `Scripts/build-smoke.sh` once to install the static schema/refreeze, non-accepting failure-evidence ordering, and exactly one K2 causal correction. After K3, the four static authority sources and all four smoke/timeout scripts refreeze. Later K may use only CURRENT_TASK and marker-delimited infrastructure-recovery bodies in the workflow, and the one-delta/no-adjacent-hardening rule is absolute. Preserve HANDOFF, selector, product/project/tests/fixtures/assets, runner/Xcode/Simulator selection, N8/P12/F25 values, trigger/permissions/action pins, exact build/test/UI commands/selectors/destinations/results, fail-fast/timeout/exit behavior, evidence/checksum/upload enforcement, and raw aggregate start-before-checkout/end-after-checksum. Readiness stays `<=300` and inside raw total. G0 proves the direct delta, cumulative protected-tree/workflow-spine equality, and attribution of every earlier lane hunk to one unique failure and repair.

Per exact workflow/ref/head, zero candidates dispatches once; one active waits; one green accepts. One terminal failure chooses either one same-head fresh-runner candidate only for pre-repository provider or post-checksum upload/transport failure with no indicated repository delta, or the next K for a distinct eligible failure/repair with one unique correction. Retry consumption is gate-wide: before the no-code retry, canonicalize an unconsumed failure/repair; the first terminal candidate plus exact GitHub receipt consumes both across every ref, and its retry is their sole disposition. GitHub history is immediately authoritative and CURRENT_TASK mirrors it only at the next otherwise-authorized commit. Setup/aggregate failures never redispatch same-head. Creating K retires all unused dispatch/retry authority at F. Exactly two candidates are valid only as that consumed provider/transport failure followed by active/green/terminal; green accepts, while terminal may create another K only for a distinct eligible failure/repair. Any consumed failure/repair recurrence on any ref, a second retry, ambiguity/ineligibility, another ordering, or more than two candidates stops technically. CURRENT_TASK freezes ordered ref predecessors/cursor; each role moves once by non-force fast-forward only after earlier roles are green, and an already-moved exact role resumes without re-push. Failed run IDs are never rerun. Accept only green exact-K complete artifacts; E remains product evidence and accepted K becomes verification/next base.

For current S0, K1 `0da62482db7e3f90742876ef05574de9e9569e32` and K2 `400f8827f5451b9a44f4d846340b51377554069d` remain failed. K1 run `31579936670` failed only aggregate `727 > 720` after every constituent/product/evidence/checksum/upload gate passed. K2 phase run `31583957094` first failed setup `232 > 120` after its boot request put exact-Simulator readiness on setup's critical path; build/unit/UI did not run, downstream evidence/checksum failures were deterministic, total `233 <= 720`, and upload succeeded. It is never accepted or redispatched. Direct-child K3 changes exactly the seven conversion paths, restores inspection/selection/setup before readiness starts, runs the bounded exact-UDID readiness step in GitHub's tracked background mode concurrently with the unchanged exact-destination build, removes only build-smoke's redundant wait, and explicitly joins readiness before tests. The independent 300/180 ceilings and raw total remain literal; all P12 values, selector, aggregate endpoints, product/tests, and exact xcodebuild invocation remain unchanged. Require green exact-head phase K3 before main advances. The first current K at index 3 or later with green phase and exact-main CI becomes S0 phase-close/S1 `P=M` while I3 remains S0.1 product evidence. A distinct eligible later failure/repair may create the next K mechanically; a repeated/overlapping failure or repair or any other technical stop does not request authority text. Never amend the completed HANDOFF.
...
failure instead uses only the monotonic evidence-bounded §18 K lane, needs no further approval message, and never consumes or resets I2.
...
unresolved input, any product/compiler/test failure, ineligible or ambiguous
infrastructure classification, a repeated or overlapping consumed failure/repair,
absence of one unique allowed-path delta, invalid ref/candidate
provenance, or another technical lane stop. A numeric K index is never a stop.
