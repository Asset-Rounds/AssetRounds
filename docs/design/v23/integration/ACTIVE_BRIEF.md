...
Current d2dd964 original35552497922 is terminal, sole-collected and root-audited F844041B: compilationPASS,13executed/2Pass/11Fail,0Swift errors/new warning signatures, diagnostic transportPASS. No active native run or collector. Retained native logs distinguish seven startup Erase errors from production/reminder fresh-binding StoreMigrationFailure. Prior restore35540673966 remains2/7 and reminder35540763669 remains32/33 on their original heads. No merge/release acceptance.

Next causal batch runtime-d2dd964/erase-causal corrects the source-proven deletion of the sole current manifest with Operations, using an authenticated same-inode handoff under surviving Data and constructor restoration before normal consumers. Pair fixture directory/bundle corrections, golden observation ordering and default-off fixed phase evidence with cold/interruption/hostile regressions. Working drafts remain exact. Source/protocol checks, one medium complete-contract review and actual-index/committed-route gates remain required before a unique native original; hosted compilation/runtime of this repair are unverified. Existing support qualification for d2dd admitted unchanged product trees only: qualify changed-source bindings and any new closed test selection before dispatch. Restore7 diagnostics remain separately unexecuted.

## Current implementation

- Destination service/AppAccess resolution and confirmed discard are committed at2e60630. Production non-photo Replace/Fork review composition is committed at4971f79 through the existing writer with a private synchronous staging capability; genuine physical package/history, exact-owner suppression, receipt readback and prepublication fences. All six restore journeys and seven authority methods remain required. Photo cross-workspace correspondence, UI/restore/lifecycle/registration remain due;49 drafts exact.
- Photo registration still requires complete lifecycle/destination closure; retained source and evidence are in CURRENT_INTEGRATION.
...
- Begin history authenticates real PREPARED prefixes and BOUND originals, rejecting quarantines, missing required timezone and omitted zero-photo parents. The77-input assessment copy and legacy fixture are retained;5d originals are sealed, while insert rejection remains unresolved.
