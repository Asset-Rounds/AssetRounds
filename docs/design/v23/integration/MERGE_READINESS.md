...
- Made unreachable before Phase 1 freeze: the App Intents (built into Release, runtime never installed), the Today Plan sheet and My Day actions, the dormant Work round rows and saved-review sheet. Reminder settings stay reachable only if their 28-method same-head set and UI evidence pass; otherwise they are hidden too.
