...
try:
 ...
 if a.check:
  if bad:raise ValueError('drift:'+','.join(bad))
  print('P06-C06 generator check PASS_STATIC_PROVISIONAL')
 else:
  for k,v in d.items():t=c.ROOT/k;t.parent.mkdir(parents=True,exist_ok=True);t.write_bytes(v)
  print('P06-C06 generator apply PASS_STATIC_PROVISIONAL generated='+str(len(d)))
except Exception as e:print('P06-C06 generator FAIL_STATIC:'+str(e),file=sys.stderr);raise SystemExit(1)
