#!/usr/bin/env python3
"""Deterministic generator for C04 tooling artifacts."""
from __future__ import annotations
import argparse,os,sys
from pathlib import Path
os.environ["PYTHONDONTWRITEBYTECODE"]="1";sys.dont_write_bytecode=True
ROOT=Path(__file__).resolve().parents[2];sys.path.insert(0,str(Path(__file__).resolve().parent));import p04_c04_contracts as c
def main():
 ...
 if a.check:print("C04 generator check "+("PASS_STATIC_SEALED" if c.FINAL_HASHES_SEALED else "PASS_STATIC_PROVISIONAL"));return 0 if not stale else 1
 for x,b in out.items():q=ROOT/x;q.parent.mkdir(parents=True,exist_ok=True);q.write_bytes(b)
 print("C04 generator apply "+("PASS_STATIC_SEALED" if c.FINAL_HASHES_SEALED else "PASS_STATIC_PROVISIONAL"))
