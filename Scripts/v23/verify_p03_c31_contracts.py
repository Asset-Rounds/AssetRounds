...
def main()->int:
 ...
 result={"cardID":c.CARD,"result":"PASS" if not fail else "FAIL","complete":a.complete,"persistencePinsPending":c.PERSISTENCE_PINS_PENDING,"fencePathCount":208,"existingPathCount":194,"newPathCount":14,"authorizedOverlapCount":2537,"unauthorizedOverlapCount":0,"failures":fail}
 print(json.dumps(result,indent=2,sort_keys=True) if a.json else f"C31 verifier {result['result']}");return 0 if not fail else 1
