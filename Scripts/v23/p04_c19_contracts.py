...
def semantics(ready):
 ...
 exact=("expectedByteLength == contentBinding.byteLength","expectedSHA256 == contentBinding.contentSHA256","state != .openable || exactBytes","current == request.exactPlanRevision ? .current : .historic","never substituted for the requested value","disposition != .approvedActivated || receipt?.decision == .approved","(disposition == .pending) == (receipt == nil)")
