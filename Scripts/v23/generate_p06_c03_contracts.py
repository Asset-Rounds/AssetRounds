...
try:
 d={k:c.pretty(v)for k,v in c.docs().items()};stale=[k for k,v in d.items()if not(c.ROOT/k).is_file()or(c.ROOT/k).read_bytes()!=v]
 if a.check:
  if stale:raise ValueError('artifact drift:'+','.join(stale))
  print('P06-C03 generator check PASS_STATIC_PROVISIONAL')
 else:
  for k,v in d.items():t=c.ROOT/k;t.parent.mkdir(parents=True,exist_ok=True);t.write_bytes(v)
  print('P06-C03 generator apply PASS_STATIC_PROVISIONAL generated='+str(len(d)))
except Exception as e:print('P06-C03 generator FAIL_STATIC:'+str(e),file=sys.stderr);raise SystemExit(1)
