#!/usr/bin/env python3
"""Generate or check canonical V23-P01-C06 tooling artifacts."""
...
from p01_c06_contracts import ContractError, all_outputs


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--apply", action="store_true", help="write canonical artifacts")
    mode.add_argument("--check", action="store_true", help="verify canonical artifacts are current")
    ...
    try:
        ...
        print(f"V23-P01-C06 generation failed: {error}", file=sys.stderr)
        return 1
