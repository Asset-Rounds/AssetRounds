...
def main()->int:
 ...
 if a.check:
  if stale:print("C30 generator stale: "+",".join(stale));return 1
  print("C30 generator check PASS (5 artifacts)");return 0
 for path,data in outputs.items():(ROOT/path).parent.mkdir(parents=True,exist_ok=True);(ROOT/path).write_bytes(data)
 print("C30 generator wrote 5 artifacts");return 0
