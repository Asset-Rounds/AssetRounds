"""Generate the five deterministic V23-P04-C35 tooling artifacts."""
...
def main() -> None:
    ...
    drift = [
        ...
        if not (contracts.ROOT / path).is_file()
        or (contracts.ROOT / path).read_bytes() != contracts.pretty(value)
    ]
    if args.check:
        if drift:
            raise SystemExit("C35 artifact drift: " + ",".join(drift))
        print("C35 generator check PASS_STATIC_PROVISIONAL generated=5")
        return

    _write_atomically(documents)
    print("C35 generator apply PASS_STATIC_PROVISIONAL generated=5")
