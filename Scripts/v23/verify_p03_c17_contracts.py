#!/usr/bin/env python3
"""Verify C17's static corpus, complete hydrated fence, and evidence artifacts."""
...
def _check_row(row: dict[str, Any], relative: str, rendered: dict[str, bytes], failures: list[str]) -> None:
    ...
    if relative in rendered:
        ...
        _assert(
            ...
            f"row generated digest:{relative}",
            failures,
        )
        return
    if path.is_file():
        ...
        _assert(
            row.get("bytes") == len(raw) and row.get("sha256") == contracts.sha256_bytes(raw),
            f"row worktree digest:{relative}",
            failures,
        )
        return
    ...
    _assert(
        row.get("bytes") == 0 and row.get("sha256") == contracts.sha256_bytes(b""),
        f"row missing digest:{relative}",
        failures,
    )
...
def main() -> int:
    ...
    _assert(not unowned_changed, "changed path outside full C17 fence", failures)
    if args.complete:
        _assert(not missing_required_changed, "required changed path missing from full C17 fence", failures)
    _assert(len(contracts.PATH_FENCE) == 40, "path fence count", failures)
    _assert(len(contracts.EXISTING_PATHS) == 26, "existing path count", failures)
    ...
    _assert(
        contracts.FENCE_DIGEST == "b765e7b546e1cf125b8d5d1af881158aa2ad95f7db34b2104a64bdc69e5c12ad",
        "fence digest authority",
        failures,
    )
    _assert(contracts.PRIOR_FENCE_PROOF["overlapCount"] == 269, "prior overlap count", failures)
    _assert(sum(row["overlapCount"] for row in contracts.PRIOR_FENCE_OVERLAPS) == 269, "prior overlap row total", failures)
    ...
    for row in source_rows:
        ...
        _assert(
            row["bytes"] == len(base_raw) and row["sha256"] == contracts.sha256_bytes(base_raw),
            f"source digest:{row['path']}",
            failures,
        )
    ...
    _assert(contract.get("artifact") == "V23P03C17IntegrationEventProjectionContractV1", "contract artifact", failures)
    _assert(
        contract.get("status") == "PASS_STATIC_PROVISIONAL" and contract.get("verificationMode") == "STATIC_ONLY",
        "contract status/mode",
        failures,
    )
    _assert(contract.get("authority", {}).get("cardID") == contracts.CARD, "contract card", failures)
    ...
    _assert(
        contract.get("authority", {}).get("coordinationProjectionDigest") == contracts.COORDINATION_PROJECTION_DIGEST,
        "contract projection digest",
        failures,
    )
    _assert(
        contract.get("authority", {}).get("hydrationTransitionDigest") == contracts.HYDRATION_TRANSITION_DIGEST,
        "contract transition digest",
        failures,
    )
    _assert(contract.get("authority", {}).get("coordinationCASSequence") == 229, "contract hydration sequence", failures)
    _assert(contract.get("authority", {}).get("allowedPathCount") == 40, "contract authority path count", failures)
    _assert(contract.get("authority", {}).get("existingPathCount") == 26, "contract existing path count", failures)
    _assert(contract.get("authority", {}).get("newPathCount") == 14, "contract new path count", failures)
    _assert(contract.get("authority", {}).get("persistentChangeMode") == "DERIVED_ONLY", "contract derived mode", failures)
    _assert(contract.get("authority", {}).get("persistentContractSchema") == "IntegrationProjectionSchemaV1", "contract schema mode", failures)
    _assert(contract.get("authority", {}).get("schemaBehaviorDelta") is False, "contract schema migration delta", failures)
    _assert(contract.get("authority", {}).get("migrationBehaviorDelta") is False, "contract migration delta", failures)
    _assert(contract.get("authority", {}).get("backupBehaviorDelta") is False, "contract backup delta", failures)
    _assert(contract.get("authority", {}).get("restoreBehaviorDelta") is False, "contract restore delta", failures)
    _assert(contract.get("authority", {}).get("deleteBehaviorDelta") is False, "contract delete delta", failures)
    _assert(contract.get("authority", {}).get("exportBehaviorDelta") is False, "contract export delta", failures)
    ...
    _assert(contract.get("requiredLifecycle") == list(contracts.LIFECYCLE_DIMENSIONS), "contract lifecycle coverage", failures)
    _assert(contract.get("requiredSemantics", {}).get("requiredBehaviors") == list(contracts.REQUIRED_BEHAVIORS), "contract behaviors", failures)
    ...
    _assert(
        evidence.get("result") == "PASS_STATIC_PROVISIONAL" and evidence.get("verificationMode") == "STATIC_ONLY",
    ...
    semantics = {
        ...
        "visibilityClasses": list(contracts.VISIBILITY_CLASSES),
        "sensitivityClasses": list(contracts.SENSITIVITY_CLASSES),
        "redactionModes": list(contracts.REDACTION_MODES),
        "eventKinds": list(contracts.EVENT_KINDS),
        "checkpointStates": list(contracts.CHECKPOINT_STATES),
        "lifecycleModes": list(contracts.LIFECYCLE_MODES),
        "requiredBehaviors": list(contracts.REQUIRED_BEHAVIORS),
        "requiredLifecycle": list(contracts.LIFECYCLE_DIMENSIONS),
    }
    _assert(evidence.get("requiredSemanticsDigest") == contracts.sha256_value(semantics), "evidence semantics digest", failures)
    ...
    _assert(
        manifest.get("pathFence") == list(contracts.PATH_FENCE)
        and manifest.get("fullFencePaths") == list(contracts.FULL_FENCE_PATHS),
        "manifest path fence",
        failures,
    )
    _assert(manifest.get("pathFenceDigest") == contracts.FENCE_DIGEST and manifest.get("pathFenceCount") == 40, "manifest fence identity", failures)
    _assert(manifest.get("existingPathCount") == 26 and manifest.get("newPathCount") == 14, "manifest classification counts", failures)
    ...
    result: dict[str, Any] = {
        ...
        "priorOverlapCount": contracts.PRIOR_FENCE_PROOF["overlapCount"],
