...
sys.path.insert(0,str(Path(__file__).resolve().parent));import p04_c20_contracts as c
...
try:
 ...
 if a.check:
  ...
  print("C20 generator check PASS_STATIC_PROVISIONAL")
 else:
  for k,v in d.items():(c.ROOT/k).parent.mkdir(parents=True,exist_ok=True);(c.ROOT/k).write_bytes(v)
  print("C20 generator apply PASS_STATIC_PROVISIONAL generated=4")
except Exception as e:print("C20 generator FAIL_STATIC:"+str(e),file=sys.stderr);raise SystemExit(1)
