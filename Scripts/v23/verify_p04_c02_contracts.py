#!/usr/bin/env python3
"""Fail-closed static verifier for V23-P04-C02 tooling artifacts."""
...
import p04_c02_contracts as contracts
...
def _fresh_outputs() -> dict[str, bytes]:
    code = (
        ...
        "sys.path.insert(0,'Scripts/v23');import p04_c02_contracts as c;"
...
def _verify_semantic_artifacts(outputs: dict[str, bytes]) -> list[str]:
    failures: list[str] = []
    try:
        ...
        sequence_bound = schema.get("properties", {}).get("sequenceBound", {})
        if sequence_bound.get("minimum") != 1 or sequence_bound.get("maximum") != 16:
            failures.append("C02 schema sequenceBound must be an integer bounded to 1...16")
        bounded_sequence = evidence.get("boundedSequence", {})
        if bounded_sequence.get("maxItems") != 16:
            failures.append("C02 evidence boundedSequence.maxItems must be 16")
        if contract.get("directPrerequisites") != list(contracts.DIRECT_PREREQUISITES) or contract.get("journeyRefs") != list(contracts.JOURNEY_REFS):
            failures.append("C02 prerequisite/journey semantics differ")
        if contract.get("optionalCapabilityProviders") != ["NONE"]:
            failures.append("C02 optional provider semantics differ")
        semantics = contract.get("semantics", {})
        for key, expected in (("roles", list(contracts.EVIDENCE_ROLES)), ("closedRoleCount", 5), ("comparison", "IMMUTABLE_SIDE_BY_SIDE_AND_TRANSPARENT_ADVISORY_OVERLAY_NEVER_CAUSAL_OR_COMPLIANCE_PROOF"), ("markup", "REVERSIBLE_ARROW_CIRCLE_TEXT_DERIVATIVES_NEVER_MUTATE_ORIGINAL"), ("sequence", "BOUNDED_FLICKER_AND_DETERMINISTIC_CONTACT_SHEET"), ("privacy", "C20_SOLE_REDACTION_AUTHORITY_FINAL_AUDIENCE_CONFIRMATION_REQUIRED")):
            if semantics.get(key) != expected:
                failures.append("C02 semantic field differs:" + key)
        if semantics.get("c05MetadataPersistence") != contracts.c05_metadata_persistence():
            failures.append("C02 C05 metadata persistence differs")
        if semantics.get("plansAndProjectionsPersistence") != contracts.C02_PLAN_PROJECTION_PERSISTENCE:
            failures.append("C02 plan/projection persistence differs")
        if semantics.get("derivativeByteMode") != contracts.C02_DERIVATIVE_BYTE_MODE:
            failures.append("C02 derivative byte mode differs")
        if semantics.get("bundleDecoding") != "EXACT_NO_ADDITIONAL_PROPERTIES":
            failures.append("C02 extra bundle rejection differs")
        if semantics.get("binaryRendering") != {"requiredMediaType": "image/png", "contactSheetBytesRequired": True, "jsonOnlyRenderingForbidden": True}:
            failures.append("C02 binary rendering boundary differs")
        if evidence.get("journey") != "FJ15" or evidence.get("evidenceRoles") != list(contracts.EVIDENCE_ROLES) or evidence.get("uiAdoptionSkipped") is not True:
            failures.append("C02 evidence receipt semantics differ")
        if brand.get("adoptionSkipped") is not True or brand.get("uiAdoptionSkipped") is not True:
            failures.append("C02 UI adoption must remain skipped")
        for value in (evidence, brand):
            if value.get("c05MetadataPersistence") != contracts.c05_metadata_persistence():
                failures.append("C02 artifact C05 metadata persistence differs")
            if value.get("plansAndProjectionsPersistence") != contracts.C02_PLAN_PROJECTION_PERSISTENCE:
                failures.append("C02 artifact plan/projection persistence differs")
            if value.get("derivativeByteMode") != contracts.C02_DERIVATIVE_BYTE_MODE:
                failures.append("C02 artifact derivative byte mode differs")
            if value.get("bundleDecoding") != "EXACT_NO_ADDITIONAL_PROPERTIES":
                failures.append("C02 artifact extra bundle rejection differs")
            if value.get("binaryRendering") != {"requiredMediaType": "image/png", "contactSheetBytesRequired": True, "jsonOnlyRenderingForbidden": True}:
                failures.append("C02 artifact binary rendering boundary differs")
        source_projection = contract.get("sourceProjection", {})
        if source_projection.get("c05MetadataPersistence") != contracts.c05_metadata_persistence():
            failures.append("C02 source projection C05 metadata persistence differs")
        if source_projection.get("bundleDecoding") != "EXACT_NO_ADDITIONAL_PROPERTIES" or source_projection.get("derivativeByteMode") != contracts.C02_DERIVATIVE_BYTE_MODE:
            failures.append("C02 source projection content boundary differs")
        c05_providers = [item for item in contract.get("providerArtifacts", []) if item.get("providerCardID") == "V23-P03-C05"]
        if len(c05_providers) != 1:
            failures.append("C02 accepted C05 provider artifact missing")
        else:
            provider = c05_providers[0]
            for key, expected in (("pathFenceDigest", contracts.ACCEPTED_C05_PROVIDER["pathFenceDigest"]), ("candidateHead", contracts.ACCEPTED_C05_PROVIDER["candidateHead"]), ("candidateTree", contracts.ACCEPTED_C05_PROVIDER["candidateTree"]), ("checkpointDigest", contracts.ACCEPTED_C05_PROVIDER["checkpointDigest"]), ("contextDigest", contracts.ACCEPTED_C05_PROVIDER["contextDigest"]), ("verificationReceiptDigest", contracts.ACCEPTED_C05_PROVIDER["verificationReceiptDigest"]), ("contracts", ["EvidenceMetadataPersistenceEnrollmentV1", "EvidenceAssociationEventRowV1", "EvidenceSequenceRevisionRowV1", "EvidenceMetadataMutationV1", "EvidenceMetadataMutationReceiptV1"])):
                if provider.get(key) != expected:
                    failures.append("C02 accepted C05 provider field differs:" + key)
        for value in (contract, evidence, brand):
            if value.get("statusFlags") != contracts.FLAGS or value.get("nativeCompileRan") is True or value.get("hostedDispatchEnabled") is True:
                failures.append("C02 activation/native/hosted flags differ")
    except (KeyError, TypeError, ValueError, UnicodeDecodeError, json.JSONDecodeError) as exc:
...
def verify(complete: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        failures.extend(_verify_semantic_artifacts(outputs))
    ...
    if complete and missing_new:
        failures.append("incomplete C02 fence; missing changed new paths:" + ",".join(missing_new))
