...
sys.path.insert(0,str(Path(__file__).resolve().parent));import p04_c13_contracts as c
...
try:
 c.authority();r,ready=c.rows();c.semantics(r,ready);n=c.counts();expected={k:c.pretty(v) for k,v in c.documents().items()}
