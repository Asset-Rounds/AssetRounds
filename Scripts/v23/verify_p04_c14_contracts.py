...
try:
 c.authority();r,ready=c.rows();c.semantics(r,ready);n=c.counts()
 for k,v in c.documents().items():
  if not(c.ROOT/k).is_file()or(c.ROOT/k).read_bytes()!=c.pretty(v):fail.append('artifact-drift:'+k)
