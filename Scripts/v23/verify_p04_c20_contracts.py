...
try:
 c.authority();r,ready=c.rows();c.source_pins();c.semantics(ready);n=c.counts();d={k:c.pretty(v) for k,v in c.documents().items()}
 for k,v in d.items():
