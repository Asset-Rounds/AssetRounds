"""Verify Card 49's static C41 corpus, exact fence, and sealed evidence."""
...
import p03_c41_contracts as contracts
...
def _check_corpus(fixture: dict[str, Any], failures: list[str]) -> None:
    _assert(fixture == contracts.CORPUS, "fixture equals deterministic C41 corpus", failures)
    _assert(fixture.get("schema") == "V21P03C41FunctionalRelationshipCorpusV1", "corpus schema", failures)
    ...
    _assert(persistence.get("schemaRelease") == "PERSISTENT_SCHEMA_V12_FUNCTIONAL_RELATIONSHIPS", "persistence schema release", failures)
    _assert(persistence.get("recordSchemaVersion") == 11 and persistence.get("predecessorSchemaVersion") == 11 and persistence.get("predecessorRecordSchemaVersion") == 10, "persistence records 11", failures)
    _assert(persistence.get("migration") == "EXACT_V11_TO_V12_COPY_ON_WRITE", "persistence migration", failures)
    ...
    _assert(persistence.get("persistedFamilies") == ["FunctionalRelationshipTypeDescriptorV1", "AssetFunctionalRelationshipEventV1"] and persistence.get("descriptorEventOnlyPersistence") is True, "descriptor/event-only persistence", failures)
    _assert(persistence.get("currentProjectionRows") == 0 and persistence.get("currentProjectionRowCount") == 0 and persistence.get("recordsSchemaVersion") == 11 and persistence.get("secondGraphStore") is False and persistence.get("secondWriter") is False and persistence.get("c35ReadOnly") is True, "projection/store/writer boundary", failures)
    _assert(fixture.get("currentProjectionRows") == [] and fixture.get("currentProjectionPersistence") == "NONPERSISTENT_REBUILD_ONLY" and fixture.get("c35ReadOnly") is True, "no current projection row", failures)
    _assert(len(fixture.get("relationshipDescriptors", [])) >= 3 and len(fixture.get("relationshipEvents", [])) >= 4, "descriptor/event corpus", failures)
    _assert({row.get("eventKind") for row in fixture.get("relationshipEvents", [])} == set(contracts.EVENT_KINDS), "event kind closure", failures)
    _assert(all(row.get("workspaceRequired") is True and row.get("qualifiedByC39") is True for row in fixture.get("relationshipDescriptors", [])), "workspace/C39 qualification", failures)
    _assert(all(row.get("writes") == 0 for row in fixture.get("dispositionPreviews", [])), "zero-write previews", failures)
    _assert(all(row.get("frozen") is True and row.get("rewriteOnLaterChange") is False for row in fixture.get("completedSnapshots", [])), "frozen snapshot boundary", failures)
    ...
    for fragment in ("cross-workspace", "wrong-endpoint-type", "self-edge", "maximum-cardinality", "cycle", "traversal", "cross-site", "unqualified-package", "c35-structural", "current-projection", "second-graph-store", "second-writer", "inferred-link", "cascade-delete"):
    ...
    for fragment in ("migration", "descriptor", "event", "projection", "disposition", "snapshot", "archive", "restore", "replay"):
    ...
    for fragment in ("backup", "journal", "compatibility", "search", "report", "delete", "released"):
        _assert(any(fragment in case_id for case_id in recovery_ids), f"recovery case:{fragment}", failures)


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--complete", action="store_true", help="require every C41 fence path to be changed")
    ...
    _assert(not unowned_changed, "changed path outside full C41 fence", failures)
    if args.complete:
        _assert(not missing_required_changed, "required changed path missing from full C41 fence", failures)
    ...
    _assert(contracts.FENCE_DIGEST == "c044f055f75dd8a2162aea7db70142b87e67fb3129f2f6a694baf8a10f30a000", "fence digest authority", failures)
    ...
    _assert(sum(row.get("overlapCount", 0) for row in contracts.PRIOR_FENCE_OVERLAPS) == 474, "prior overlap total", failures)
    _assert(len(contracts.PRIOR_FENCE_OVERLAPS) == 31, "prior overlap owner count", failures)
    _assert(contracts.PRIOR_FENCE_PROOF.get("fenceCount") == 49 and contracts.PRIOR_FENCE_PROOF.get("priorOwnedPathCount") == 823, "prior fence proof counts", failures)
    _assert(contracts.PRIOR_FENCE_PROOF.get("authorizedOverlapCount") == 474 and contracts.PRIOR_FENCE_PROOF.get("unauthorizedOverlapCount") == 0, "prior fence authorization", failures)
    ...
    _assert(schema == contracts.schema_document(), "schema does not equal generated C41 corpus schema", failures)
    ...
    _assert(contract.get("artifact") == "V23P03C41FunctionalRelationshipContractV1" and contract.get("status") == "PASS_STATIC_PROVISIONAL", "contract identity/status", failures)
