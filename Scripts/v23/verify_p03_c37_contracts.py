...
def main()->int:
 ...
 expected_persistence={"persistentSchemaVersion":29,"recordsSchemaVersion":28,"persistentKindLifecycleModelCount":102,"durableFamilyCount":2,"persistedFamilies":["AssetPoseEventRow","SpatialAnchorObservationRow"]}
 for key,value in expected_persistence.items():
  if c.PERSISTENCE.get(key)!=value:fail.append(f"persistence:{key}")
 if c.PERSISTENCE_PINS_PENDING:fail.append("persistence pins pending")
 if len(c.POSE_OUTCOMES)!=8 or len(set(c.POSE_OUTCOMES))!=8:fail.append("pose outcomes")
 if len(c.TEST_METHODS)!=5 or {x[len("testV23P03C37")] for x in c.TEST_METHODS}!={"G","A","H","I","R"}:fail.append("selectors")
 ...
 result={"cardID":c.CARD,"result":"PASS" if not fail else "FAIL","complete":a.complete,"persistencePinsPending":c.PERSISTENCE_PINS_PENDING,"fencePathCount":196,"existingPathCount":182,"newPathCount":14,"authorizedOverlapCount":2155,"unauthorizedOverlapCount":0,"failures":fail}
 print(json.dumps(result,indent=2,sort_keys=True) if a.json else f"C37 verifier {result['result']}");return 0 if not fail else 1
