...
final class V9_93AssetLabelOutputTests: XCTestCase {
    ...
    func testV23P04C30R01RetryReexportsExactAcceptedBytesAndPreservesHistoricExportOnlyWarning() async throws {
        ...
        let chrome = GlobalizedSharePrintLabelSurfacesV1(appLanguage: .english)
        let activePresentation = activeExport.sharePresentation(surfaces: chrome)
        XCTAssertEqual(activeExport.documentLanguage?.effectiveLanguage, .english)
        XCTAssertEqual(activePresentation.status, chrome.label(.activeExactReprint))
        XCTAssertNil(activePresentation.warning)
        XCTAssertEqual(activePresentation.claimBoundary, chrome.label(.claimBoundary))
        XCTAssertEqual(activeExport.artifacts.map(\.bytes), accepted.artifacts.map(\.bytes))
        ...
        let historicPresentation = historicExport.sharePresentation(surfaces: chrome)
        XCTAssertEqual(historicPresentation.status, chrome.label(.historicExportOnly))
        XCTAssertEqual(historicPresentation.warning, chrome.doNotDeploy)
        XCTAssertEqual(historicExport.documentLanguage, activeExport.documentLanguage)
