...
final class V30_P04_C04TraditionalChineseLocalizationTests: XCTestCase {
    func testAllTraditionalChineseLanesPreserveEnglishKeysAndTypedResourceStructure() throws {
        let fixture = try load("FieldEvidenceAppTests/Fixtures/V30/Locales/zh-Hant.json")
        ...
        for lane in ["app", "report", "accessibility"] {
            let catalog = try load("FieldEvidenceApp/Resources/Globalization/zh-Hant.\(lane).json")
            XCTAssertEqual(catalog["locale"] as? String, "zh-Hant")
            ...
            for (key, entry) in strings {
                ...
                XCTAssertEqual(Set(localizations.keys), Set(["en", "zh-Hant"]))
                let en = try XCTUnwrap(localizations["en"])
                let target = try XCTUnwrap(localizations["zh-Hant"])
    ...
    func testTraditionalChineseRegionalPreferencesResolveToTheSameBaseResource() throws {
        for profile in ["zh-Hant-US", "zh-Hant-TW"] {
            let resolution = SystemLanguageResolverV1().resolve(
                preferredLanguageIdentifiers: [profile], declaredLocalizationIdentifiers: ["en", "zh-Hant"])
            XCTAssertEqual(resolution.effectiveLanguage.rawValue, "zh-Hant")
            XCTAssertEqual(resolution.resourceIdentifier, "zh-Hant")
        ...
        let otherScript = SystemLanguageResolverV1().resolve(
            preferredLanguageIdentifiers: ["zh-Hans-US"],
            declaredLocalizationIdentifiers: ["en", "zh-Hant"])
        XCTAssertEqual(otherScript.effectiveLanguage, .english)
        XCTAssertEqual(otherScript.provenance, .englishFallback)
    }

    func testTraditionalChineseProfilesRoundTripFormattingWithoutChangingCanonicalValues() throws {
    ...
    @MainActor
    func testTraditionalChineseCriticalCopyKeepsConfirmationTokenAndPermissionPurpose() throws {
        let strings = try catalogStrings("accessibility")
        let erase = try direct(strings, "v30.critical.erase-confirmation")
        XCTAssertEqual(String(format: erase, locale: Locale(identifier: "zh-Hant-US"), EraseAllService.requiredConfirmation),
                       "輸入 ERASE 以確認。")
        ...
        XCTAssertTrue(denied.contains("拒絕"))
    ...
    func testTraditionalChineseReviewBundleKeepsDraftAndStorefrontBoundaries() throws {
        let bundle = try load("docs/design/v30/locales/zh-Hant/V30P04C04ReviewPacketV1.json")
        ...
        XCTAssertEqual(metadata["appStoreLanguage"] as? String, "Chinese (Traditional)")
    ...
    func testChineseBodyShapesPaginatesAndExtractsWithEnglishChromeBoundary() throws {
        ...
        let elements = [try GlobalizedDocumentElementV1(
            semanticID: "zh-hant.authored-body", role: .paragraph, text: body)]
        for paper in [LocalePaperSizeV1.usLetter, .a4] {
            let formatting = try FormattingLocaleProfileV1(localeIdentifier: "zh-Hant-US",
                ianaTimeZoneIdentifier: "America/New_York", calendar: .gregorian,
                numberingSystem: .latin, units: .usCustomary)
            ...
            XCTAssertEqual(result.receipt.orderedSemanticIDs, ["zh-hant.authored-body"])
    ...
    func testTraditionalScriptIdentityAndFileTerminologyStayDistinct() throws {
        let resolver = SystemLanguageResolverV1()
        let declared = ["en", "zh-Hans", "zh-Hant"]
        for profile in ["zh-Hant-US", "zh-Hant-TW"] {
            let traditional = resolver.resolve(preferredLanguageIdentifiers: [profile],
                declaredLocalizationIdentifiers: declared)
            XCTAssertEqual(traditional.resourceIdentifier, "zh-Hant")
        }
        let simplified = resolver.resolve(preferredLanguageIdentifiers: ["zh-Hans-US"],
            declaredLocalizationIdentifiers: declared)
        XCTAssertEqual(simplified.resourceIdentifier, "zh-Hans")
        let strings = try catalogStrings("accessibility")
        let fileEntries = strings.filter { _, row in
            let localizations = row["localizations"] as? [String: [String: Any]]
            let unit = localizations?["en"]?["stringUnit"] as? [String: String]
            return unit?["value"] == "Choose a file"
        }
        XCTAssertFalse(fileEntries.isEmpty)
        for key in fileEntries.keys {
            XCTAssertEqual(try direct(strings, key), "選擇檔案")
        }
        let fixture = try load("FieldEvidenceAppTests/Fixtures/V30/Locales/zh-Hant.json")
        let preparation = try XCTUnwrap(fixture["translationPreparation"] as? [String: Any])
        XCTAssertEqual(preparation["sourceLocale"] as? String, "en")
        XCTAssertEqual(preparation["mechanicalSimplifiedConversion"] as? Bool, false)
        XCTAssertEqual(preparation["independentHumanReview"] as? String, "NOT_EXECUTED")
    }
    ...
    private func assertParallel(_ english: [String: Any], _ chinese: [String: Any], _ key: String) throws {
        ...
        for (field, value) in english {
            if field == "stringUnit" {
                ...
                let targetUnit = try XCTUnwrap(chinese[field] as? [String: String])
    ...
    private func catalogStrings(_ lane: String) throws -> [String: [String: Any]] {
        try XCTUnwrap(load("FieldEvidenceApp/Resources/Globalization/zh-Hant.\(lane).json")["strings"] as? [String: [String: Any]])
    }
    ...
    private func direct(_ strings: [String: [String: Any]], _ key: String) throws -> String {
        ...
        let unit = try XCTUnwrap(localizations["zh-Hant"]?["stringUnit"] as? [String: String])
