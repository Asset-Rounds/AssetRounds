...
final class S10_4AutomatedBrandLabTests: XCTestCase {
    ...
    func testPinnedOverlaySelectorAndExactSevenPlusSevenShardContract() throws {
        ...
        XCTAssertEqual(workValidationRouteSource.utf8.count, 537)
        XCTAssertEqual(
            Data(workValidationRouteSource.utf8).sha256,
            "D892E9C8E7D15C9DA08AFE1DAF1D28885BCE79AFEC70D11130D4664BDC96C696"
        )
        ...
        let restoredWorkValidationAdjacency =
        ...
        XCTAssertEqual(
            workValidationRouteSource.components(
                separatedBy: restoredWorkValidationAdjacency
            ).count - 1,
            1
        )
        XCTAssertEqual(
        ...
        let removedWorkValidationDiagnosticForms = [
        ...
        for removed in removedWorkValidationDiagnosticForms {
            XCTAssertEqual(uiSource.components(separatedBy: removed).count - 1, 0, removed)
            XCTAssertEqual(
                workflowSource.components(separatedBy: removed).count - 1,
                0,
                removed
            )
        }
        let removedWorkValidationDiagnosticGate =
            #"        if automationShard?.shardID == "s10.4.current.ax-text" {"# +
                "\n" +
        ...
        XCTAssertEqual(
            uiSource.components(
                separatedBy: removedWorkValidationDiagnosticGate
            ).count - 1,
            0
        )
        ...
        XCTAssertEqual(workSavingPositioningSource.utf8.count, 13_825)
        XCTAssertEqual(
            Data(workSavingPositioningSource.utf8).sha256,
            "0623371CABEECFA7A5CD8FC4558B5CDD08C5A385FC525B0F0E1F361CF7BD68CC"
        )
        let workSavingDirectGestureAdjacency =
        ...
        XCTAssertEqual(
            workSavingPositioningSource.components(
                separatedBy: workSavingDirectGestureAdjacency
            ).count - 1,
            1
        )
        ...
        for removedWorkSavingDiagnosticForm in [
            ...
            "S10_4_AX_TEXT_WORK_SAVING_DIAGNOSTIC",
            ...
            "diagnosticQuerySample",
            "diagnosticElementObject",
            "diagnosticFrameObject",
        ...
        for retiredWorkSavingExactEquality in [
            ...
            XCTAssertEqual(
                workSavingPositioningSource.components(
                    separatedBy: retiredWorkSavingExactEquality
                ).count - 1,
                0,
                retiredWorkSavingExactEquality
            )
        }
    ...
    func testMinimumOSCameraDeniedLegacyTabCorrectionIsNarrowAndDiagnosticFree() throws {
        ...
        let removedI247DiagnosticForms = [
            ...
            "appScreenshotAttachment",
            "appTreeAttachment",
            "captureRouteAttachment",
            "auditedElementAttachment",
