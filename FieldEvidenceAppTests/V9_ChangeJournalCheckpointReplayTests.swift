...
extension V9_ChangeJournalCheckpointReplayTests {
    func testV30P02C02UnicodeWriterJournalCheckpointReplayAndColdReopenPreserveUTF8() throws {
        ...
        let identifiers = try { () -> (siteID: UUID, assetID: UUID) in
            ...
            let replayResult = try destinationJournal.replayResult(batch)
            let localizedReplay = try replayResult.v30LocalizedState(limits: limits)
            XCTAssertEqual(replayResult.receipt, replayed)
            XCTAssertEqual(replayResult.nextCursor, batch.afterCursor)
            XCTAssertFalse(replayResult.isDeferred)
            XCTAssertEqual(localizedReplay.state, .recovered)
            XCTAssertEqual(localizedReplay.messageKey, .replayRecovered)
            XCTAssertTrue(localizedReplay.permitsSuccessAnnouncement)
            XCTAssertFalse(localizedReplay.claimsRemoteSynchronization)
            XCTAssertEqual(try batch.canonicalData(limits: limits), encodedBatch)
