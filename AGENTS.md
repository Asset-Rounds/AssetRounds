...
The owner sets the primary session model and effort in the app. Owner update, 2026-09-26: use `gpt-6-astra` for coding, choosing supported `low`, `medium` or `high` effort by complexity and risk. Treat “light” as low effort, not a separate unsupported setting. Use Astra high for persistence, migration, restore, security, difficult cross-family reasoning and their independent reviews; medium for understood bounded implementation; low for simple mechanical code changes. Independent consequential review remains nonauthor Astra. Use `gpt-6-sol` medium or high for bounded support work such as log extraction, inventories, evidence summaries and record consistency; it does not replace required Astra safety review. These are task-routing defaults, not claims of lower cost or guaranteed speed.

Reuse an existing relevant helper/context before spawning a replacement merely to change model or effort. Do not restart ongoing work solely to apply these defaults. When a new helper is needed, pass a concise scoped handoff and authoritative file/evidence references rather than unrelated conversation history. Every assignment names the question, owned paths or read-only scope, expected deliverable, constraints, verification and escalation conditions. Record the actual reviewer model and effort; tool limits take precedence over unsupported settings. Repository policy never changes the primary session settings.

Helper budget (owner updated 2026-09-26): up to 7 concurrent active subagents plus root are authorized. After configuration reload, the current tool session exposes 8 total concurrent slots, matching the authorized 7 helpers plus root; actual tool capacity remains the hard limit. An idle helper does not consume active execution capacity. Use only helpers with useful independent work, not every slot by default. Before retirement, write a short handoff so knowledge can be reused. Reuse an existing relevant helper before spawning another; use follow-up/resume to activate it, since a message-only tool may not start a turn. Choose local build/test worker counts from observed CPU, memory and live jobs on this 16 GB Mac, and never rebuild products used by a running native test.
...
Historical owner decision, 2026-09-26: replace Claude roles with GPT-6 Astra. The later task-based Astra/Sol routing above supersedes the all-Astra support-work default. The owner later authorized up to 7 helpers plus root, subject to the actual session capacity described above. The independent reviewer remains read-only and separate from the authors. Historical Claude verdicts and their model attribution are preserved.
